...
coef_l3_mega <- tidy(lm_mega_l3_cohort_site_l) %>%
...
augment(lm_mega_l3_cohort_site_l)
...
coef_l3_no_cohort_mega <- tidy(lm_mega_l3_site_l) %>%
  mutate(site = gsub("site", "", str_extract(term, "site[a-z,_]*")),
         coef_term = ifelse(grepl("log", term), "log", "lin"),
         mod = "l3_no_cohort") %>%
...
gg_decay_mega_plus <-
  ggplot(data = dat_l, 
    ...
    geom_line(data = mutate(augment(lm_mega_l3_cohort_site_l, se_fit = TRUE),
                            time = round(exp(`log(time + 1)`), digits = 0) - 1,
                            age = time + 5,
                            proportion = .fitted + 1,
                            year_abn = as.numeric(levels(cohort))[cohort])) +
...
mega_lm_mod_l3 <- augment(lm_mega_l3_cohort_site_l) %>% 
...
qqPlot(lm_mega_l3_cohort_site_l)
...
# ----------------------------------------------------------------------- #
test1 <- dat_l %>% group_by(site, year_abn) %>% summarize(initial_area_abn = unique(initial_area_abn))
test1 <- ungroup(test1)
