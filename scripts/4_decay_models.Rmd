...
gg_coef_rate_change_by_site_l3_vert <-
  ...
  geom_vline(xintercept = 0, linetype = "dashed") + 
  labs(y = "Site", 
       x = "Rate of change of time to decay by half (years per year)") + 
  theme(legend.position = "none",
        plot.margin = unit(c(0.4, 0.6, 0.2, 0.2), "cm"))
...
mean_initial_area_abn <- dat_l %>% filter(age == 5) %>%
  ...
  group_by(site) %>%
  summarize(mean = mean(initial_area_abn))
...
ggplot(data = mean_initial_area_abn, 
       mapping = aes(y = fct_reorder(site, desc(mean)), x = percent_of_total)) + 
  geom_col() + theme_classic() +
...
ggplot(data = filter(dat_l, site == "shaanxi") %>% 
  ...
  theme_classic() + 
  geom_col() + 
  labs(title = "Initial Area Abandoned in each year, Shaanxi",
       x = "Additional area abandoned", y = "Year"
       )
...
cc_extrapolate_mean_age <- function(extrapolate_to_year = 2100, 
                                    summarize_area = TRUE, 
                                    fitted_values_df,
                                    site_ = "shaanxi") {
  ...
  if(summarize_area) {
    future_df <- future_df %>% 
    summarize(exp_total_area_abn = sum(exp_area_ha, na.rm = TRUE),
              mean_age = 
...
future_df_l3 <- 
  lapply(site_df$site, function(y) {
    lapply(1987:2150, function(x) {
      cc_extrapolate_mean_age(extrapolate_to_year = x,
...
write_csv(extrapolate_df_l3, path = paste0(p_dat_derived, run_label, "/extrapolate_df_l3", run_label, ".csv"))
```
...
gg_extrapolate_mean_age <-
  ggplot(data = extrapolate_df_l3 %>% filter(year < 2122), 
       mapping = aes(x = year, y = mean_age, color = fct_reorder(site, mean_age))) + 
  geom_line(size = 1) + 
  theme_classic() + 
...
extrapolated_df_s <- filter(dat_l, site == "shaanxi") %>% group_by(year) %>% 
  ...
  mutate(
    ...
    expected_mean = sapply(year, FUN = function(i) {
      cc_extrapolate_mean_age(extrapolate_to_year = i, summarize_area = TRUE,
                              fitted_values_df = fitted_df_l3) %>% 
...
gg_extrapolated_area_by_age_bin <-
  ggplot(data = future_df_l3 %>% filter(year <= 2120#, site == "shaanxi"
                                        ),
...
ggsave(plot = gg_extrapolated_area_by_age_bin, 
       filename = paste0(p_plots, run_label, "/decay/", "extrapolate_age_by_age_bin", run_label, ".pdf"), 
...
old_abn_df <- future_df_l3 %>% filter(age > 50#, exp_area_ha > 0
  ...
  group_by(site, year) %>%
  summarize(exp_area_over_50 = sum(exp_area_ha, na.rm = TRUE))
...
write_csv(future_df_l3_no_cohort, path = paste0(p_dat_derived, run_label, "/future_df_l3_no_cohort", run_label, ".csv"))
write_csv(extrapolate_df_l3_no_cohort, path = paste0(p_dat_derived, run_label, "/extrapolate_df_l3_no_cohort", run_label, ".csv"))
...
png(filename = paste0(p_plots, run_label, "/decay/", "extrapolate_mean_age_no_cohort", run_label, ".png"), 
    width = 6, height = 4.5, units = "in", res = 400)
...
png(filename = paste0(p_plots, run_label, "/decay/", "extrapolate_area_abn_no_cohort", run_label, ".png"), 
    width = 6, height = 4.5, units = "in", res = 400)
