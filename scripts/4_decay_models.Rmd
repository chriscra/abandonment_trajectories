...
write_csv(dat_l, file = paste0(p_dat_derived, "abn_decay_data", run_label, ".csv"))
```
...
gg_lin_log_lin_cohort_tmp_s <-
  ...
  geom_function(fun = function(x) {
    ...
    color = viridis(n = 9)[2], size = 1.25, inherit.aes = FALSE) + 
...
ggsave(plot = gg_lin_log_lin_cohort_tmp_s, 
       filename = paste0(p_plots, run_label, "/decay/", "ms_fig_l3_cohort", run_label, "_s.png"), 
  width = 5, height = 4.5, units = "in", dpi = 400)

ggsave(plot = gg_lin_log_lin_cohort_tmp_s, 
       filename = paste0(p_plots, run_label, "/decay/", "ms_fig_l3_cohort", run_label, "_s.pdf"), 
  width = 5, height = 4.5, units = "in")

ggsave(plot = gg_lin_log_lin_cohort_tmp_s +
         scale_color_viridis(), 
       filename = paste0(p_plots, run_label, "/decay/", "ms_fig_l3_cohort_viridis", run_label, "_s.pdf"), 
  width = 5, height = 4.5, units = "in")
...
gg_model_curves_by_site_l3 <-
  ggplot(data = fitted_df_l3 %>% filter(age < 140) %>%
           mutate(site = capwords(ifelse(grepl("herze", site, ignore.case = TRUE),
                                         gsub("_", " & ", site),
                                  ifelse(grepl("bel", site, ignore.case = TRUE),
                                         gsub("belarus", "Belarus/Smolensk", site),
                                  ifelse(grepl("goias", site, ignore.case = TRUE),
                                         gsub("goias", "Goiás", site),
                                  ifelse(grepl("shaanxi", site, ignore.case = TRUE),
                                         gsub("shaanxi", "Shaanxi/Shanxi", site),
                                         gsub("_", " ", site))))))),
...
# for MS
ggsave(plot = gg_model_curves_by_site_l3 + 
         labs(caption = NULL) +
         scale_x_continuous(n.breaks = 15) + 
         theme(legend.position = c(0.875, 0.65)),
       filename = paste0(p_plots, run_label, "/decay/", "mean_decay_curves", run_label, ".pdf"), 
...
# Oscar's recommendation of how to do the linear combination with "car" - but it requires filling out the hypothesis.matrix, which I don't entirely know how to do. So, I ended up doing the lienar combination using multcomp
library(car)
...
Three alternative assumptions include:

2. We could alternatively assume that the amount of abandoned land each year would be more likely to gradually decline, so that less and less land is abandoned each year.
- Linear decline in area abandoned over 30 years, so that 0 ha are abandoned in 2050

3. Limit how much total area can be abandoned, based on the maximum extent of cropland at each site. 
...
cc_extrapolate_abn <- function(extrapolate_to_year = 2100, 
  ...
  future_df <- tibble(
    ...
    mean_initial_area_abn = mean_initial_area_abn %>% filter(site == site_) %>% .$mean_area_abn, 
...
# Proportion of land that is over 30 years old far into the extrapolation:
future_df_l3 %>% mutate(
  ...
  left_join(., 
            ...
            by = "site") %>%
  mutate(prop = 100 * over_30/total_abn_in_2120)
...
ggsave(plot = 
         plot_grid(
           ...
           gg_extrapolation_panel_area_c + 
             scale_x_continuous(breaks = seq(from = 2000, to = 2120, by = 40)),
           labels = "auto", ncol = 2),
       filename = paste0(p_plots, run_label, "/decay/", "extrapolation_combo_SI", run_label, ".pdf"),
