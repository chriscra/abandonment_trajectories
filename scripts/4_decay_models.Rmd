...
time_to_l3 <- bind_rows(#select(mean_coef_df_wide_l3, -contains("half_")), 
                          ...
                          lapply(seq(from = 0.5, to = 1,# 0.9, 
                                     by = 0.1), FUN = function(x) {
                            select(mean_coef_df_wide_l3, -contains("half_")) %>%
                              
                              # (a * lambertW0((b * exp((b + p - 1)/a))/a))/b + 4
...
gg_time_to_range_l3 <-
  ...
  guides(fill = "legend") + scale_x_continuous(breaks = seq(from = 5, to = 50, by = 5))
...
png(filename = paste0(p_plots, run_label, "/decay/", "model_decay_curves_by_site_l3", run_label, ".png"), 
    width = 7, height = 6, units = "in", res = 400)
print(gg_model_curves_by_site_l3 + coord_cartesian(ylim = c(0, 1)))
dev.off()
...
# with quadrants:
coef_df_l3 %>% group_by(site, term) %>% summarize(count = n())
