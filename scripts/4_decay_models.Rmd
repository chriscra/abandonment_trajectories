...
dat_l <- left_join(dat_l,
  ...
  filter(!is.na(proportion)) # remove all na proportions
...
dat_l %>% filter(area_ha > 0) %>% group_by(site) %>% summarize(first_year = min(year_abn))
...
for (i in 1:11) {
  ...
  png(filename = paste0(p_plots, run_label, "/decay/", "l3_cohort", run_label, site_df$label[i], ".png"), 
...
# save
png(filename = paste0(p_plots, run_label, "/decay/", "l3_cohort_coefs_rate_of_change_by_site", run_label, ".png"), 
    width = 5, height = 5, units = "in", res = 400)
