...
write_csv(dat_l, file = paste0(p_dat_derived, "persistence_dat", run_label, ".csv"))
...
dat_l <- read_csv(file = paste0(p_dat_derived, "persistence_dat", run_label, ".csv")) %>%
...
write_csv(area_dat, file = paste0(p_dat_derived, "area_dat", run_label, ".csv"))
...
write_csv(turnover_dat, file = paste0(p_dat_derived, "turnover_dat", run_label, ".csv"))
...
gg_fig4 <- ggplot(data = filter(dat_l, site == site_df$site[9]),
...
ggsave(plot = gg_fig4 + 
       ...
       filename = paste0(p_plots, "fig4.pdf"), 
...
ggsave(plot = gg_fig4, filename = paste0(p_plots, "fig4_green.pdf"), 
...
ggsave(plot = gg_lin_log_lin_cohort_tmp_s, 
       filename = paste0(p_plots, "fig4_cont.pdf"
                         # old path name
...
ggsave(plot = gg_fig4 + scale_color_viridis(discrete = TRUE),
       filename = paste0(p_plots, "fig4_viridis.pdf"), 
  width = 5, height = 4.5, units = "in")


ggsave(plot = gg_lin_log_lin_cohort_tmp_s +
         scale_color_viridis(), 
       filename = paste0(p_plots, "fig4_viridis_cont.pdf"), 
  width = 5, height = 4.5, units = "in")
...
# are the coefficient estimates exactly the same?
all.equal(
  coef_l3_mega %>% select(site, coef_term, year_abn, estimate),
  coef_df_l3 %>% select(site, coef_term = term, year_abn, estimate)
)

tjoin <- left_join(
  coef_l3_mega %>% select(site, coef_term, year_abn, estimate_full = estimate), 
  coef_df_l3 %>% select(site, coef_term = term, year_abn, estimate_indiv = estimate)
  )

# yes, they are identical!
tjoin %>% mutate(test = all.equal(estimate_full, estimate_indiv)) %>% .$test %>% table()
tjoin %>% mutate(diff = estimate_full - estimate_indiv) %>% .$diff %>% round(., digits = 14)
...
ggsave(plot = gg_decay_mega + scale_color_viridis(discrete = TRUE), 
       filename = paste0(p_plots, run_label, "/decay/", "decay_mod_grid", run_label, ".pdf"), 
    width = 8, height = 8, units = "in")


ggsave(plot = gg_decay_mega, 
       filename = paste0(p_plots, run_label, "/decay/", "decay_mod_grid_green", run_label, ".pdf"), 
    width = 8, height = 8, units = "in")
...
for (i in 1:11) {
  qqPlot(lm(formula = I(proportion - 1) ~ 0 + log(time + 1):cohort + I(time):cohort, 
        data = filter(dat_l, year_abn < 2011, site == site_df$site[i])),
...
gg_model_curves_by_site_l3 <-
  ...
  scale_color_discrete(labels = c("belarus" = "Vitebsk, Belarus /\nSmolensk, Russia",
...
ggsave(plot =
       ...
       filename = paste0(p_plots, "/fig3_tall.pdf"),
         # old path name
...
ggsave(plot =
       ...
       filename = paste0(p_plots, "/fig3.pdf"),
...
gg_cohort_coefs_grid <-
  ggplot(data = coef_df_wide_l3 %>% 
           left_join(., select(filter(dt_recult_rate, term == "year_abn"),
                               site, roc_estimate = estimate), 
                     by = "site"),
  ...
  facet_wrap(vars(fct_reorder(site, #mean_time_to_0.5
                              roc_estimate
                              )), 
             scales = "free_y",
             labeller = as_labeller(cap_update_labels))  # see _util_misc_functions.R
...
```{r l3-roc-lincom}
# Discussion with Oscar, May 18, 2021
# Oscar's recommendation of how to do the linear combination with "car" - but it requires filling out the hypothesis.matrix, which was unclear. So, I ended up doing the linear combination using multcomp
library(car)
linearHypothesis(lm_coef_l3, c("year_abn + year_abn:sitewisconsin = 0"))
...
for (i in 1:11) {
  ...
  qqPlot(lm_rate_of_change_ind[[i]], main = site_df$site %>% gsub("_", " ", .) %>% capwords() %>% .[i],
...
However, this second assumption is almost certainly unrealistic.
Two alternative assumptions include:

2.2) We could alternatively assume that the amount of abandoned land each year would be more likely to gradually decline, so that less and less land is abandoned each year.
...
[There are a range of other extrapolations we could also carry out (noted below), but we stick to these.]
2.4) Or, finally, we could base this on the trend in abandoned land each year (running a regression on the gain columns in part d of the main results panel figures).
- There amount of newly abandoned land is based on the trend in the gain in abandonment each year. Conservatively, we assume this to be either declining, or staying constant. Sites where the gain in abandonment each year is increasing is conservatively assumed to be constant through time.

- Constant recultivation rate at each site. 

Future work could include looking at scenarios that change the recultivation (decay) rates:
...
```{r}
area_summary_df %>% select(site, total_site_area_ha_2017, cropland_area_1987, area_abn_ha_2017, area_as_prop_crop, area_as_prop_site) %>%
  mutate(crop_as_prop_site_area = cropland_area_1987 / total_site_area_ha_2017)

round((sum(area_summary_df$area_abn_ha_2017) / sum(area_summary_df$cropland_area_1987)), 
    digits = 4) * 100

area_summary_df %>% select(site, area_abn_ha_2017, cropland_area_1987, area_as_prop_crop) %>% 
  filter(site != "mato_grosso") %>%
  summarise(#mean_prop_crop = mean(area_as_prop_crop),
            total_area_abn = sum(area_abn_ha_2017),
            total_cropland_area = sum(cropland_area_1987)
            ) %>%
  mutate(total_abn_as_prop_of_total_crop = total_area_abn / total_cropland_area)
```
...
gg_trend_in_new_abn <- 
  ...
  theme_classic() +
    geom_col() + 
  labs(y = "Area (10^3 ha)", title = "Trend in newly abandoned cropland each year") +
...
ggsave(plot = gg_trend_in_new_abn,
       filename = paste0(p_plots, "_2021_03_13/decay/trend_in_new_abn.pdf"),
...
obs_v_expected_df <- 
  lapply(1:11, function(i) {
    ...
    dat_l %>% filter(site == my_site) %>% group_by(year) %>% 
      summarise(site = unique(site), observed = sum(area_ha)) %>% 
      mutate(
        expected = 
...
dat_l$age %>% cut(breaks = 5, right = T, include.lowest = T)
min(dat_l$age)
sort(dat_l$age)

# all sites:

gg_obs_v_exp <- ggplot(data = dat_l,
       mapping = aes(x = year, y = area_ha / 10^3, 
...
dat_l %>% filter(site == "shaanxi", year == 2017) %>%
...
## Extrapolating without cohort f.e. (l3)
...
write_csv(future_df_l3_no_cohort, file = paste0(p_dat_derived, run_label, "/future_df_l3_no_fe", run_label, ".csv"))
write_csv(extrapolate_df_l3_no_cohort, file = paste0(p_dat_derived, run_label, "/extrapolate_df_l3_no_fe", run_label, ".csv"))
...
# save
write_csv(mean_persistence_df, file = paste0(p_dat_derived, "mean_persistence_df.csv"))
