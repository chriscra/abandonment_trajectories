...
gg_cohort_coefs_grid <- 
  ...
  facet_wrap(vars(fct_reorder(site, mean_time_to_0.5)), 
             scales = "free_y",
             labeller = as_labeller(cap_labels))  # see _util_misc_functions.R
...
```{r l3-rate-of-change-testing}
coef_df_wide_l3 %>% 
  mutate(time = year_abn - 1987) %>%
  filter(
    site == "chongqing",
    !is.na(time_to_0.5)
    ) %>% 
  lm(time_to_0.5 ~ year_abn, data = .) %>%
  # lm(time_to_0.5 ~ time, data = .) %>%
  tidy(., conf.int = TRUE) %>% mutate(site = "chongqing")
...
t1 <- dt_recult_rate %>%
  filter(term == "year_abn") %>%
  arrange(desc(term), estimate) #%>% print(n = 22)

t1 %>% mutate(diff = conf.low - conf.high) 
t2 %>% mutate(diff = conf.low - conf.high,
              plus = diff - 0.892 * 2)

# plot the new coefficient range:
gg_rate_of_changes_2 <- 
  dt_recult_rate %>% filter(term == "year_abn") %>%
  left_join(., mean_coef_df_wide_l3, by = "site") %>%
  ggplot(data = .,
         aes(y = fct_reorder(site, estimate), 
             x = estimate)) + 
  theme_classic() +
  geom_point() + 
...
# models themselves:
lm_rate_of_change_ind$belarus %>%
  tidy(., conf.int = TRUE)

confint(lm_coef_l3)
library(car)

# oscar's recommendation of how to do the linear combination with "car" - but it requires filling out the hypothesis.matrix, which I don't entirely know how to do.
linearHypothesis(lm_coef_l3, hypothesis.matrix)
...
lm_coef_l3 # all sites together
lm_coef_l3$coefficients
confint(lm_coef_l3$coefficients[1])

install.packages("multcomp")
library(multcomp)
mod_test <- glht(lm_coef_l3, linfct = c("year_abn + year_abn:sitebosnia_herzegovina = 0"))
mod_test_w <- glht(lm_coef_l3, linfct = c("year_abn + year_abn:sitewisconsin = 0"))
...
# save combo with facet grid

# for SI ------------------------- #
ggsave(plot = plot_grid(gg_cohort_coefs_grid,
                        gg_rate_of_changes_2,
                        ncol = 2, rel_widths = c(1, 0.6), align = "h", axis = "b"),
       filename = paste0(p_plots, run_label, "/decay/", "rate_of_change_recultivation_speed_combo", run_label, ".pdf"), 
...
coef_df2_l3 %>% arrange(estimate_slope)

# full set of sites
coef_df_wide_l3 %>% 
  mutate(time = year_abn - 1987) %>%
  filter(!is.na(time_to_0.5)) %>% 
  lm(time_to_0.5 ~ year_abn + site + year_abn:site, data = .) %>%
  # lm(time_to_0.5 ~ time, data = .) %>%
  tidy(., conf.int = TRUE) %>% print(n = 26)


# the confidence interval on the slope is different from the one determined via:
coef_df2_l3 %>% filter(site == "belarus")
-0.892 -5.91
-0.892 +0.803 # cq
( 0.089 + 0.292) 
...
0.32 # estimate
-1.72 # lower CI from individual calculation
2.36
...
-2.07 # lower CI from group lm calculation
2.71

-0.8919682 + 1.21 # estimate
-0.8919682 - 0.188 # low
-0.8919682 + 2.61 # high


lm(time_to_0.5 ~ time * site - 1, 
   data = filter(mutate(coef_df_wide_l3, time = year_abn - 1987), !is.na(time_to_0.5))) %>%
  tidy(., conf.int = TRUE) %>%
  mutate(termm = ifelse(grepl("time", term), "slope", "int"),
         site = gsub("site|time:", "", term)) %>%
   # print(n = 22) %>%
  filter(site %in% c("time", "shaanxi"))
...
offset <- t2 %>% filter(site == "belarus") %>% .$estimate
t1 %>% arrange(site)

plot_coefs_grouped_lm <- t2 %>% arrange(site) %>%
  mutate(adj_est = ifelse(site == "belarus", estimate, estimate + offset),
         adj_CI_low = ifelse(site == "belarus", conf.low, conf.low + offset),
         adj_CI_high = ifelse(site == "belarus", conf.high, conf.high + offset)) %>%
  ggplot(data = ., mapping = aes(x = adj_est, y = fct_reorder(site, adj_est))) + 
  theme_classic() +
  geom_point() + 
  geom_errorbar(mapping = aes(xmin = adj_CI_low, xmax = adj_CI_high), width = 0.2) +
  geom_vline(xintercept = 0, linetype = "dashed")

# coefficients as calculated separately:
gg_rate_of_changes_2

# original confidence intervals:
gg_coef_rate_change_by_site_l3_vert # much wider, calculated by adding the high and low ends of the CI on the first site's slope (Belarus, i.e. the offset)
...
# ----------------------- QQ diagnostics plot ----------------------- #
png(filename = paste0(p_plots, run_label, "/decay/", "rate_of_change_l3_diag_ind_qq2", run_label, ".png"), 
    width = 9, height = 8, units = "in", res = 400)
par(mfrow = c(3, 4))
for (i in 1:11) {
  # plot(lm_rate_of_change_ind[[i]], 2, main = site_df$site %>% gsub("_", " ", .) %>% capwords() %>% .[i])
  qqPlot(lm_rate_of_change_ind[[9]], main = site_df$site %>% gsub("_", " ", .) %>% capwords() %>% .[9], 
...
ggsave(plot = plot_grid(gg_l3_cohort_coef_all + theme(legend.position = "bottom"),
                        gg_coef_rate_change_by_site_l3_vert,
                        ncol = 2, rel_widths = c(1, 0.6), align = "h", axis = "b"),
       filename = paste0(p_plots, run_label, "/presentation/", "rate_of_change_recultivation_speed_old", run_label, ".pdf"), 
       width = 11, height = 6, units = "in")
...
ggsave(plot = plot_grid(gg_cohort_coefs_grid,
                        gg_coef_rate_change_by_site_l3_vert,
                        ncol = 2, rel_widths = c(1, 0.6), align = "h", axis = "b"),
       filename = paste0(p_plots, run_label, "/decay/", "rate_of_change_recultivation_speed_combo_old", run_label, ".pdf"), 
       width = 11, height = 6, units = "in")
