...
lapply(1:11, function(i) {
  ...
  values(site_mean_soc[[1]]) <- 
    soc_mean %>% 
    filter(site == site_df$site[i], timespan == 20) %>% 
    .$mean_annual_soc
  
  values(site_mean_soc[[2]]) <- 
    soc_mean %>% 
    filter(site == site_df$site[i], timespan == 80) %>% 
    .$mean_annual_soc
...
soc_mean %>% arrange(site) %>% print(n = 22)
```
...
# plot
i <- 9
plot(site_forest_c_total[[i]])
plot(site_forest_c_masked[[i]], range = minmax(site_forest_c_total[[i]])[, 1])
plot(site_savanna_mask[[i]])

plot(site_mean_soc[[i]])
plot(site_soc_masked[[i]])
...
## Multiply accumulation rate layers by age time series
...
A new issue: how do I exclude the carbon stored in uncultivated croplands that are less than 5 years, and therefore, not technically considered to be abandoned by our abandonment definition?
Strictly speaking, our current carbon estimate is an overestimate of the carbon stored in abandoned croplands, because it includes carbon sequestration in fallow periods that last between 1-4 years.
This isn't likely to be much, but it's something.

So, my new approach is to exclude carbon sequestration in abandoned croplands that are younger than 5 years. 
...
```{r convert-total-c-to-dt}

site_forest_soc_combined_per_pixel[[9]]

total_c_seq_rate_dt <- spatraster_to_dt(site_forest_soc_combined_per_pixel[[9]])

i
site_df$site[i]

# save as a parquet file
write_parquet(x = total_c_seq_rate_dt, paste0(p_dat_derived, "carbon/", site_df$site[i],
                                              "_forest_soc_combined_per_pixel.parquet"))
toc()

total_c_seq_rate_dt <- read_parquet(file = paste0(p_dat_derived, "carbon/", site_df$site[i],
                                           "_forest_soc_combined_per_pixel.parquet"))

colnames(total_c_seq_rate_dt) <- c("x", "y", "Y20", "YSS")
max_age_dt
total_c_seq_rate_dt
age_dt
...
mdt2 <- spatraster_to_dt(c(age_t[[9]], site_forest_soc_combined_per_pixel[[9]]))
setnames(mdt2, old = "forest_soc_Y20_per_pixel", new = "Y20")
setnames(mdt2, old = "forest_soc_YSS_per_pixel", new = "YSS")
...
```{r merge-combined-c-to-age}
age_dt
total_c_seq_rate_dt

# round x and y columns to deal with floating point imprecision:
round_digits1 <- 8
age_dt[, ':='(x = round(x, round_digits1), 
              y = round(y, round_digits1))]
lc_dt[, ':='(x = round(x, round_digits1), 
              y = round(y, round_digits1))]
total_c_seq_rate_dt[, ':='(x = round(x, round_digits1), 
                           y = round(y, round_digits1))]
tic()
mdt <- merge(
  x = age_dt,#[, c("x", "y", paste0("y", 2010:2017))], 
  y = total_c_seq_rate_dt, 
  all.x = FALSE, 
  by = c("x", "y"), sort = FALSE
  )
toc()
...
nrow(lc_dt)
nrow(total_c_seq_rate_dt)
nrow(age_dt)
nrow(mdt)

mdt_r <- dt_to_raster(mdt, crs("+proj=longlat +datum=WGS84 +no_defs"))
writeRaster(mdt_r, paste0(p_tmp, "mdt.tif"))
mdt_r <- terra::rast(paste0(p_tmp, "mdt.tif"))
plot(mdt_r)
```
...
cvals0 <- 
  site_carbon_df %>% 
  filter(site == site_df$site[i],
         year %in% 2010:2015, type == "Observed") %>%
  .$total_c_Mg

my_year <- "y2011"

cvals <- sapply(paste0("y", 1987:2017), function(my_year) {
  mdt[get(my_year) >= 5 & get(my_year) < 21, sum(get(my_year) * Y20)] + 
  mdt[get(my_year) >= 21, sum(get(my_year) * YSS)]})
...
site_carbon_df %>% 
  filter(site == site_df$site[i],
         ...
         type == "Observed") %>%
  mutate(new = cvals,
         pdiff = (new - total_c_Mg) / total_c_Mg)

abandonment_threshold <- 5

new_cdf <- cc_calc_carbon_accumulation(age_dt = mdt2, site = "shaanxi")
new_cdf1 <- cc_calc_carbon_accumulation(age_dt = mdt2, site = "shaanxi", abandonment_threshold = 1)


cc_calc_carbon_accumulation <- function(age_dt, site,
                                        years_covered = 1987:2017,
                                        abandonment_threshold = 5) {
  carbon_df <- tibble(
    year = years_covered,
    C_up_to_20 = sapply(paste0("y", year), function(my_year) {
      age_dt[get(my_year) >= abandonment_threshold & 
               get(my_year) < 21, sum(get(my_year) * Y20)]
      }),
    C_21_30 = sapply(paste0("y", year), function(my_year) {
      age_dt[get(my_year) >= 21, sum(get(my_year) * YSS)]
      }),
    total_c_Mg = C_up_to_20 + C_21_30,
         site = site,
         type = paste0(">=", abandonment_threshold))
...
new_cdf2 <- 
  rbind(
    site_carbon_df %>% filter(site == site_df$site[i]),
  tibble(year = 1987:2017,
       total_c_Mg = sapply(paste0("y", year), function(my_year) {
         mdt[get(my_year) >= 5 & get(my_year) < 21, sum(get(my_year) * Y20)] + 
           mdt[get(my_year) >= 21, sum(get(my_year) * YSS)]}),
       site = "shaanxi",
       type = ">=5"
  ...
  tibble(year = 1987:2017,
       total_c_Mg = sapply(paste0("y", year), function(my_year) {
         mdt[get(my_year) >= 1 & get(my_year) < 21, sum(get(my_year) * Y20)] + 
           mdt[get(my_year) >= 21, sum(get(my_year) * YSS)]}),
       site = "shaanxi",
       type = ">=1"
       ))
...
ggplot(data = rbind(site_carbon_df %>% filter(site == site_df$site[9]), 
                    new_cdf %>% select(year, total_c_Mg, site, type),
                    new_cdf1 %>% select(year, total_c_Mg, site, type)), 
...
# terra:
global(site_total_c_accumulation[[i]][["y2010"]],
       fun = "sum", na.rm = TRUE) %>% 
    as_tibble() %>% 
    rownames_to_column(var = "year") %>%
    mutate(#year = gsub("y", "", year),
...
cdf2 <- sapply(grep('V', colnames(tdt), value = TRUE), function(i) {  
  ...
  # sum the column age values multiplied by the carbon accumulation rate
...
mdt
rm(tdt)
tdt <- data.table::copy(mdt)

indx <- grep('inc', colnames(tdt))
