...
Crucially, however, such opportunities depend on the duration and persistence of abandonment, yet we lack a detailed empirical understanding of land-use outcomes following abandonment.
Here, we leverage new high resolution annual maps of agricultural abandonment to examine the timing of abandonment at eleven sites that have experienced abandonment since the 1980s.
...
By modeling the trajectories of individual abandoned croplands and their subsequent recultivation, we are able to estimate the length of abandonment, regardless of when it took place in the 30-year time series.
At most sites, more than 50% of abandoned croplands were recultivated within 30 years, and some sites, particularly those Eastern Europe, Russia, and the US, showed accelerating recultivation.
Moreover, if abandonment and recultivation continue at similar rates, the mean duration of abandonment at most sites will likely plateau between `r extrapolate_df_l3 %>% select(site, max_mean_age) %>% unique() %>% arrange(max_mean_age) %>% .$max_mean_age %>% .[1] %>% round(digits = 1)` and `r extrapolate_df_l3 %>% select(site, max_mean_age) %>% unique() %>% arrange(max_mean_age) %>% .$max_mean_age %>% .[9] %>% round(digits = 1)` years by about 2040.
Our results show that counter to recent assumptions, most abandonment is unlikely to be permanent.
Without active efforts to reduce turnover, ecological succession is unlikely to last long enough to produce appreciable gains in biodiversity and carbon.
...
Broad demographic, economic, and environmental changes are reorganizing both people and agriculture, sometimes resulting in cropland abandonment. 
...
Yet until now, scientists have mostly relied on single snapshots in time to estimate abandonment, with misleading results.
Using annual land-cover maps, we track abandonment and recultivation at eleven sites that have recently experienced abandonment.
Contrary to assumptions, abandonment is short-lived and frequently recultivated within 30 years.
Potential environmental benefits largely depend on how long ecosystems have to regenerate in abandoned croplands and accumulate carbon and biodiversity, a long process in many ecosystems.
Until policymakers take steps to reduce turnover, abandonment risks remaining a missed opportunity.
