...
v <- lapply(merge_files$volgograd, FUN = function(i){
  raster(i)
})
v <- lapply(merge_files$volgograd, function(i) raster(i))


plot(v[[1]], maxpixels = 5000)
v
class(v)
tic()
v1 <- stack(v)
...
tic()
writeRaster(v1, filename = paste0(p_dat, "Abandonment/final_maps_raw/volgograd/volgograd.tif"))
toc()
warnings()
"/Users/christophercrawford/Google Drive/_Projects/data/Abandonment/final_maps_raw/volgograd/volgograd.tif"
...
plot(bt$y1987)
plot(input_raster$y1987)

input_raster <- bt

original_codes <- site_dt %>% filter(site == site)

other_original_code <- original_codes %>% select(other) %>% unlist(use.names = FALSE)
woody_original_code <- original_codes %>% select(woody_veg) %>% unlist(use.names = FALSE)
cropland_original_code <- original_codes %>% select(cropland) %>% unlist(use.names = FALSE)
grassland_original_code <- original_codes %>% select(grassland) %>% unlist(use.names = FALSE)

input_raster <- input_raster + 10 # add 10 to the input raster, to facilitate recoding

input_raster[input_raster == 10] <- NA
input_raster[input_raster == other_original_code + 10] <- 1 # set other to 1
input_raster[input_raster == woody_original_code + 10] <- 2 # set woody veg to 2
input_raster[input_raster == cropland_original_code + 10] <- 3 # set cropland to 3
input_raster[input_raster == grassland_original_code + 10] <- 4 # set grassland to 4
```
