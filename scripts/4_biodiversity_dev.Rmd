...
plot(s_pnv, maxpixels = 100000, main = "Shaanxi, PNV",
     ...
     col = rgb(
       filter(pnv_table, Number %in% unique(values(s_pnv)))$R, 
...
plot(b_pnv, maxpixels = 100000, main = "Belarus, PNV",
     breaks = c(0, filter(pnv_table, Number %in% unique(values(b_pnv)))$Number), # note, you need a 0 at the beginning of the breaks
     col = rgb(
       filter(pnv_table, Number %in% unique(values(b_pnv)))$R, 
       filter(pnv_table, Number %in% unique(values(b_pnv)))$G, 
       filter(pnv_table, Number %in% unique(values(b_pnv)))$B, 
       maxColorValue = 255
       )
     )
legend("bottomleft", 
       legend = filter(
         pnv_table, Number %in% 
           unique(values(b_pnv)))$New.global.consolidated.biome.scheme, 
       fill = rgb(
         filter(pnv_table, Number %in% unique(values(b_pnv)))$R, 
         filter(pnv_table, Number %in% unique(values(b_pnv)))$G, 
         filter(pnv_table, Number %in% unique(values(b_pnv)))$B, 
...
# plots
# shaanxi
plot(s_pnv_250, maxpixels = 100000, main = "Shaanxi, PNV",
     breaks = c(0, filter(pnv_table, Number %in% unique(values(s_pnv_250)))$Number), # note, you need a 0 at the beginning of the breaks
     col = rgb(
       filter(pnv_table, Number %in% unique(values(s_pnv_250)))$R, 
       filter(pnv_table, Number %in% unique(values(s_pnv_250)))$G, 
       filter(pnv_table, Number %in% unique(values(s_pnv_250)))$B, 
       maxColorValue = 255
       )
     )
legend("topleft", 
       legend = filter(
         pnv_table, Number %in% 
           unique(values(s_pnv_250)))$New.global.consolidated.biome.scheme, 
       fill = rgb(
         filter(pnv_table, Number %in% unique(values(s_pnv_250)))$R, 
         filter(pnv_table, Number %in% unique(values(s_pnv_250)))$G, 
         filter(pnv_table, Number %in% unique(values(s_pnv_250)))$B, 
...
# belarus
plot(b_pnv_250, maxpixels = 100000, main = "Belarus, PNV",
     breaks = c(0, filter(pnv_table, Number %in% unique(values(b_pnv_250)))$Number), # note, you need a 0 at the beginning of the breaks
     col = rgb(
       filter(pnv_table, Number %in% unique(values(b_pnv_250)))$R, 
       filter(pnv_table, Number %in% unique(values(b_pnv_250)))$G, 
       filter(pnv_table, Number %in% unique(values(b_pnv_250)))$B, 
       maxColorValue = 255
       )
     )
legend("bottomleft", 
       legend = filter(
         pnv_table, Number %in% 
           unique(values(b_pnv_250)))$New.global.consolidated.biome.scheme, 
       fill = rgb(
         filter(pnv_table, Number %in% unique(values(b_pnv_250)))$R, 
         filter(pnv_table, Number %in% unique(values(b_pnv_250)))$G, 
         filter(pnv_table, Number %in% unique(values(b_pnv_250)))$B, 
...
b_area_250 <- pnv_table %>%
  ...
  select(Number, 
         Name = New.global.consolidated.biome.scheme) %>%
  mutate(
    area_percent = (table(getValues(b_pnv_250))/ncell(b_pnv_250))*100, # percentage in each
    area_km2 = 
...
habitat <- raster("/Users/christophercrawford/Google Drive/_Projects/data/Habitats/Jung_GlobalHabitatTypes/iucn_habitatclassification_composite_lvl1_ver003.tif")

# color palette information
habitat_table <- read_delim(file = "/Users/christophercrawford/Google Drive/_Projects/data/Habitats/Jung_GlobalHabitatTypes/iucn_habitatclassification_code_ver003/styles/level1.clr", delim = " ", col_names = c("Number", "R", "G", "B", "Opacity", "Name"))
habitat_table
ht2
pnv_table
habitat_col <- rgb(pnv_table$R, pnv_table$G, pnv_table$B, maxColorValue = 255)
pnv_table
...
plot(b$y2017, main = "Belarus 2017", breaks = plot_breaks, col = plot_cols)
...
plot(b_habitat, main = "Belarus Habitat classes", breaks = c(0, 100, 300, 500, 1400), col = plot_cols[c(2, 4, 4, 3)])
