...
biomes %>% st_geometry() %>% plot(main = biome_names[2])
...
# --------------------------------------------------------------- #
predicts <- ReadPREDICTS(path = "/Users/christophercrawford/Google Drive/_Projects/data/Bd/PREDICTS/database.rds")
...
sites_ln <- sites %>% 
  ...
  mutate(years_ln = log(Years_since_fragmentation_or_conversion),
         LandUse_cond = recode(LandUse, # old = new
...
filter(sites_ln, LandUse == "Secondary vegetation (indeterminate age)")
...
gg_richness_ysv <- ggplot(data = filter(sites_2nd_ln#, LandUse == "Young secondary vegetation"
                     )) +
  theme_classic() +
  geom_point(mapping = aes(x = Years_since_fragmentation_or_conversion, 
                           y = Species_richness#, color = land_cover
                           ), alpha = 0.3) + 
  geom_smooth(method = "lm", mapping = aes(x = Years_since_fragmentation_or_conversion, y = Species_richness#,
                                           #fill = land_cover, 
                                           #color = land_cover
                                           )) +
