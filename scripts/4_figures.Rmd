...
Not all of the code is necessary; this script is included largely for exploratory purposes.
Code chunks are meant to be walked through sequentially.
...
terra::animate(lcc$shaanxi, 
     levels = lc_plot_cols$name, # see "_util/_util_misc.R"
...
# ------------------------------------------- #
# save as gif: age of abandonment
# ------------------------------------------- #
lapply(1:11, function(site_index) {
  ...
  saveGIF(
    terra::animate(age_t[[site_index]][[c(2,2:31)]], 
                   ...
                   colNA = "gray95",
                   # legend = FALSE,
                   # plg=list(title.cex=1.25),
                   mar = c(mval+0.5, mval, mval + 0.2, mval+1.5),
                   n = 1),
    movie.name = file_out_tmp,
    # ani.height = 350, ani.width = 350
)
  
  # Reload:
  tmp_gif <- image_animate(image_read(file_out_tmp), fps = 5)
...
# now for land cover:
# ------------------------------------------- #
lapply(3, function(site_index) {
  file_out_tmp <- paste0(p_plots, "animations/", "land_cover",
                       site_df$label[site_index], "3.gif")
  saveGIF(
    terra::animate(lcc[[site_index]], 
                   levels = lc_plot_cols$name, # see "_util/_util_misc.R"
                   col = lc_plot_cols$color,
                   type = "classes",
                   main = paste0(inset_map_titles[site_index], ", ",
                                 # "Land Cover, ",
                                 1987:2017),
                   maxcell = 500000,
                   colNA = "gray95",
                   legend = FALSE,
                   mar = c(mval+0.5, mval, mval, mval-0.1),
                   n = 1),
    movie.name = file_out_tmp,
    # ani.dev = "pdf", ani.type = "pdf", 
    # ani.height = 350, ani.width = 350,
    # ani.res = 100 #units = "px"
    )
...
lapply(c(1,3,9), function(site_index) {
  tic()
  cc_save_gif(raster = age_r[[site_index]], 
            ...
            frames_per_second = 5,
            file_out = paste0(p_plots, "/", run_label, "/", "animations/age",
                              site_df$label[site_index], ".gif"))
toc()
})
...
lapply(c(1,3,9), function(site_index) {
  file_out_tmp <- paste0(p_plots, "/", run_label, "/", "animations/lcr",
                              site_df$label[site_index], ".gif")
  saveGIF(
    raster::animate(lcr[[site_index]], 
                   # levels = lc_plot_cols$name, # see "_util/_util_misc.R"
                   col = lc_plot_cols$color,
                   main = paste0(inset_map_titles[site_index], ", ", 1987:2017),
                   # main = paste0("Land Cover, ", capwords(site_df$site[site_index]), " ", 1987:2017),
                   maxpixels = 500000,
                   colNA = "gray95",
                   legend = TRUE, n = 1),
    movie.name = file_out_tmp
...
gg_age_class_bins_grid + 
  geom_line(data = area_dat %>% filter(lc == "Abandoned (>=5)"),
            mapping = aes(x = year, y = area_ha / (10^3)),
            size = 2, alpha = 0.8)


# save a version of the above, with the total area abandoned (>= 5, and >=1 years)
ggsave(plot = gg_age_class_bins_grid + 
         geom_line(data = area_dat %>% filter(grepl("Abandoned", lc)),
                   mapping = aes(x = year, y = area_ha / (10^3),
                                 group = lc, color = lc), 
                   size = 1.5) +
         scale_color_manual(values = plot_cols[grep("Abandoned", names(plot_cols))],
                            name = "Total area abandoned"),
       filename = paste0(p_plots, run_label,
                         "/abn_area_by_age_grid_w_total", run_label, ".pdf"),
       width = 7.5, height = 6, units = "in", dpi = 400
)



# FINAL VERSION: save a version of the above, with the total area abandoned (>= 5) and the potential area abandoned
ggsave(plot = gg_age_class_bins_grid + 
         geom_line(data = 
                     bind_rows(
                       area_dat %>% mutate(type = "Observed"),
                   ...
                   size = 1.5) + 
         labs(linetype = "Scenario", fill = "Observed age (years)") + theme(legend.position = c(0.88, 0.11)),
       filename = paste0(p_plots, run_label,
                         "/abn_area_by_age_grid_w_obs_pot_lines", run_label, ".pdf"),
       width = 7.2, height = 5.5, units = "in", dpi = 400
)




# version for ppt, with just the observed area:
ggsave(plot =
         ggplot(data = persistence_dat) + 
           theme_classic() +
           labs(y = expression("Area abandoned (10"^{3}*" ha)") , 
                x = "Year", fill = "Age (years)") +
           theme(legend.title = element_text(size = 10)) +
           scale_fill_brewer(palette = "Greens", direction = 1) +
           facet_wrap(vars(site), scales = "free_y", labeller =
                        as_labeller(cap_update_labels)) +
           theme(legend.position = c(0.885, 0.12), 
                 legend.spacing = unit(2, units = "mm"),
                 legend.margin = margin(t = 0, r = 0, b = 0, l = 0),
                 strip.text.x = element_text(size = 8)) + 
         ...
         labs(linetype = "Scenario", 
              fill = "Observed age (years)",
              alpha = "Scenario") + 
           scale_alpha_discrete(range = c(0, 1)) +
         theme(legend.position = c(0.88, 0.11)),
       filename = paste0(p_plots, run_label,
                         "/abn_area_by_age_grid_w_pot_area_line_only", run_label, ".pdf"),
       width = 7.2, height = 5.5, units = "in", dpi = 400
)
...
ggsave(plot =
         ...
         theme(legend.position = c(0.88, 0.11)),
       filename = paste0(p_plots, run_label,
                         "/abn_area_by_age_grid_w_obs_area_line_only", run_label, ".pdf"),
       width = 7.2, height = 5.5, units = "in", dpi = 400
)
...
ggsave(plot =
         ...
         geom_line(data = 
                   ...
                   size = 1.5) + 
         labs(alpha = "Scenario", fill = "Observed age (years)") + 
           scale_alpha_discrete(range = c(1, 0)) + 
         theme(legend.position = c(0.88, 0.11)),
       filename = paste0(p_plots, run_label,
                         "/abn_area_by_age_grid_w_obs_area_line", run_label, ".pdf"),
       width = 7.2, height = 5.5, units = "in", dpi = 400
)
...
gg_area_lc_facet_grid <-
  ...
  theme(legend.position = c(0.9, 0.12),
        legend.title = element_text(size = 10)) +
  scale_color_manual(values = plot_cols[-1]
                     ) +
...
ggsave(plot = gg_area_lc_facet_grid,
       filename = paste0(p_plots, run_label, "/area_by_lc_grid_all", run_label, ".pdf"), 
...
```{r potential-vs-obs-area-abn}
# ---------------------------------------------------------------------- #
# plot observed area abandoned vs. potential area abandoned, assuming no recultivation
# ---------------------------------------------------------------------- #
gg_potential_area_facet_grid <-
  ggplot(data = 
           bind_rows(area_dat %>% mutate(type = "Observed"),
                     potential_area_dat %>% mutate(type = "Potential")) %>%
           filter(grepl("Abandoned", lc)),
         mapping = aes(x = year, y = area_ha / (10^6), # convert to millions of ha
                       # group = interaction(lc, type), 
                       color = lc, linetype = type)) +
  theme_classic() +
  # theme(axis.text.x = element_text(angle = 320, vjust = 1, hjust = 0)) +
  labs(y = expression("Area  (10"^{6}*" ha)") , 
       x = "Year", 
       # title = "Observed Abandonment vs. Potential Abandonment without recultivation",
       color = "Abandonment\ndefinition", linetype = "Scenario") + 
  geom_line(size = 1.5) + 
  theme(legend.position = c(0.88, 0.11),
  ...
  scale_color_manual(values = brewer_pal(palette = "Greens")(7)[c(2,6)]) +
  # scale_color_manual(values = plot_cols[1:2]) +
  facet_wrap(vars(site), scales = "free", labeller = as_labeller(fancy_labels))



ggsave(plot = gg_potential_area_facet_grid,
       filename = paste0(p_plots, run_label, "/potential_area_grid", run_label, ".pdf"), 
    width = 7, height = 6, units = "in"
)


show_col(brewer_pal(palette = "Greens")(7))
```
...
site_carbon_df %>%
  mutate(
    # old20 = site_carbon_df0$C_up_to_20,
         old21 = site_carbon_df0$C_21_30,
         oldtotal = site_carbon_df0$total_C_Mg,
         # diff20 = all.equal(old20, C_up_to_20),
         diff21 = 100 * (C_21_30 - old21) / C_21_30,
         difftotal = 100 * (total_C_Mg - oldtotal) / total_C_Mg
         ) %>%
  filter(threshold == 5, type == "Observed",
         year %in% c(2012:2017))

# plot:

gg_total_c <-
  ggplot(data = site_carbon_df %>% filter(threshold == 5), 
         mapping = aes(x = year, y = total_C_Mg / 10^6,
                       # group = interaction(type, threshold),
                       # color = as_factor(threshold),
                       linetype = type
                       )) +
  geom_line(size = 1.5, 
            col = brewer_pal(palette = "Greens")(7)[6]) + 
  # scale_color_manual(values = brewer_pal(palette = "Greens")(7)[c(3, 6)]) +
  labs(x = "Year", 
       y = expression("Carbon Accumulated (10"^{6}*" Mg C)"),
       linetype = "Scenario"
       ) +
  theme_classic() + 
  theme(
...
# figure for MS:
ggsave(gg_total_c,
       file = paste0(p_plots, run_label, "/carbon/carbon_accumulated_over_time.pdf"), 
       width = 6.5, height = 5, units = "in", dpi = 400)

# figure for PPT:
# just potential, to be shown before both lines together
gg_total_c_pot <-
  ggplot(data = site_carbon_df %>% filter(threshold == 5), 
         mapping = aes(x = year, y = total_C_Mg / 10^6,
                       linetype = type, alpha = type
                       )) +
  geom_line(size = 1.5, 
            col = brewer_pal(palette = "Greens")(7)[6]) + 
  labs(x = "Year", 
       y = expression("Carbon Accumulated (10"^{6}*" Mg C)"),
       linetype = "Scenario", alpha = "Scenario"
       ) +
  theme_classic() + 
    theme(
      legend.position = c(0.88, 0.11),
...
ggsave(gg_total_c_pot,
       file = paste0(p_plots, run_label, "/carbon/carbon_accumulated_over_time_pot_only.pdf"), 
       width = 6.5, height = 5, units = "in", dpi = 400)
...
site_carbon_tmp <- 
  ...
  filter(#type == "Observed", 
         threshold == 5) %>%
  left_join(
    bind_rows(potential_area_dat %>% mutate(type = "Potential"),
              area_dat %>% mutate(type = "Observed")) %>% 
      filter(grepl(">=5", lc)) %>%
      mutate(threshold = 5) %>% 
      select(site, year, area_abn_ha = area_ha, type, threshold),
    by = c("site", "year", "type", "threshold")) %>%
  left_join(area_summary_df %>% 
              select(site,
                     area_abn_ha_2017,
                     total_site_area_ha_2017, 
                     total_crop_extent_ha,
                     area_2017_as_prop_total_crop), by = "site") %>%
  mutate(var = total_C_Mg /
           # 10^6 /
           area_abn_ha,
         # area_abn_ha_2017
         # total_crop_extent_ha
...
site_carbon_tmp <- 
  site_carbon_tmp %>%
  left_join(site_carbon_tmp %>%
              filter(year == 2017) %>%
              select(site, type, var_2017 = var), 
  by = c("site", "type")
  )

# add inset 
gg_carbon_inset <-
  ggplot(data = site_carbon_tmp %>% filter(!is.na(var)), 
         mapping = aes(x = year, y = var, linetype = type,
                       color = fct_reorder(site, var_2017, .desc = TRUE))) +
  geom_line(
    size = 1
            ) + 
  labs(x = "Year", 
...
# save 
ggsave(gg_carbon_inset,
       file = paste0(p_plots, run_label, "/carbon/carbon_accumulated_per_ha.pdf"), 
...
ggsave(ggdraw() +
         draw_plot(gg_total_c, width = 0.75) +
         draw_plot(gg_carbon_inset, x = 0.575, y = .1, width = .45, height = .3),
       file = paste0(p_plots, run_label,
                     "/carbon/carbon_accumulated_over_time_inset.pdf"), 
       width = 8, height = 5.5, units = "in", dpi = 400)

```


```{r potential-age-bins}
gg_potential_area_age_bins

potential_area_dat
potential_persistence_dat

gg_potential_area_age_bins <-
    ggplot(data = potential_persistence_dat) + 
    theme_classic() +
    labs(y = expression("Area abandoned (10"^{3}*" ha)") , 
         x = "Year", 
         #title = "Area of Abandoned Land, by Age Class",
  ...
  geom_line(data = 
         ...
         mapping = aes(x = year, y = area_ha / (10^3), # convert to millions of ha
                       # group = interaction(lc, type), 
                       # color = lc, 
                       linetype = type),
         size = 1.5)
...
# save
ggsave(plot = gg_potential_area_age_bins,
       filename = paste0(p_plots, run_label,
       "/potential_area_age_bins", run_label, ".pdf"),
       width = 7, height = 5, units = "in", dpi = 400
)
...
gg_turnover_facet_grid <-
  ggplot() +
    ...
    geom_col(data = turnover_dat %>% filter(change != "net"),
             mapping = aes(x = year, y = area_ha / (10^3), 
                           group = change, fill = change)) + 
    geom_line(data = turnover_dat %>% filter(change == "net"),
...
object_size(world_states)
object_size(world_10)

plot(world_10_simple)

world_states_simple <- ms_simplify(world_states, keep = 0.1,
                                   keep_shapes = FALSE)
world_states_simple <- st_make_valid(world_states_simple)

world_10_simple <- ms_simplify(world_10, keep = 0.1,
                               keep_shapes = FALSE)
world_10_simple <- st_make_valid(world_10_simple)

world_states_simple %>% st_drop_geometry() %>% as_tibble() %>% select(name, name_alt, type_en, adm0_label, adm0_a3, geonunit)

# --------------------------- #
# union state boundaries
states_union_simple <- world_states_simple %>% 
  group_by(admin) %>%
  summarise(dissolved = "what") %>% st_cast()

states_union <- world_states %>% 
  group_by(admin) %>%
  summarise(dissolved = "what") %>% st_cast()

states_union %>% filter(geonunit == "Afghanistan") %>% st_geometry() %>% plot()
world_states_simple %>% filter(geonunit == "Afghanistan") %>% st_geometry() %>% plot()

states_union %>% filter(geonunit == "Bosnia and Herzegovina") %>% st_geometry() %>% plot()
world_states_simple %>% filter(geonunit == "Bosnia and Herzegovina") %>% st_geometry() %>% plot()

world_states %>% filter(geonunit == "Bosnia and Herzegovina") %>% st_geometry() %>% plot()
world_states %>% filter(name_en == "Bosnia and Herzegovina") %>% st_geometry() %>% plot()
world_10 %>% filter(name_en == "Bosnia and Herzegovina") %>% st_geometry() %>% plot()


world_states %>% filter(grepl("Bosnia", admin)) %>% st_geometry() %>% plot()
world_states_simple %>% filter(grepl("Bosnia", admin)) %>% st_geometry() %>% plot()
states_union %>% filter(grepl("Bosnia", admin)) %>% st_geometry() %>% plot()
states_union_simple %>% filter(grepl("Bosnia", admin)) %>% st_geometry() %>% plot()
states_union_simple
names(world_states)
states_union$geonunit
```
...
gg_globe_labels <- 
  ...
  theme_minimal() +
  geom_sf(data = world %>% filter(continent != "Antarctica"), 
          fill = NA, color = "gray60", size = sf_width) +
  geom_sf(data = site_sf %>%
            mutate(fancy_labels = fancy_labels), 
          mapping = aes(fill = fancy_labels), size = sf_width2) + 
  labs(#subtitle = "Map projection: longlat", 
       x = NULL, y = NULL) +
  theme(legend.position = "none") + 
  
  geom_label_repel(data = site_sf,
                   aes(label = fancy_labels, geometry = geometry),
                   stat = "sf_coordinates",
                   min.segment.length = 0.0, segment.alpha = 1,
                   point.padding = 0.2, box.padding = 0.2,
                   label.padding = 0.2,
...
ggsave(plot = gg_globe_labels,
       filename = paste0(p_plots, "fig0_labels.pdf"),
       width = 7, height = 3.35, units = "in")
...
gg_biome_world <- 
  ggplot() +
  theme_minimal() +
  geom_sf(data = biomes2017, 
          mapping = aes(fill = BIOME_NAME)) + 
    theme(legend.position = "bottom") + 
    guides(fill = guide_legend(ncol = 3)) + 
    coord_sf(expand = TRUE, crs = st_crs(paste0("+proj=", "eqearth")))

ggsave(plot = gg_biome_world,
       filename = paste0(p_plots, "fig0_biomes.pdf"),
       width = 7, height = 5, units = "in")
```
...
age_t_bins <- lapply(1:11, function(i) {
  ...
  filename = paste0(p_dat_derived, "age_rasters/", run_label, "/2017_bins/", 
...
# reclassify max_age rasters into bins
max_age_t_bins <- lapply(1:11, function(i) {
  terra::classify(
  max_age_t[[i]],
  rcl = select(rcl_df, is, becomes), 
  filename = paste0(p_dat_derived, "max_age/", run_label, "/bins/", 
...
age_t_bins # see "_util_files.R"

# ----------------------------------------------------- #
# horizontal
# ----------------------------------------------------- #
mval <- 1
include_borders_simple <- FALSE
include_borders <- FALSE

{
  pdf(file = paste0(p_output, "plots/fig1", 
                    if(include_borders) {"_borders"},
                    if(include_borders_simple) {"_borders_simple"},
                    run_label, ".pdf"),
      width = 7, height = 6)
...
max_age_t_bins # see _util_files.R
...
{
pdf(file = paste0(p_output, "plots/fig1/fig1_inset_legends", run_label, ".pdf"), 
    width = 4, height = 5)
plot.new()
...
time_to_combo %>% filter(mod == "l3", type == "mean", proportion == 0.5)
...
gg_quadrant <-
  ...
  geom_hline(yintercept = mean(quadrant_df$time_to), 
             linetype="dashed", alpha = 0.4, colour = "red") +
...
# derive mean half-live
quad_end_mean <- 
  quadrant_endpoints %>%
  group_by(site, area_2017_as_prop_total_crop, area_abn_ha_2017) %>%
  summarise(mean_hl = mean(mean),
            median_hl = median(mean)
            )
...
# ------------------------------------------------------------------------------- #
# Quadrant, with endpoint derived half-lives
# ------------------------------------------------------------------------------- #
gg_quadrant_endpoints_all <-
  ggplot(data = quadrant_endpoints, 
       mapping = aes(x = area_2017_as_prop_total_crop,
                     y = mean,
                     size = area_abn_ha_2017/10^3,
                     # group = site,
  ...
  geom_vline(xintercept = mean(quadrant_endpoints$area_2017_as_prop_total_crop), 
             linetype="dashed",# alpha = 0.4, 
             colour = "red") +
  geom_hline(yintercept = mean(quadrant_endpoints$mean, na.rm = TRUE), 
             linetype="dashed", #alpha = 0.4, 
             colour = "red") +
    
    # add half-lives by endpoint:
  geom_point(alpha = 0.6,
    ...
    # add point for mean half-live across 
    geom_point(data = quad_end_mean, # see mean below,
               mapping = aes(x = area_2017_as_prop_total_crop,
                             y = mean_hl,
                             size = area_abn_ha_2017/10^3
                             ),
    ...
    geom_text_repel(data = quad_end_mean, # see mean below,
               mapping = aes(x = area_2017_as_prop_total_crop,
                             y = mean_hl,
                             label = cap_update_labels(site),
                             size = area_abn_ha_2017/10^3
                             ),
               inherit.aes = FALSE,
               box.padding = 1, point.padding = 1, min.segment.length = 0,
                    size = 3, segment.color = 'grey10') +
  labs(x = "Area abandoned in 2017 as a percent (%) of total cropland extent",
       y = "Half-life (years)",
       size = "Area abandoned\n(2017, kha)",
       caption = "Dashed red lines represent mean values",
       color = "Endpoint\n(Years)") +
...
# save 
ggsave(plot = gg_quadrant_endpoints_all,
       filename = paste0(p_plots, run_label, "/quadrant_endpoints_all", run_label, ".pdf"), 
       # width = 9, height = 7,
       width = 7, height = 6,
       # width = 3.42, height = 3.4, 
       units = "in")
...
# ------------------------------------------------------------------------------- #
# just the mean values derived from the endpoints analysis:
# ------------------------------------------------------------------------------- #
  
gg_quadrant_endpoints_mean <- 
  ggplot(data = quad_end_mean, 
       mapping = aes(x = area_2017_as_prop_total_crop,
                     y = mean_hl,
                     label = cap_update_labels(site),
                     size = area_abn_ha_2017/10^3,
  ...
  geom_vline(xintercept = mean(quad_end_mean$area_2017_as_prop_total_crop), 
             linetype="dashed", #alpha = 0.4, 
             colour = "red") +
  geom_hline(yintercept = mean(quad_end_mean$mean_hl), 
             linetype="dashed",# alpha = 0.4,
             colour = "red") +
  ...
  labs(x = "Area abandoned as a percent (%) of total cropland extent",
       y = "Mean half-life (years) across endpoints",
       size = "Area abandoned \n(2017, kha)",
       caption = "Dashed red lines represent mean values") +
  theme_classic() + 
    theme(legend.position = c(0.15, 0.8)) +
...
# ------- save -------- #
# save 
ggsave(plot = gg_quadrant_endpoints_mean,
       filename = paste0(p_plots, run_label, "/quadrant_endpoints_mean", run_label, ".pdf"), 
       width = 7, height = 6, units = "in")

# ------------------------------------------------------------------------------- #
# just the median values derived from the endpoints analysis:
# ------------------------------------------------------------------------------- #
  
gg_quadrant_endpoints_median <-
  ggplot(data = quad_end_mean, 
       mapping = aes(x = area_2017_as_prop_total_crop,
                     y = median_hl,
                     label = cap_update_labels(site),
  ...
  labs(x = "Area abandoned as a percent (%) of total cropland extent",
       y = "Median half-life (years) across endpoints",
       size = "Area abandoned \n(2017, kha)",
       caption = "Dashed red lines represent mean values") +
  theme_classic() + 
    theme(legend.position = c(0.15, 0.8)) +
...
# ------- save -------- #
# save 
ggsave(plot = gg_quadrant_endpoints_median,
       filename = paste0(p_plots, run_label, "/quadrant_endpoints_median", run_label, ".pdf"), 
       width = 7, height = 6, units = "in")


# with box plots:
quadrant_endpoints %>% 
  select(site, area_2017_as_prop_total_crop) %>% 
  unique()

# gg_quadrant_endpoints_box <-
  ggplot(data = quadrant_endpoints, 
       mapping = aes(x = area_2017_as_prop_total_crop,
                     y = mean,
                     # size = area_abn_ha_2017/10^3,
  ...
  geom_hline(yintercept = mean(quadrant_endpoints$mean, na.rm = TRUE), 
    ...
    scale_color_viridis(direction = -1) +
    geom_rug(sides = "bl") + 
    geom_text(data = quadrant_endpoints %>% 
                select(site, x = area_2017_as_prop_total_crop) %>% 
                unique() %>% mutate(y = 0),
              mapping = aes(x, y, label = site), inherit.aes = FALSE,
...
for (i in c(10, 15, 20, 25)){
  gg_quadrant_endpoints_subset <-
  ggplot(data = quadrant_endpoints %>% filter(endpoint %in% i), 
       mapping = aes(x = area_2017_as_prop_total_crop,
                     y = mean,
                     label = cap_update_labels(site),
                     size = area_abn_ha_2017/10^3,
  ...
  geom_vline(xintercept = mean(quadrant_endpoints %>% filter(endpoint %in% i) %>%
                                 .$area_2017_as_prop_total_crop), 
             linetype="dashed", alpha = 0.4, colour = "red") +
  geom_hline(yintercept = mean(quadrant_endpoints %>% filter(endpoint %in% i) %>% 
                                 .$mean, na.rm = TRUE), 
             linetype="dashed", alpha = 0.4, colour = "red") +
  ...
  geom_point(alpha = 0.6) + 
    ...
    geom_text_repel(box.padding = 1, point.padding = 1, min.segment.length = 0,
                    size = 3, segment.color = 'grey10', color = "black") +
  labs(x = "Area abandoned in 2017 as a percent (%) of total cropland extent",
       y = "Half-life (years)",
       size = "Area abandoned\n(2017, kha)",
       subtitle = paste0("Endpoint: ", i, " years"),
       caption = "Dashed red lines represent mean values",
       color = "Endpoint\n(Years)") +
  theme_classic() +
...
# save 
ggsave(plot = gg_quadrant_endpoints_subset,
       filename = paste0(p_plots, run_label, "/quadrant_endpoints_", i, "yr", run_label, ".pdf"), 
       width = 7, height = 6,
       units = "in")
}

quadrant_endpoints %>% filter(endpoint %in% c(10, 15, 20, 25),
                              site == "shaanxi")

# Final figure for manuscript:
# subset 5, 10, 15, 20
gg_quadrant_endpoints_subset_10_25 <-
  ggplot(data = quadrant_endpoints %>% filter(endpoint %in% c(10, 15, 20, 25)), 
       mapping = aes(x = area_2017_as_prop_total_crop,
                     y = mean,
                     label = cap_update_labels(site),
                     size = area_abn_ha_2017/10^3,
                     # color = site, 
                     color = as_factor(endpoint),
                     group = site
                     )) + 
  geom_vline(xintercept = mean(quadrant_endpoints %>% 
                                 filter(endpoint %in% c(10, 15, 20, 25)) %>%
                                 .$area_2017_as_prop_total_crop), 
             linetype="dashed", #alpha = 0.4,
             colour = "red") +
  geom_hline(yintercept = mean(quadrant_endpoints %>% 
                                 filter(endpoint %in% c(10, 15, 20, 25)) %>%
                                 .$mean, na.rm = TRUE), 
             linetype="dashed", #alpha = 0.4, 
             colour = "red") +
  ...
  labs(x = "Area abandoned in 2017 as a proportion of total cropland extent",
       y = "Half-life (years)",
       size = "Area abandoned\n(2017, kha)",
       caption = "Dashed red lines represent mean values",
       color = "Endpoint\n(Years)") +
...
# save 
ggsave(plot = gg_quadrant_endpoints_subset_10_25,
       filename = paste0(p_plots, run_label, "/quadrant_endpoints_10_25yr", run_label, ".pdf"), 
       width = 7, height = 6,
       units = "in")
```
