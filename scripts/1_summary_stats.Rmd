...
area_dat <- area_dat %>% 
  mutate(lc = replace(lc, lc == "Abandoned (>1)", "Abandoned (>=1)"))
...
```{r}
abn_lc_area_2017 <- read_csv(file = paste0(p_dat_derived, run_label, "/", "abn_lc_area_2017", run_label, ".csv"))
...
abn_2yr_ages_df <- lapply(1:11, FUN = function(i){
  tmp_b <- stack(
    raster(abn_2yr_mask[[i]]), # abandonment identified using the two-year method (2017-1987)
    age_r[[i]]$y2017, # abandonment age in 2017, calculated using annual time series, five year abandonment definition
    raster(terra::cellSize(abn_2yr_mask[[i]], unit = "ha", mask = FALSE)) # area (ha), calculated as SpatRaster, then converted to Raster*
               )
...
write_csv(abn_2yr_ages_df, 
          file = paste0(p_dat_derived, run_label, 
                        "/derived_data/", "abn_2yr_ages_df", run_label, ".csv"))


abn_2yr_ages_df <- read_csv(file = paste0(p_dat_derived, run_label, 
                                          "/derived_data/", "abn_2yr_ages_df", run_label, ".csv"))
...
# recultivation age
dt_recult_age <- data.table::copy(dt)
dt_recult_age <- cc_calc_recult_age(dt = dt_recult_age, dt_age = dt_age, dt_diff = dt_diff, abn_threshold = 1)
dt_recult_diff <- cc_diff_dt(dt_recult_age)
recult_length <- cc_extract_length(dt_recult_diff, length_type = "recultivation")
recult_length <- data.table(length = recult_length)


dt_combo <- dt_age - dt_recult_age
}
...
```{r terra-extrapolate-no-recultivation}
# terra solution:
age_t[[9]]
...
```{r seff}
# extract run codes:
list.files("scripts/cluster/slurm_out") %>% grep("*run*", ., value = TRUE) %>%
...
paste("seff", paste0("39537810_", c(2, 4, 5, 11)), collapse = "; ")
paste("seff", paste0("39537811_", c(3, 6)), collapse = "; ")
paste("seff", paste0("39537812_", 9), collapse = "; ")
