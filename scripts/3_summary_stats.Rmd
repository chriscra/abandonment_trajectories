...
ggplot(data = length_distill_df %>% 
  ...
  scale_x_continuous(breaks = seq(from = 5, to = 30, by = 5)) + 
  scale_y_continuous(breaks = c(0, 0.1, 0.13))
...
abn_2yr_mask <- lapply(1:11, function(i) {
  rast(x = paste0(p_dat_derived, "abn_2yr/", site_df$site[i], "_abn_2yr_mask.tif"))
  })
...
# reload
abn_cc_2017_mask <- lapply(1:11, function(i) {
  rast(x = paste0(p_dat_derived, "abn_cc/", site_df$site[i], "_abn_cc_2017_mask.tif"))
  })
...
# calculate the age of the "abandonment" identified in the 2-yr-mask (i.e. those pixels that were cropland in 1987 and non-crop in 2017)

s_2yr_age2 <- terra::mask(x = age_t$shaanxi$y2017, mask = abn_2yr_mask$shaanxi, maskvalues = 1, inverse = TRUE)

freq(abn_2yr_mask$shaanxi)


test1 <- classify(abn_2yr_mask$shaanxi, 
           # takes way longer to change values to 0 than to NA_real_
           rcl = tribble(
                       ~"is", ~"becomes", 
                       NA_real_, 0,
...
test2 <- classify(age_t$shaanxi$y2017, 
           # takes way longer to change values to 0 than to NA_real_
           rcl = tribble(
                       ~"is", ~"becomes", 
                       NA_real_, 0,
...
plot(s_2yr_age2>=0, col = "black")
plot(abn_2yr_mask$shaanxi, col = "black")
...
ncell(abn_2yr_mask$shaanxi)
ncell(age_t$shaanxi$y2017)



# combine the 2yr mask (2017 - 1987) with the age of abandonment in 2017 
# and the area of each pixel

testb <- brick(brick(abn_2yr_mask$shaanxi), 
               brick(age_t$shaanxi)[[31]],
               brick(terra::cellSize(abn_2yr_mask$shaanxi, unit = "km")))
names(testb) <- c("twoyr_2017", "age_2017", "area_km2")
testb


plot(testb$twoyr_2017)
plot(testb$age_2017)

# convert to data.table
testdt <- as.data.table.raster(testb)
object_size(testdt)
testdt[twoyr_2017 > 0]

# filter to those pixels that are counted as "abandonment" in the 2 year method (2017-1987), 
# and count the number of pixels
testdt[twoyr_2017 > 0, .(count = .N), by = age_2017][order(age_2017)]
testdt[twoyr_2017 > 0, .(count = .N, sum_area_km2 = sum(area_km2)), by = age_2017][order(age_2017)]

testdt[twoyr_2017 > 0, sum(area_km2)] # 3555.808 km2

twoyr_age_df <- testdt[twoyr_2017 > 0, 
                       .(count = .N, sum_area_km2 = sum(area_km2)), 
                       by = age_2017
                       ][order(age_2017)] %>%
...
abn_2yr_ages_df <- lapply(1:11, FUN = function(i){
  testb <- brick(brick(abn_2yr_mask[[i]]), 
               brick(age_t[[i]])[[31]],
               brick(terra::cellSize(abn_2yr_mask[[i]], unit = "km")))
...
write_csv(abn_2yr_ages_df, 
          file = paste0(p_dat_derived, run_label, "/derived_data/", "abn_2yr_ages_df", run_label, ".csv"))
...
total_area_by_site <- 
  ...
  summarise(total_pixels = sum(count),
            total_area = sum(sum_area_km2)) #%>% as.numeric()
...
# abn_2yr_ages_df %>% 
  mutate(#total_pixels = 5079796, total_area = 3555.808,
         percent_px = 100 * count/total_pixels,
         percent_area = 100 * sum_area_km2/total_area) %>%
  filter(age_2017 < 5) %>%
  group_by(site) %>%
...
# why would there be values in the 2 year mask that have an NA value in the age mask?
# The NA values are a mix of non abandonment, non-veg, etc. 
# The two year method doesn't allow for some of the cleaning steps we took, like our 5- and 8-year moving windows.
# 
...
# save the summary file
# ----------------------------------------------------------- #
write_csv(abn_2yr_overestimation, file = paste0(p_dat_derived, run_label, "/derived_data/", "abn_2yr_overestimation", run_label, ".csv"))
