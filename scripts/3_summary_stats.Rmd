...
gg_mean_length_ridges <-
  ...
  theme_classic() +
  labs(y = "Proportion of all abandonment periods in each age class", 
...
ggsave(plot = gg_mean_length_ridges + 
         theme(legend.spacing.x = unit(0.5, "mm"),
               ...
               legend.position = c(0.17, -0.18), legend.direction = "horizontal", 
               legend.box = "horizontal",
               plot.margin = margin(5, 5, 45, 5),
               legend.box.just = "left",
               legend.text.align = 0, #legend.margin = c(0,0),
...
ggsave(plot = gg_mean_length_ridges,
       filename = paste0(p_plots, "/fig2_large.pdf"), 
         # paste0(p_plots, run_label, "/mean_lengths_w_ridges_hist", run_label, ".pdf"), 
...
gg_raw_hist <-
  ggplot(data = filter(length_distill_df, #site == site_df$site[9], 
                     length_type == "all",
                     length >= 5)) + 
    theme_classic() +
...
area_summary_df <- area_summary_df %>%
  mutate(area_as_prop_site = area_abn_ha_2017/total_site_area_ha_2017,
         area_as_prop_total_crop = area_abn_ha_2017/total_crop_extent_ha,
