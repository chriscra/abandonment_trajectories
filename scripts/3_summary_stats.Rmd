...
gg_mean_length_by_threshold <- 
  ...
  theme_classic() +
  theme(axis.text.x = element_text(angle = 320, vjust = 1, hjust = 0, size = 5),
        plot.caption = element_text(face = "italic"),
...
# ----------------- save ---------------- #
png(filename = paste0(p_output, "plots/", run_label, "/mean_lengths_by_threshold1", run_label, ".png"), 
...
# alternative plot, with violin plots? or point clouds?
...
gg_mean_length_ridges <-
  ...
  # scale_y_discrete(breaks = site_df$site, 
  #                  labels = site_df$description) + 
  
  scale_x_continuous(breaks = c(5, 10, 15, 20, 25, 30), expand = c(0, 0)) + 
  theme_classic() +
  labs(y = "Site", x = "Mean abandonment duration (years)",
       color = "Mean of:", linetype = ""
       ) + 
  ...
  guides(color = guide_legend(order = 1, nrow = 2, override.aes = list(linetype = 0)
                              ), 
...
ggsave(plot = gg_mean_length_ridges,
       filename = paste0(p_plots, "/fig2.pdf"), 
         # paste0(p_plots, run_label, "/mean_lengths_w_ridges_hist", run_label, ".pdf"), 
...
gg_raw_hist <- ggplot(data = filter(length_distill_df, #site == site_df$site[9], 
                     length_type == "all",
                     length >= 5)) + 
    theme_classic() +
...
ggplot() +
  geom_density_ridges(
    data = vt, 
    mapping = aes(y = site, x = length, group = site),
    stat = "binline", bins = 26,
    scale = 0.95,
    draw_baseline	= FALSE
    ) + 
...
# 
# tic()
# tt_area <- expanse(total_crop_mask, unit = "ha")
# toc() # much much faster than the cellSize %>% global() method below, but crashes with the full site rasters (except shaanxi, for some reason)
# 
# tic()
# tt_cell_areas <- cellSize(total_crop_mask, unit = "ha", mask = TRUE)
# global(tt_cell_areas, fun = "sum", na.rm = TRUE)
# toc()

# ------------------------------------------------------ #
# trying this for all rasters:
# ------------------------------------------------------ #
tmpFiles(current = T, remove = TRUE)
...
lc <- lapply(seq_along(site_input_raster_files), function(i) {rast(site_input_raster_files[i])})
...
tic()
lc_total_crop_mask <- lapply(lc, function(x) {
  reclassified <- classify(x, rcl = tribble(
  ~"is", ~"becomes", 
  0, NA,
  1, NA,
  2, NA,
...
if(!dir.exists(paste0(p_dat_derived, "total_crop_mask/"))) {
  dir.create(paste0(p_dat_derived, "total_crop_mask/"))
}

# write new total_crop_mask rasters to file:
lapply(1:11, function(i) {
  writeRaster(lc_total_crop_mask[[i]], 
...
for(i in 1:11) {
  print(names(lc_total_crop_mask[i]))
  print(freq(lc_total_crop_mask[[i]]))
  }


# this seems to crash with the larger places. so, try the manual method of calculating cellSize
# expanse(lc_total_crop_mask$shaanxi, unit = "ha")

# terra method (this yields a different area estimation than raster:area(), due to different cell sizes)
lc_total_crop_mask_area_ha_terra <- sapply(1:11, function(i) {
  size <- cellSize(lc_total_crop_mask[[i]], unit = "ha", mask = FALSE)
  product <- size * lc_total_crop_mask[[i]]
  area_ha <- global(product, fun = "sum", na.rm = TRUE)
...
# raster method
tic()
lc_total_crop_mask_area_ha_raster <- sapply(1:11, function(i) {
  # calculate area raster
  bin <- raster(lc_total_crop_mask[[i]])
  size <- raster::area(bin)
  product <- size * bin
  area_ha <- cellStats(product, "sum") * 100
  area_ha
  })
toc()

raster(lc_total_crop_mask[[i]])
lc_total_crop_mask[[i]]
lc_total_crop_mask_area_ha_terra / 
lc_total_crop_mask_area_ha_raster

total_crop_mask <- tibble(site = site_df$site,
       total_crop_extent_ha_t = lc_total_crop_mask_area_ha_terra,
       total_crop_extent_ha_r = lc_total_crop_mask_area_ha_raster
       )
...
area_ever_abn <- lapply(1:11, function(i) {
  ...
  # area_r <- cellSize(bin, unit = "ha", mask = FALSE, transform = FALSE)
  # plot(area_r)
  ...
  tibble(site = site_df$site[i],
         ...
         # area_ever_abn_ha = global(bin_area, fun = "sum", na.rm = TRUE) %>% as.numeric()
  )
  }
) %>% bind_rows()
toc() # about 350 seconds

area_ever_abn_original
area_ever_abn2
area_ever_abn
area_ever_abn3
identical(area_ever_abn, area_ever_abn3)


area_ever_abn %>% summarise(total_ever_abn_Mha = sum(area_ever_abn_ha)/1e6)
# area_ever_abn3 %>% summarise(total_ever_abn_Mha = sum(area_ever_abn_ha))
...
area_summary_df <- area_summary_df %>% 
  ...
  left_join(., total_crop_mask, by = c("site")) %>%
...
area_summary_df <- area_summary_df %>%
  mutate(area_as_prop_site = area_abn_ha_2017/total_site_area_ha_2017,
         area_as_prop_total_crop = area_abn_ha_2017/total_crop_extent_ha_r,
         area_as_prop_crop87 = area_abn_ha_2017/cropland_area_1987)
...
# ---------------------------------------------------------------- #
i <- 9
lct_87_17 <- lapply(1:11, function(i) {lct[[i]] %>% subset(., c("y1987", "y2017"))})

names(lct_87_17) <- site_df$site
plot(lct_87_17$shaanxi)
# ---------------------------------------------------------------- #
# 3. reclassify the rasters, so that it's just cropland in 1987 (0-1), and just cropland in 2017 (0-1)
# ---------------------------------------------------------------- #
lct_rcl <- lapply(1:2, function(i) {
  classify(lct_87_17[[i]], 
           rcl = tribble(
             ~"is", ~"becomes", 
             NA, 0,
             0, 0,
             ...
             3, 1,
             4, 0),
           filename = paste0(p_dat_derived, "input_rasters/subsets/", site_df$site[i], "_87_17_rcl.tif"))
  }
  )
names(lct_rcl) <- site_df$site
tmpFiles(current = TRUE, )

# ---------------------------------------------------------------- #
# 4. Subtract the two (1987 - 2017), so that pixels that were cropland (1) in 1987, but not in 2017 (0) (i.e. are "abandoned" by this definition) have a value of 1.
# Pixels that are cropland in both will be 0 (as will pixels that are non cropland in both)
# Pixels that are cropland in 2017 (1) but not in 1987 (0) will have a value of -1
# ---------------------------------------------------------------- #
lct_sub <- lapply(1:11, function(i) {
  lct_rcl[[i]]$y1987 - lct_rcl[[i]]$y2017
  }
  )

names(lct_sub) <- site_df$site


# ---------------------------------------------------------------- #
# 5. calculate the area in each class
# ---------------------------------------------------------------- #

abn_area_2017_2yr <- sapply(1:11, function(i) {
  abn_mask <- classify(lct_sub[[i]], rcl = 
             tribble(
               ~"is", ~"becomes", 
               -1, NA,
               0, NA,
  ...
  size <- cellSize(abn_mask, unit = "ha", mask = TRUE)
...
# make abn mask
abn_age_mask_cc <- lapply(1:11, function(i) {
  classify(age_t[[i]]$y2017, 
           rcl = tibble("is" = 0:30, becomes = if_else(is < 5, NA_real_, 1))
           )})

names(abn_age_mask_cc) <- site_df$site

plot(abn_age_mask_cc$shaanxi)

abn_area_2017_cc <- sapply(1:11, function(i) {
  size <- cellSize(abn_age_mask_cc[[i]], unit = "ha", mask = TRUE)
  area_ha <- global(size, fun = "sum", na.rm = TRUE)
  as.numeric(area_ha)
  })

abn_area_2017_cc %>% sum()/1e6
...
area_summary_df %>% 
  ...
  mutate(area_ha_2017_terra = abn_area_2017_cc#,
...
abn_mask_2yr_01 <- lapply(1:11, function(i) {
  classify(lct_sub[[i]], rcl = 
             tribble(
               ~"is", ~"becomes", 
               -1, 0,
               0, 0,
...
abn_age_mask_cc_01 <- lapply(1:11, function(i) {
  classify(age_t[[i]]$y2017, 
           rcl = tibble("is" = 0:30, becomes = if_else(is < 5, 0, 1))
           )})

jaccard_similarity <- lapply(1:11, function(i) {
  add <- abn_mask_2yr_01[[i]] + abn_age_mask_cc_01[[i]]
  intersection <- classify(add, rcl = tribble(~"is", ~"becomes", 
                                              0, NA,
                                              1, NA,
                                              2, 1))
  
  union <- classify(add, rcl = tribble(~"is", ~"becomes", 
                                              0, NA,
                                              1, 1,
                                              2, 1))
  
  int_area <- intersection %>% 
