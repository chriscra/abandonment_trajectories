...
# calculate area abandoned summary stats:
# ----------------------------------------------------------- #
area_summary_df <- 
  area_dat %>% 
...
# ----------------------------------------------------------- #
# 2. 
# calculate the are at each site that is abandoned at any point during the time series (>= 5 years)
# ----------------------------------------------------------- #
...
# ---------------------------------------------------------------------- #
# What percentage of the abandonment periods are recultivated?
# ---------------------------------------------------------------------- #
...
# area_recultivated_df <- 
  ...
  summarise(total_area_abn_remaining_2017 = sum(area_ha),
            ...
            total_area_abn_recultivated_2017 = 
              total_initial_area_abn - 
              total_area_abn_remaining_2017) %>%
  mutate(prop_recult_5 = total_area_abn_recultivated_2017/total_initial_area_abn)

persistence_dat %>% 
  filter(age >= 10, year == 2017, site == "shaanxi") %>% print(n = 30)





# add the initial_area_abn based on the threshold = 10
persistence_dat %>% 
  left_join(.,
            persistence_dat %>% filter(age == 10) %>% 
              select(site, year_abn, initial_area_abn_10 = area_ha), 
            by = c("site", "year_abn")) %>%
   # filter(age >= 10, year == 2017, site == "shaanxi") %>% print(n = 30)
  filter(year == 2017, age >= 10) %>%
  group_by(site) %>%
  summarise(total_area_abn_10_remaining_2017 = sum(area_ha),
            total_initial_area_abn_10 = sum(initial_area_abn_10),
            total_area_abn_10_recultivated_2017 = 
              total_initial_area_abn_10 - 
              total_area_abn_10_remaining_2017) %>%
  mutate(prop_recult_10 = total_area_abn_10_recultivated_2017/total_initial_area_abn_10)

persistence_dat %>% filter(age %in% c(5, 10), year_abn %in% 1988:1992, site == "shaanxi")
```
