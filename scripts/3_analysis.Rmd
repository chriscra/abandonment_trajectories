...
# site <- "belarus"
for (run in 1:7) {
  load(file = paste0(p_output, "frag_", site, run, ".rds"),
       verbose = TRUE)
}

frag_s_l <- vector(mode = "list", length = 7)
names(frag_metrics_l) <- metrics_list
for (run in 1:7) {frag_s_l[[run]] <- eval(parse(text = paste0("frag_", site, run)))}

frag_s <- bind_rows(frag_s_l)

frag_s$metric %>% unique
