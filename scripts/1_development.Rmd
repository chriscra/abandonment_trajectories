...
# ------------------------------------------------------------------
# trying to calculate length of time, or age, of each pixel
tt <- copy(DT_diff)

DT # 2 = noncrop, 1 = crop
bin # 1 = noncrop, 0 = crop
tt # differences: 1 = transition to noncrop, -1 transition to crop
gg_10_px_traj

age <- copy(bin)

# start by playing with one row:
age[1] %>% length
for (i in names(age)) {
  if (age[1, ..i] > 0) {
    age[1, ..i] <- age[1, ..(i-1)] + 1
...
if (age[1, "y1988"] > 0) {
    age[1, "y1988"] <- age[1, "y1987"] + 1
}

age[1, "y1988"]
nms <- names(age)
nms[1]
age[1, ..nms] # this works, but how does one subset?

nms_1 <- nms[1] # you can either subset like this, but it would require assigning in each iteration
age[1, ..nms_1]
age[1][[names(age)[i + 1]]]
age[1, names(age)[i + 1], with = F] # returns a data.table
age[1][[names(age)[i + 1]]] # returns a vector


# this is just one row/pixel:
bin
age
for (i in 2:ncol(age)) {
  if (age[1][[names(age)[i]]] > 0) {
    age[1][[names(age)[i]]] <- age[1][[names(age)[i - 1]]] + 1
}
}


# nested loop to do all rows:


tic()
for (i in 1:nrow(age)) {
  for (j in 2:ncol(age)) {
  if (age[i][[names(age)[j]]] > 0) {
    age[i][[names(age)[j]]] <- age[i][[names(age)[j - 1]]] + 1
    }
  }
}
toc() # 0.131 sec for just 15 rows
# for 15,000 rows, it should be 1000x longer, 0.131 * 1000 = 131 seconds
# 254.958 sec.

(94000000/15 * 0.131) / 60 / 60 # jeeeeez 228 hours



# trying it now in the data.table syntax:
dt <- age

# first, write a function:
calculate_age <- function(dt) {
  # dt must be a 1 row dt
  for (j in 2:ncol(dt)) {
    if (dt[1][[names(dt)[j]]] > 0) {
      dt[1][[names(dt)[j]]] <- dt[1][[names(dt)[j - 1]]] + 1
    }
  }
  return(dt)
}
...
tic()
for (i in 1:15) {
  calculate_age(dt = dt, row_num = i)
}
toc()

vec <- unlist(dt[1], use.names = FALSE)
vec[1]
for (i in 2:length(vec)) {
  if (vec[i] > 0) {vec[i] <- vec[i - 1] + 1}
}
...
dt[, sum := NULL][, rowsum := NULL]
dt
paste0("v", 1:14)
dt[, c("v1", "v2", "v3", "v4", "v5" , "v6",  "v7" , "v8" , "v9" , "v10", "v11", "v12", "v13", "v14") := calculate_age(dt = .SD), by = seq_len(nrow(dt))]
new_col_names <- paste0("v", 1:14)



# this finally works!
# a single line, looped over the columns
age <- copy(bin)
age[1988 > 0, y1988 := y1987 + 1][] # simple
age[get(names(age)[2]) > 0, c(names(age)[2]) := get(names(age)[1]) + 1] # coded to accept names with indices to be looped over
...
age <- copy(bin)
for (i in 2:ncol(age)) {
  age[get(names(age)[i]) > 0, # subset rows that are greater than 0 (i.e. 1, for noncrop), and
      c(names(age)[i]) := get(names(age)[i-1]) + 1] # set them equal to the previous column's value in that row, plus 1.
}
print(age)
...
# --------------------------------------------
# trying it with set
# so, this worked too, but it's a bit slower
tic()
for (x in 2:ncol(age)) {
  set(age, 
...
toc() # 94.22 seconds for 64 million rows, so I'll use the other since it's simpler, and keep this in the back pocket until then. 
