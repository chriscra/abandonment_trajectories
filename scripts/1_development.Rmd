...
test <- na.omit(dt_merge[1:100000])


tic(); cc_update_lc(dt_merge); toc() # 46.621, nice, this seems to have worked.
# to remove all rows with NA values
dt_merge <- na.omit(dt_merge) # this removed about 2 million rows

tic(); dt_merge <- dt_merge - 1; toc() # 6.8 GB, didn't work
tic(); cc_make_binary(dt_merge); toc() # 6.8 GB,
...
# benchmarking
tic(); calc_age(big); toc() # 88.971 seconds for 62.9 million rows, 14 col
...
# We could start by simply removing pixels with an age in the final year equal to the number of years in the time-series.
# 3. 
...
which(row1 == 0)[1] # this is the index of the first column with a zero. This is where the periods start to refer to abandonment age.
which(dt[5,] == 0)[1]
...
# -------------------------------
# set rows with no 0s to NA (i.e. consistently non-crop)
# -------------------------------
names(dt)[ncol(dt)]
set(dt, i = which(dt[["V14"]] == 14), j = 1:ncol(dt), value = NA) # first code.
set(dt, i = which(dt[[length(dt)]] == length(dt)), j = 1:length(dt), value = NA) # to work with any number of columns. lenght() works just the same as ncol() but apparently is a bit faster

dt

# -------------------------------
# filter out rows that are consistently crop, (have a row sum of 0, so no years with noncrop)
# -------------------------------
# calculate rowsum
dt # can be age, or binary 0 or 1
dt[, rowsum := rowSums(.SD)][rowsum == 0] # can also include .SDcols = 1:14

dt[, rowSums(.SD) > 0] # you can have an expression in j, which returns logical vector

dt[dt[, rowSums(.SD) > 0]]


# -------------------------------
# select only cells that are noncrop in first year: dt[V1 > 0]
# -------------------------------
dt[V1 > 0]
...
# diff
# -------------------------------
# what's the best way to identify where the first transition happens?
...
dt
dt_lead <- copy(dt)
dt_lead[, V1 := NULL][, end := NA]

dt_diff <- dt_lead - dt # this works just the same.
dt_diff[, end := NULL]
dt_diff

for (i in 1:nrow(dt[V1 > 0])){
  set(dt[V1 > 0], # select only those rows that are not 0 in the first year
      i, # row
      1:(which(dt[V1 > 0][i, ] == 0)[1] - 1), # columns
      value = NA
      )
}
...
# by columns... how do I select 0
dt <- cc_create_dt()
dt[V1 > 0, V1 := if(V2 == 0) {NA} else {V1}]
# select all positive values and make them NA, until you reach a cell whose value to the right is 0



filter_abn <- function(dt) {
  for (x in 2:ncol(dt)) {
    set(dt, 
        which(dt[[x]] > 0), 
        j = x, 
...
# extract periods from each pixel:
dt[]

# code with set
for (x in 2:ncol(age)) {
  set(age, 
      which(age[[names(age)[x]]] > 0),  # the dt[[colname]] returns the column as a vector. nice!
      j = names(age)[x], 
      value = age[get(names(age)[x]) > 0, get(names(age)[x-1])] + 1)
}
print(age)

# old code, row by row - VERY SLOW
for (i in 1:nrow(dt)) {
  for (j in 2:ncol(dt)) {
    if (dt[i][[names(dt)[j]]] > 0) {
      dt[i][[names(dt)[j]]] <- dt[i][[names(dt)[j - 1]]] + 1
