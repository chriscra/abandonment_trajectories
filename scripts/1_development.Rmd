...
for (i in 1:(ncol(age) - 1)) {
  if (age[1][[names(age)[i + 1]]] > 0) {
    age[1][[names(age)[i + 1]]] <- age[1][[names(age)[i]]] + 1
}
}
...
age[1]
bin[1]
# turn this into a function, then send this across all rows of the data.table using the by = 1:nrow()
...
age[1, ..nms] # returns the whole vector

age[1][["y1987"]]
# both of these work
age[1, nms[1], with = FALSE]
age[1][[names(age)[1]]]
...
age[, sum := sum(.SD, na.rm = TRUE), by = 1:nrow(DT_diff), .SDcols = 1:13]
