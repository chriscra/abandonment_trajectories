...
tic("load raster")
r <- brick(paste0(p_dat, "Abandonment/belarus_small.tif"))
...
dt1 <- copy(dt)
dt1[, c("x", "y") := NULL]
identical(dt[, -c("x", "y")], dt1)

tic("write out cleaned abandonment age data.table")
fwrite(dt, file = paste0(p_dat_derived, "age.csv"))
toc()
...
# write out length vector
fwrite(length, file = paste0(p_dat_derived, "length.csv"))

dt_length <- data.table(length = length)
dt_length[, .N, by = lengths]
