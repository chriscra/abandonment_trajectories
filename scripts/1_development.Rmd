...
# cc_count_blips_ind(dt, recultivation_threshold = 1)


# tic("write out blips_count")
# print("writing out blips_count data.frame")
# fwrite(blips_count, file = paste0(path_out, name, "_blips_count.csv"))
# toc(log = TRUE)
dt <- cc_create_bin()
cc_fill_blips(dt, recultivation_threshold = 3, replacement_value = 1)
cc_calc_age(dt)
dt3 <- dt
...
s
str_detect(class(s), "Raster")
grep("Raster", class(s)[1]) > 0

cc_process_rasters(site = "shaanxi",
                   path = p_dat_derived,
                   label = "blip1",
                   clean_blips = TRUE)
