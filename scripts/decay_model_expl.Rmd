...
your_path <- "/Users/christophercrawford/Google_Drive/_Projects/abandonment_trajectories/data_derived/" # 
...
ggplot(data = filter(dat_l, site == "shaanxi"), 
       mapping = aes(x = time, y = proportion,
                     group = year_abn, color = year_abn)) + 
  geom_point() + scale_color_distiller(palette = "Greens") + 
  # scale_x_continuous(n.breaks = 30) + 
  labs(title = "I(proportion - 1) ~ 0 + (log(time_1)):cohort + I(time_1):cohort") + 
  geom_line(data =
              mutate(
...
ggplot(data = filter(dat_l, site == "shaanxi"), 
  ...
  # scale_x_continuous(n.breaks = 30) + 
  labs(title = "I(proportion - 1) ~ 0 + (log(time_1)):cohort + I(time_1):cohort") + 
  geom_line(data = mutate(augment(lm(formula = I(proportion - 1) ~ 
                                      0 + (log(time_1)):cohort + 
                                      I(time_1 - 1):cohort,
                                    #time:cohort, 
                                    data = filter(dat_l, site == "shaanxi"))),
...
ggplot(data = filter(dat_l, site == "shaanxi"), 
  ...
  labs(title = "I(proportion - 1) ~ 0 + (log(time_1)):cohort + I(time_1):cohort") + 
  geom_line(data = mutate(augment(lm(formula = I(proportion - 1) ~ 
                                       0 + log(I(age-4)):cohort + 
                                       I(age-5):cohort,
                                     data = filter(dat_l, site == "shaanxi"))),
                          #time_1 = round(exp(`log(time_1)`), digits = 0),
                          age = `I(age - 5)` + 5,
                          proportion = .fitted + 1,
                          year_abn = as.numeric(levels(cohort))[cohort])) + coord_cartesian(xlim = c(4.99, 5.01), ylim = c(0.99, 1.01))
...
plot(lm_lin_log_lin_l[[5]])
plot()

dev.off()
par(mfrow = c(2, 2))
plot(lm_lin_log_lin_l[[2]])
plot(lm_lin_log_lin_l$belarus, 2)
plot(lm_lin_log_lin_l$bosnia_herzegovina, 2)
plot(lm_lin_log_lin_l$chongqing, 2)
...
gg_fiddle <- ggplot(data = filter(dat_l, site == "shaanxi"), 
       mapping = aes(x = time_1, y = proportion,
                     group = year_abn, color = year_abn)) + 
  geom_point() + scale_color_distiller(palette = "Greens") + 
  # scale_x_continuous(n.breaks = 30) + 
...
# combo
png(filename = paste0(p_output, "plots/fiddling0.png"), 
    width = 10.5, height = 6, units = "in", res = 400)
print(plot_grid(gg_fiddle + 
                  theme(legend.position = "none"),
                gg_fiddle + 
                  coord_cartesian(xlim = c(0.95, 1.2), ylim = c(0.975, 1.025)) + 
                  labs(
                    title = "",
                    subtitle = "zoomed in") + 
                  theme(legend.position = "none",
                        rect = element_rect(fill = "transparent")),
                cowplot::get_legend(gg_fiddle),
