...
- Cropland abandonment is largely fleeting, limiting its potential environmental benefits
- Cropland abandonment is largely fleeting, requiring new approaches to harness potential environmental benefits
- Cropland abandonment is unlikely to result in major environmental benefits without new approaches
- Cropland abandonment is largely fleeting, limiting its potential environmental benefits absent new approaches


# Abstract

Agricultural expansion is a major cause of land-use change globally, yet millions of hectares of cropland are also being abandoned due to demographic, economic, and environmental changes.
...
Here, we quantify the duration and persistence of cropland abandonment at eleven sites on four continents from 1987 and 2017.
...
However, those environmental benefits largely depend on how long ecosystems are allowed to regenerate and to accumulate carbon and biodiversity following abandonment.
Existing studies have often relied on single snapshots in time to estimate abandonment, and ignore recultivation.
Using 30-m annual land-cover maps, we tracked abandonment and recultivation at eleven sites across four continents.
Contrary to previous assumptions, abandonment was short-lived, with most abandoned croplands being recultivated within three decades.
...
knitr::include_graphics(path = paste0(p_plots, "fig2.pdf"))
```
...
`r decay_stats$t0_min` and `r decay_stats$t0_max` years at `r decay_stats$t0_min_site` and `r decay_stats$t0_max_site` respectively.
...
(ref:caption-mean-abn-duration-w-max) [UPDATE]Mean length of time abandoned (years) throughout our time series across study sites. Because abandonment and recultivation can occur multiple times during our time series, we show both the mean abandonment duration calculated across all periods of abandonment (in red) and the maximum length of abandonment (in blue).
