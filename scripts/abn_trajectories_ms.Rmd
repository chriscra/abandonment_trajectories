...
As such, information on the long-term persistence and outcomes of abandoned lands is critical to understand the potential for these areas to help mitigate the ongoing climate and biodiversity crises, and design appropriate policy responses.
...
Annual land-cover time series are becoming more common, but are typically limited to a short timespan (e.g. 2-12 years [@Alcantara2013; @Estel2015; @Lark2015]; but see @Dara2018).
These approaches all lack the spatial and temporal detail needed to understand long-term outcomes for individual croplands, and by masking dynamic patterns of abandonment and recultivation, may result in significant overestimates of abandonment [@Yin2020].
...
We found substantial amounts of cropland abandonment, observing `r area_abn_stats$total_area_abn_Mha_all_sites` million ha (Mha) of croplands abandoned across our eleven sites as of 2017 (`r area_abn_stats$total_area_abn_as_percent_of_total`% of the `r area_abn_stats$total_area_Mha_all_sites` Mha total site area; Figure \@ref(fig:site-locations).
Cropland abandonment at individual sites ranged between `r area_abn_stats$min_area_as_prop_site`% (`r area_abn_stats$min_area_as_prop_site_site`) and `r area_abn_stats$max_area_as_prop_site`% (`r area_abn_stats$max_area_as_prop_site_site`) of the total area of each site (Figures \@ref(fig:area-abn-by-age-class) and \@ref(fig:area-abn-panel)^[Note: figure numbers will be updated to read "Fig. S1," etc. in the final version.]).
The notable exception was Mato Grosso, where only `r area_abn_stats$mg_abn_as_percent`% of the total site area (`r area_abn_stats$mg` ha) was abandoned as of 2017 (see Section \@ref(mato-grosso)).
...
The time it takes to plateau depends on the time it takes for complete turnover, ranging between `r extrapolation_stats$plateau_min_time`-`r extrapolation_stats$plateau_max_time` years for all sites except Mato Grosso (`r decay_stats$mg_0` years) and Shaanxi (`r decay_stats$shaanxi_0` years).

Furthermore, only a relatively small proportion of abandonment croplands persist beyond 30 years (less than `r extrapolation_stats$prop_over_30_upper_range`% at most sites; Figure \@ref(fig:extrapolation-area-by-age)).
...
However, we also uncover significant levels of recultivation, observing that on average, 37% of abandoned croplands were recultivated by 2017, resulting in a mean abandonment duration of around 15 years.
...
Our modeled decay rates portray a more dynamic process, where abandonment is rarely an endpoint but rather part of a cycle of turnover on decadal time scales [@Munroe2013].
...
Similarly, Bosnia & Herzegovina, which had the longest mean abandonment duration, experienced accelerating recultivation towards the end of the time series.

There is little information on variation in agricultural fallow periods around the world [@Chazdon2016a], but the abandonment and impermanence that we observe is in line with that observed in other studies.
To our knowledge, the only other study [@Dara2018] using an annual time series to investigate agricultural abandonment (in a grassland region of northern Kazakhstan between 1991-2017), observed abandonment of about 40% of cultivated areas, and the subsequent recultivation of about 20% of those abandoned lands.
...
Secondary forests were recleared more quickly in the Brazilian Amazon (50% within 5 to 8 years), where 80% of secondary forests were younger than 20 years old, and around 40% were younger than 5 years old as of 2017 [@Chazdon2016a; @Nunes2020; @Smith2020].
...
Our results show that, contrary to optimistic assumptions, abandonment is unlikely to be a permanent phenomenon.
Furthermore, estimates of abandonment based on short time series or two points in time (e.g. the 83 Mha of abandoned cropland estimated by N\ae ss et al. [@Naess2021]) likely overestimate the amount of durable abandonment on the order of 20-45% or more. 
...
Yet, evidence suggests that natural regeneration can be a viable ecosystem restoration strategy under the right conditions (e.g. following less intense disturbances, in landscapes containing intact source ecosystems), resulting in secondary growth that can approach the habitat and carbon values of reference intact ecosystems [@Crouzeilles2019; @Gilroy2014; @Arroyo-Rodriguez2017].

However, even under the right conditions, recovery requires time.
Some aspects of ecosystems recover more quickly than others (along with the species associated with those elements), but in most cases, it takes multiple decades to recover species richness to values close to those in reference systems. 
...
Similarly, @Isbell2019 showed that despite relatively quick gains in biodiversity following abandonment (averaging over 1/3 diversity after just one year of succession), both biodiversity and productivity grow slowly over time, only reaching 73% of the diversity and 53% of the productivity levels of a reference ecosystem after 91 years.
In Eurasian grasslands, where widespread abandonment has occurred, plant communities had not fully recovered richness or community composition to reference levels after 24 years [@Kampf2016], nor had bird species richness and diversity after 18 years [@Kamp2011].
...
Based on estimates of potential carbon accumulation rates during the first 30 years of natural regeneration across biomes developed by @Cook-Patton2020, we would expect the abandoned croplands we observe to accumulate between 30-70 Mg C per ha over 20 years, or 18%-45% of the 110-250 Mg C per ha total that could be accumulated over 100 years [@Cook-Patton2020].
...
Given that much of the abandonment we observe occurred in grassy biomes, it is important to note the potential for soil carbon sequestration.
...
Soil degradation, a lack of nearby source populations, a lack of natural disturbance regimes, and climate change all pose significant challenges.
...
This could be a function of sociopolitical upheaval in Eastern Europe and Russia in the early 1990s [@Prishchepov2012], but it could simply be a function of time: the longer a field is abandoned, the less likely it is to be recultivated [@Dara2018]. 
If the least viable lands are abandoned first, then the most recently abandoned land should therefore be most attractive for recultivation.
...
However, even when only considering abandonment longer than 10 years, we still observed between `r recult_stats$min_10`% and `r recult_stats$max_10`% recultivation at `r recult_stats$min_10_site` and `r recult_stats$max_10_site` respectively, enough to significantly curtain regeneration and any associated benefits.
...
Agricultural abandonment is the product of broad societal changes, and the loss of certain landscapes and ways of life can be painful.
Recent behavioral research on farmer decisions to recultivate abandoned croplands highlights the importance of sociopolitical factors such as corruption, political and institutional support for agriculture, and demographics for understanding and managing abandonment, alongside biophysical and environmental conditions [@Prishchepov2021a].
...
The relative durability of abandonment at our Chinese sites point to the potential influence of large-scale reforestation programs implemented over the last 20 years, despite opportunities to improve biodiversity outcomes [@Hua2016]. 
New policies could also involve supporting sustainable long-term cultivation of some lands, while protecting (“sparing”) those that had previously been part of long-term fallowing cycles.
...
We use annual land cover maps for 1987-2017 [@Yin2020], derived from publicly available Landsat satellite imagery (30 m resolution) and mapping four land cover classes: 1) cropland, 2) herbaceous vegetation (e.g. grassland), 3) woody vegetation (e.g. forests), and 4) non-vegetation (e.g. water, urban, or barren land).
Our 11 sites were mapped with high accuracy [*ask He for stat to insert here*] and provide broad coverage of different continents and ecosystems (Figure \@ref(fig:site-locations)).
...
We focus exclusively on cropland abandonment; pasture abandonment is more difficult to discern from satellite imagery and is not captured into our data.
...
We implemented five- and eight-year moving window temporal filters to smooth land cover trajectories in our time series and address land-cover changes that are temporally unlikely (see Section \@ref(methods-si)).
...
Because a pixel may be abandoned and recultivated multiple times throughout the time series, we calculate the mean abandonment duration in two ways: 1) across all distinct periods of abandonment, and 2) across only the longest period of abandonment experienced by each pixel (Figure \@ref(fig:mean-abn-duration)).

## Modeling abandonment decay

Because some abandonment periods are limited by the end of the time series, we next model the recultivation of abandoned croplands as a function of time since initial abandonment.
We track recultivation ("decay") by calculating the proportion of each cohort of pixels abandoned in a given year that remain abandoned in each year following abandonment.
...
Importantly, we also include *site* and *cohort* level fixed effects, fitting unique coefficients for each cohort at each site.
...
We then ran a linear model on this value for each cohort to uncover temporal changes in recultivation patterns at each site (see Section \@ref(section-methods-rate-of-change)).
Trends were considered statistically significant when the 95% confidence interval for model coefficients (slopes) did not include zero (Figure \@ref(fig:decay-rate-of-change)).
...
We considered a range of alternatives to these two assumptions, as detailed in Section \@ref(section-extrapolation-si) (*TBD. tentative expectations*), but found these general patterns to hold, if anything reducing the extrapolated abandonment duration even further.

# Data Availability Statement

Code to replicate these analyses is available on GitHub at [https://github.com/chriscra/abandonment_trajectories](https://github.com/chriscra/abandonment_trajectories), and is archived at [Zenodo (TBD)](https://doi.org/). *Question for He: do you intend to make these data public?*
