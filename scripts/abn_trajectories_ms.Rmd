...
area_summary_df$total_area_abn_remaining_2017 %>% sum()

# area_summary_df %>% filter(site != "mato_grosso") %>% slice_max(area_abn_ha_2017) %>% select(area_abn_ha_2017) %>% 
#   # as.numeric() %>% 
#   prettyNum(big.mark = ",")
...
We find substantial amounts of cropland abandonment, observing a total of `r area_abn_stats$total_area_abn_Mha_all_sites` Mha of abandoned croplands by 2017 across our eleven sites (`r area_abn_stats$total_area_abn_as_percent`% of the `r area_abn_stats$total_area_Mha_all_sites` Mha total land area across sites).
Cropland abandonment at individual sites ranged between `r area_abn_stats$min` ha (`r area_abn_stats$min_site`) and `r area_abn_stats$max` (`r area_abn_stats$max_site`) ha as of 2017 (Figure \@ref(fig:area-abn-by-age-class)), and because our sites varied in size, this corresponded to between `r area_abn_stats$min_area_as_prop_site`% (`r area_abn_stats$min_area_as_prop_site_site`) and `r area_abn_stats$max_area_as_prop_site`% (`r area_abn_stats$max_area_as_prop_site_site`) of the total area of each site (Figure \@ref(fig:area-abn-panel)).
...
The site locations, along with the age of abandoned croplands in years, as of 2017, is shown in \@ref(fig:site-locations).
...
However, we also find that a significant area of these abandoned croplands were recultivated by the end of our time series (on average, `r recult_stats$mean_thr_5`% of abandoned area across all sites; Figure \@ref(fig:recult-by-threshold)).
As a result of this recultivation, the mean duration of abandonment across all sites was short: `r abn_duration_stats$mean_duration` years (SD = `r abn_duration_stats$sd_duration`), with a minimum site mean duration of `r abn_duration_stats$duration_min` years (`r abn_duration_stats$duration_min_site`) and a maximum of `r abn_duration_stats$duration_max` years (`r abn_duration_stats$duration_max_site`; see Figure \@ref(fig:mean-abn-duration)).
The age of abandoned croplands varied within sites, with individual site standard deviations ranging between `r abn_duration_stats$sd_min` years (`r abn_duration_stats$sd_min_site`) and `r abn_duration_stats$sd_max` years (`r abn_duration_stats$sd_max_site`).
...
(ref:caption-area-abn-by-age-class) Area abandoned at each site, shown according to the age of each abandoned field (in years). 
...
## Abandonment persistence, decay, & recultivation
-	mean decay rates by site across all cohorts (Fig. ). 
-	time to 50% recultivation, 100% recultivation, min and max by site.
-	rate of changes through time.

We tracked groups ("cohorts") of cropland that were abandoned in the same year through time, modeling the proportion remaining abandoned (i.e. recultivated) as a function of time elapsed since initial abandonment (Figure \@ref(fig:abn-decay-s)).
...
To investigate how the speed at which abandoned croplands are recultivated, we calculated the time required for a cohort of abandoned cropland to decline by half.
A simple linear regression showed that most sites did not have a slope significantly different from zero (Figure \@ref(fig:decay-rate-of-change)).
However, most sites showed a negative trend, indicating that abandoned croplands were being recultivated more quickly over the course of the time series (especially Bosnia & Herzegovina).
...
We selected 11 sites from @Yin2020 that were mapped with high accuracy [*ask He for stat to insert here*] and provided broad coverage of different continents and ecosystems (see Figure \@ref(fig:site-locations)).
...
Due to a linear relationship between model residuals and time when including only one term for *time*, we also tested models containing multiple *time* predictor terms, including log and linear terms.
...
```{r abn-decay-s, fig.cap = '(ref:caption-abn-decay-s)', include = FALSE, echo=FALSE}
...
We thank the following researchers at the University of Wisconsin, whose land cover maps made this analysis possible: Amintas Brandão, Johanna Buchner, David Helmers, Benjamin G. Iuliano, Niwaeli E. Kimambo, Katarzyna E. Lewińska, Elena Razenkova, Afag Rizayeva, Natalia Rogova, Seth A. Spawn, & Yanhua Xie.
We are also deeply indebted to Alex Wiebe, whose statistical advice informed and significantly improved our abandonment decay models.
We also thank members of The Drongos research group for advice and companionship.
This work was supported by Princeton University's Research Computing high performance cluster resources (*Insert text here from PRC website*), and by the High Meadows Foundation.
...
Methods text that cannot fit in the main text.
...
```{r recult-by-threshold, fig.cap = '(ref:caption-abn-thresholds-recultivation)'}
...
Can also include maps for each site of both a) abandonment *age in 2017* (e.g. Figure \@ref(fig:map-abn-age-2017-s)) and b) *max age* for all sites (e.g. Figure \@ref(fig:plot-max-age)).
...
(ref:caption-decay-rate-of-change) The rate of change of decay rates through time at each site. The four quadrants on the left show the time required for a cohort of abandoned land to decline by half, in terms of years per year. Solid lines are linear regressions, the slopes of which are shown at right.
