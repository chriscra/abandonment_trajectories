...
(ref:caption-mean-abn-duration) Mean^[Note: The SI currently includes histograms for the full distribution of duration values at each site. However, if it would be helpful, we could modify this figure to show thin violin plots behind the mean values, to represent the distributions here in the main text. I’m currently keeping it simple.] length of time abandoned (years) throughout our time series across study sites. The mean abandonment duration is calculated across all periods of abandonment (a single pixel may go through multiple periods of abandonment; in red) and across the longest period of abandonment for each pixel throughout the time series (maximum length; in blue).
...
In order to exclude short-term fallow periods, we define croplands as abandoned when they remain uncultivated for five or more consecutive years (following FAO [@FAO2016]; we explore the impact of changing this threshold in SI Section \@ref(abn-thresholds)).
...
We classified a pixel as "abandoned" when it transitioned from cropland to either herbaceous or woody vegetation (collectively referred to as "non-cropland"), and subsequently remained classified as non-cropland for five or more consecutive years (following FAO [@FAO2016]). 
...
(ref:caption-area-abn-by-age-class) Cumulative area abandoned at each site through time, according to age class (in years).^[Previously included in main text, and can be moved back if there is room.]
