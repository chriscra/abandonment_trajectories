...
Previous title: Impermanence of cropland abandonment limits potential for environmental benefits

Alternative titles:

1. Cropland abandonment^[David, you had suggested a title with the phrase “Rural land abandonment,” but I’ve decided to stick with the alternative “cropland” because it is more precisely what the paper is about, rather than broader “rural land abandonment.”] is largely fleeting, requiring new approaches to harness potential environmental benefits
2. Cropland abandonment is unlikely to result in major environmental benefits without new approaches
3. Cropland abandonment is largely fleeting, limiting its potential environmental benefits absent new approaches

# Abstract (currently 195 / 250 words)
...
# Significance Statement (currently 126 / 120 words)
...
Coupled with environmental degradation and changing agricultural technologies, urbanization and rural outmigration have contributed to decreasing economic viability of some agricultural lands and a growing global trend of cropland abandonment [@ReyBenayas2007; @Prishchepov2021].
Given increasing competition for land, these abandoned croplands have been targeted for diverse goals such as biofuel production [@Naess2021], carbon sequestration [@Griscom2017; @Chazdon2016a], or simply recultivation [@Meyfroidt2016].
To conservationists, these abandoned croplands represent a potentially large opportunity for biodiversity recovery as native vegetation regenerates [@Chazdon2020; @Xie2019; @Navarro2012].^[I had originally included this as a way to orient readers to our emphasis on the biodiversity conservation perspective. David, you recommended we condense it into the previous sentence - was that to save length, or because you think it better to be more neutral at this stage? I don't feel strongly, just curious.]
...
When derived from satellite imagery, abandonment is most frequently estimated by simply taking the difference in cultivated area between land cover maps for two time points (e.g., 1992 and 2015 [@Naess2021]).
Annual land-cover time series are becoming more common, but are typically limited to a few years (e.g., 2-12 years [@Alcantara2013; @Estel2015; @Lark2015]), coarse in resolution (e.g., $\geq250$-m [@Naess2021; @Estel2015]), and most analyses are restricted to a single region [@Dara2018].
...
We detected `r area_abn_stats$total_area_ever_abn_Mha_all_sites` million ha (Mha)^[Note that I have changed the order in which these stats are shared... let me know what you think of how it reads now.] of croplands that were abandoned at least once between 1987 and 2017 across our eleven sites.
...
Accordingly, the mean duration of abandonment across all sites was short: `r abn_duration_stats$mean_of_means_duration` years (SD = `r abn_duration_stats$sd_of_mean_duration`), ranging from `r abn_duration_stats$duration_min` years (`r abn_duration_stats$duration_min_site`, Russia/Uralsk, Kazakhstan) to `r abn_duration_stats$duration_max` years (`r abn_duration_stats$duration_max_site`; Figure \@ref(fig:mean-abn-duration))^[I made a few changes here: updated axis labels, removed the blue dots and dashed lines, reduced site names in size and switched them to the right side. I was unable to easily add a scale to the y-axis for each site, but the distance between each site is about 0-0.13 in terms of proportion. Readers should be able to interpret without a scale.].
...
Recultivation occurred quickly: we predict that >50% of abandoned croplands will be recultivated within 25 years^[Clarified based on DW comment about it being unclear what the 25 years represented] of initial abandonment at almost all sites (Figure \@ref(fig:abn-decay-s).
...
For comparison, complete turnover took `r decay_stats$t0_min` and `r decay_stats$t0_max` years respectively in `r decay_stats$t0_min_site`/Uralsk and `r decay_stats$t0_max_site`.^[Added in response to DW suggestion to include these stats for symmetry's sake.]
The mean decay trajectories also display wide variation in the percentage of abandoned croplands that persist beyond 20 years: as few as `r decay_stats$percent_at_20_min`% in `r decay_stats$percent_at_20_min_site`/Uralsk, up to `r decay_stats$percent_at_20_max`% in `r decay_stats$percent_at_20_max_site`/Shanxi.
...
## Projecting^[updated to "projection" based on VR recommendation] future abandonment and recultivation
...
Even in Shaanxi/Shanxi, the site we project will have the most durable abandonment, only `r extrapolation_stats$prop_over_50_shaanxi`% of abandoned land will remain so for longer than 50 years.
...
For example, while Shaanxi/Shanxi had one of the shortest mean abandonment durations (because most abandonment took place towards the end of the time series)^[Attempting to clarify based on DW comment that the juxtaposition of the short mean duration and longest half-life and turnover time], current recultivation trajectories indicate more persistent abandonment than we can observe during the time series.
...
Furthermore^[This paragraph was moved up from the start of the next section at the suggestion of VR & HY], estimates of abandonment based on short time series or two points in time (e.g., 83 Mha estimated by Næss et al. [@Naess2021]) likely overestimate the amount of persistent abandonment, due to both a failure to exclude short-term fallow periods, and, as our analysis shows, high rates of recultivation.
...
Moreover, the two-year assessments identify different areas of abandonment from derived from our full annual time series, with spatial agreement ranging from `r abn_2yr_stats$jaccard_low`% (Mato Grosso) to `r abn_2yr_stats$jaccard_high`% (`r abn_2yr_stats$jaccard_high_site`) (see Table \@ref(tab:twoyr-diff-table), Section \@ref(twoyr-vs-annual)).
...
Accordingly^[I'm not sure I like how this and the next paragraph flow. Any suggestions would be appreciated!], the recultivation that we observe may dramatically limit the scope for abandoned croplands to play a major role in carbon sequestration or the recovery of biodiversity.
...
Natural regeneration can be a viable restoration strategy under the right conditions, especially after low-intensity disturbances (e.g., selective logging) and when close in proximity to relatively undisturbed or mature ecosystems.
In such conditions, plant and animal dispersal can result in secondary growth that can approach the habitat and carbon values of undisturbed ecosystems [@Crouzeilles2016; @Gilroy2014; @Arroyo-Rodriguez2017].

However, even under optimal conditions, recovery requires time, typically multiple decades, in order to recover species richness to values close to those in reference systems [@Meli2017; @Acevedo-Charry2019; @Isbell2019; @Nerlekar2020; @Chazdon2020].
Rarer, forest-adapted, and old-growth dependent species return even more slowly, and the recovery towards old-growth ecosystems in terms of community composition, species similarity, and vegetation structure can take much longer than just the recovery of total species richness or abundance (which can be dominated by widespread generalist species [@Meli2017; @Arroyo-Rodriguez2017; @Crouzeilles2016; @Rydgren2020]).

Tropical ecosystems are among the most speciose in the world.
In some cases they can regenerate quickly, but never on a time scale as short as the abandonment durations we observe here.
...
Recovery can be rapid for some animal groups in specific cases (e.g., birds and dung beetles in the Colombian Andes, [@Gilroy2014]), but typically takes a long time for most types of vertebrate species.
Across tropical forests, amphibian, bird, mammal, and reptile species richness largely recovers within 40 years, but species compositional similarity for these vertebrate groups takes much longer to recover (if at all), particularly for late-successional species, insectivorous birds^[The paper I cite does include data for insectivorous mammals as well as birds, but this particular point is specific to the birds, so I’ve clarified], and forest specialists [@Acevedo-Charry2019].
No vertebrate groups reach species compositional similarity^[Clarified what is meant by "similarity"] to reference old-growth forests, even in the oldest secondary forests (30-65 years; [@Acevedo-Charry2019]).
...
On average, secondary grasslands distributed around the world that are between 1-251 years old contain only 53-76% of the plant species richness of undisturbed grasslands (sometimes called ancient or "old-growth" grasslands^[This clause is not strictly necessary, but I include it to be extra clear. Some grassland ecologists have recently argued for a concept of "old-growth grasslands", as in the citation here.] [@Nerlekar2020]).
...
Eurasian grasslands, where widespread abandonment has occurred, had not seemed fully recovered with respect to either plant species richness or community composition after 24 years of observation [@Kampf2016], nor with respect to bird species richness and diversity after 18 years of observation [@Kamp2011].
...
Grassland biomes,^[Separated this paragraph out and added a bit more detail and a citation to try to give grassland biomes as much "respect" as forested biomes.] where much of the abandonment we observed occurred, can also harbor substantial amounts of carbon sequestered in soils (@Lal2004).
However, soil carbon may accumulate even more slowly than aboveground forest biomass, taking a century or longer to return to reference levels [@Yang2019].
...
Without such disturbances, grassland biodiversity may be lost if low-intensity farmlands are abandoned, particularly in historically unforested ecosystems [@ReyBenayas2007; @Nerlekar2020; @Queiroz2014; @Sirami2008].
...
Our abandonment definition implies a minimum abandonment duration of five years.^[Though redundant, I’m leaving this in to clarify that the duration starts right when cropland -> non-cropland (not after the five year mark).]
...
Code to replicate these analyses is available on GitHub at <https://github.com/chriscra/abandonment_trajectories>, and is archived at [Zenodo (TBD)](https://doi.org/).^[I (Chris) plan to make the raw abandonment maps available, whether in my GitHub repository, or in an alternative location.]
...
A version of this manuscript that includes the Supporting Information [can be found here](https://drive.google.com/file/d/1yY7qbChP3gna0xjn5KZyiRKR-OAmpLKN/view?usp=sharing).
