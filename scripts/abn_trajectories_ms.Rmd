...
Previous analyses explored the age and persistence of secondary forests in the Neotropics by identifying regeneration through the detection of woody regrowth in satellite imagery, finding that forest regeneration is often short-lived [@Reid2018; @Nunes2020; @Smith2020; @Schwartz2020].
Half of secondary forests were recleared in as little as 5 to 8 years in the Brazilian Amazon [@Aguiar2016; @Chazdon2016a; @Nunes2020], and 20 years or less in southern Costa Rica [@Reid2018].
...
We found substantial amounts of cropland abandonment, observing `r area_abn_stats$total_area_abn_Mha_all_sites` million ha (Mha) of croplands abandoned across our eleven sites as of 2017 (`r area_abn_stats$total_area_abn_as_percent`% of the `r area_abn_stats$total_area_Mha_all_sites` Mha total site area; Figure \@ref(fig:site-locations).
...
However, we also found that many of these abandoned croplands were recultivated during our time series: `r recult_stats$mean_thr_5`% of abandoned area, averaged across all sites (Figure \@ref(fig:recult-by-threshold)).
...
As a result, estimates of abandonment based on short time series or two points in time (e.g. the 83 Mha of abandoned cropland estimated by @Naess2021) likely significantly overestimate the amount of durable abandonment, on the order of 20-45% or more. 
...
<!-- Formerly from the results, possibly for right before "Additional challenges to regeneration" -->
Abandoned cropland followed different successional trajectories at each site, where the proportion of abandoned cropland distributed between forest and grassland (as of 2017) ranged from a minimum of `r lc_stats$forest_min`% in forest (`r lc_stats$forest_min_grassland`% in grassland) at `r lc_stats$forest_min_site` to a maximum of `r lc_stats$forest_max`% in forest (`r lc_stats$forest_max_grassland`% in grassland) at `r lc_stats$forest_max_site` (see Figure \@ref(fig:abn-prop-lc)). 
Changes in land cover over time are shown for each site in Figure \@ref(fig:area-by-lc).
...
Second, and more consequentially, we recognize that while we use a five year abandonment definition (following FAO [@FAO2016]), the length of fallow periods may vary widely around the world, and many studies use different definitions of abandonment. 
...
Differentiating "true" abandonment from short-term fallowing or crop rotations, is difficult because agricultural practices can vary widely by region, and studies use many different definitions [@Yin2018].
...
(*TBD, Tentative expectation:*) We considered a range of alternatives to these two assumptions, as detailed in Section \@ref(section-extrapolation-si) (*TBD*), but found these general patterns to hold, if anything reducing the extrapolated abandonment duration even further.
