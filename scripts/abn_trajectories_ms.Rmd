...
We selected 11 sites from @Yin2020 that were mapped with high accuracy [*insert stat here from He*] and which provided broad geographic coverage (see Figure \@ref(fig:site-locations)).
...
```{r abn-panel-s, fig.cap = '(ref:caption-abn-age-class)', echo=FALSE}
...
```{r abn-age-class, fig.cap = '(ref:caption-abn-age-class)', include = FALSE, echo=FALSE}
# (ref:caption-mean-abn-length) Sites included in this study, from @Yin2020.
# (ref:caption-mean-abn-length)
# cite inline using the following  \@ref(fig:mean-abn-length)

knitr::include_graphics(path = paste0(p_plots, run_label, "/abn_area_by_class_bins", run_label, "_s.png"))
```
...
knitr::include_graphics(path = paste0(p_plots, "persistence_proportion_b1_s.png"))
```
