---
title: "Impermanence of cropland abandonment limits potential for environmental benefits (draft)"
...
abn_duration_stats$mean_of_sds_duration
...
While sometimes seen as a threat to food security and cultural landscapes, abandonment is increasing viewed as an opportunity for carbon sequestration and habitat restoration.
However, those environmental benefits largely depend on how long ecosystems have to regenerate and accumulate carbon and biodiversity following abandonment.
...
Contrary to previous assumptions, abandonment was short-lived and most abandoned croplands were recultivated within three decades.
Unless policymakers take steps to reduce recultivation and provide incentives for regeneration, abandonment will remain a missed opportunity.
...
<!-- Agriculture, cropland abandonment, biodiversity conservation, secondary succession, carbon sequestration, land cover mapping -->
...
Here, we utilize these new annual time-series of cropland abandonment [@Yin2020] to investigate the duration and persistence of abandonment at eleven sites across four continents, including sites in Brazil, the United States, Eastern Europe, Russia, the Middle East, and China (Figure \@ref(fig:site-locations)).
...
area_summary_df %>% 
  select(site, area_abn_ha_2017, area_ever_abn_ha, cropland_area_1987, total_crop_extent_ha_r, total_site_area_ha_2017, 
         area_as_prop_site, area_as_prop_total_crop, area_as_prop_crop87) %>% 
  arrange(area_as_prop_total_crop)

area_summary_df %>% 
  select(site, area_abn_ha_2017, area_ever_abn_ha, cropland_area_1987, total_crop_extent_ha_r, area_as_prop_total_crop, area_as_prop_crop87) %>% 
  arrange(area_as_prop_total_crop) %>% 
  mutate(prop_total_to_87 = total_crop_extent_ha_r/cropland_area_1987)


area_summary_df %>% select(site, area_abn_ha_2017, total_site_area_ha_2017, area_as_prop_site) %>% arrange(area_as_prop_site)

names(area_abn_stats)
# ---------------------------------------------------------------------- #
...
area_abn_stats <- list(
  ...
  "total_crop_extent_area_Mha_2017_all_sites" = round(area_summary_df$total_crop_extent_ha_r %>% sum() / 10^6 , digits = 2),
  ...
  "total_area_abn_2017_as_percent_of_total_crop_extent" = round(
    (sum(area_summary_df$area_abn_ha_2017) / sum(area_summary_df$total_crop_extent_ha_r)), 
    digits = 4) * 100,
...
decay_stats <- list(
  ...
  "shaanxi_0.5" = time_to_l3 %>% filter(p == 0.5, site == "shaanxi") %>% select(time_to_value) %>% round(., digits = 0),
  "shaanxi_0" = time_to_l3 %>% filter(p == 0, site == "shaanxi") %>% select(time_to_value) %>% round(., digits = 0),
  
  "mg_0.5" = time_to_l3 %>% filter(p == 0.5, site == "mato_grosso") %>% select(time_to_value) %>% round(., digits = 0),
  "mg_0" = time_to_l3 %>% filter(p == 0, site == "mato_grosso") %>% select(time_to_value) %>% round(., digits = 0),
...
(ref:caption-site-locations) Observed duration of cropland abandonment (in years) as of 2017 in our eleven study sites.
...
We detected `r area_abn_stats$total_area_abn_Mha_2017_all_sites` million ha (Mha) of abandoned croplands across our eleven sites as of 2017 (Figure \@ref(fig:site-locations)).^[*As a reminder, this is the total abandonment as of 2017. I report the area that was abandoned at least once during the time series below, but please let me know if you think I should change the order in which the numbers are reported. I could also update the percentages to reflect the "ever abandoned" stat.*]
...
knitr::include_graphics(path = paste0(p_plots, run_label, "/mean_lengths_w_ridges_hist", run_label, ".pdf"))
```
...
These mean decay trajectories also display wide variation in the proportion of abandoned croplands that persist beyond 20 years: as few as `r decay_stats$percent_at_20_min`% in `r decay_stats$percent_at_20_min_site`, up to `r decay_stats$percent_at_20_max`% in `r decay_stats$percent_at_20_max_site`/Shanxi.
...
Using a new high-resolution annual land-cover time series, we uncovered significant amounts of abandonment since 1987, constituting `r area_abn_stats$total_area_abn_as_percent_of_total`% of the total land area, and `r area_abn_stats$total_area_abn_as_percent_of_crop87`% of cropland in 1987, across our eleven sites.
However, we also uncover significant levels of recultivation, observing that on average, 37% of abandoned croplands were recultivated by 2017, lasting 15 years on average.
We observed a wide range of variation across sites, with some sites showing accelerating rates of recultivation.
Yet, even the sites with the most durable abandonment are unlikely to see large amounts of land persist uncultivated for longer than 50 years.
...
Furthermore, by using an annual time series and tracking cohorts of abandoned croplands based on the year of abandonment, we developed a more accurate picture of abandonment persistence.
For example, while Shaanxi had among the shortest observed abandonment durations, current recultivation trajectories indicate more durable abandonment than we can observe during the time series, given an increase in the rate of abandonment coupled with a decrease in recultivation rates later in the time series.
Similarly, Bosnia & Herzegovina, which had the longest mean abandonment duration, experienced accelerating recultivation towards the end of the time series, challenging future persistence.

The abandonment and impermanence that we observe generally matches that observed in other studies, although such studies are relatively rare [@Chazdon2016a].
To our knowledge, the only other study that uses an annual time series to investigate agricultural abandonment (in a grassland region of northern Kazakhstan between 1991-2017; [@Dara2018]), observed abandonment of about 40% of cultivated areas, and the subsequent recultivation of about 20% of that abandonment.
This recultivation, while in line with our most durable site (Shaanxi), was lower than both our average across sites (`r recult_stats$mean_thr_5`%), and that seen at our sites closest to Kazakhstan:  Orenburg and Volgograd, Russia, which saw recultivation of `r area_recult_threshold %>% filter(abn_threshold == 5, site == "orenburg") %>% .$proportion_recultivated %>% round(digits = 4) * 100`% and `r area_recult_threshold %>% filter(abn_threshold == 5, site == "volgograd") %>% .$proportion_recultivated %>% round(digits = 4) * 100`% respectively (Figure \@ref(fig:recult-by-threshold)).

Our projected half-lives of 12-24 years at most sites were similar to that observed by Reid et al. (20 years @Reid2019), but were slightly longer than those observed elsewhere in the Neotropics [@Chazdon2016a; @Nunes2020; @Smith2020].
Secondary forests were recleared more quickly in the Brazilian Amazon (50% within 5 to 8 years), where 80% of secondary forests were younger than 20 years old [@Chazdon2016a; @Nunes2020; @Smith2020].
Across the tropics, only 33% of forests regenerating after recent deforestation were older than 10 years [@Vancutsem2021].
However, these differences may be the result of 1) a time delay between abandonment and the detection of woody vegetation regrowth, 2) our exclusion of abandonment less than five years, or 3) our method of modeling recultivation for each cohort individually, thereby reducing the influence of the time series length.

## Implications for biodiversity recovery and carbon sequestration

Our results show that, contrary to optimistic assumptions, most cropland abandonment is unlikely to be permanent.
Furthermore, estimates of abandonment based on short time series or two points in time (e.g. 83 Mha estimated by Næss et al. [@Naess2021]) likely overestimate the amount of durable abandonment by roughly 20-45% or more. 
Recultivation at this scale may dramatically limit the scope for abandoned croplands to play a major role in carbon sequestration or the regeneration of biodiversity.
...
Yet, evidence suggests that natural regeneration can be a viable ecosystem restoration strategy under the right conditions (e.g. following less intense disturbances, in landscapes containing intact source ecosystems), resulting in secondary growth that can approach the habitat and carbon values of intact ecosystems [@Crouzeilles2019; @Gilroy2014; @Arroyo-Rodriguez2017].

However, even under the right conditions, recovery requires time.
Some aspects of ecosystems recover more quickly than others, but in most cases, it takes multiple decades to recover species richness to values close to those in reference systems. 
Rarer, forest-adapted, and old-growth dependent species return even more slowly, and metrics that better capture the recovery towards old-growth ecosystems, such as community composition, species similarity, and vegetation structure, can take much longer to recover than simply total species richness or abundance, which can be dominated by widespread generalist species [@Meli2017; @Arroyo-Rodriguez2017; @Crouzeilles2016; @Rydgren2020].

Tropical ecosystems are among the most speciose in the world.
In some cases they can regenerate quite quickly, though not on a time scale as short as the abandonment we observe here.
...
These sites retained relatively high forest cover (76% on average), however, and recovery is likely to be even slower in less-forested ecosystems [@Rozendaal2019].

Though some studies indicate quick recovery of vertebrate groups [@Gilroy2014], most point to longer recovery times. 
A recent meta-analysis of tropical forests found that while amphibian, bird, mammal, and reptile species richness largely recovered within 40 years, species compositional similarity took much longer to recover (if at all), particularly for late-successional species, insectivores, and forest specialists [@Acevedo-Charry2019].
None of the vertebrate groups reached similarity to reference old-growth forests, even in the oldest secondary forests (30-65 years).

Older evidence indicates that grassland ecosystems can recover more quickly following disturbances than forests [@Jones2009], but recent studies have highlighted the long recovery times for secondary grasslands. 
Nerlekar & Veldman [@Nerlekar2020] show that on average, secondary grasslands (between 1-251 years old) contained only 53-76% of the plant species richness of old-growth grasslands; even after full recovery of species richness (with minimum estimated recovery times of >100 years), compositional similarity to old-growth grasslands remained low (43%).
Similarly, Isbell et al. [@Isbell2019] showed that after quick initial gains in biodiversity following abandonment (averaging over 1/3 diversity after just one year), both biodiversity and productivity grew slowly over time, reaching only 73% of the diversity and 53% of the productivity levels of a reference ecosystem after 91 years.
Eurasian grasslands, where widespread abandonment has occurred, had not fully recovered plant species richness or community composition after 24 years [@Kampf2016], nor bird species richness and diversity after 18 years [@Kamp2011].
...
While carbon accumulates quickly over the first few decades of regeneration, it can take between 50-100 years to achieve similar levels of biomass as old-growth forests [@Poorter2016; @Martin2013]. 
A study of Neotropical aboveground biomass recovery showed that forests were able to recover 90% of the biomass of old-growth forests after a median of 66 years, yet were typically at <50% biomass after 20 years.
But, like biodiversity, carbon accumulation estimates vary by biome and by prior land use. 
Based on estimates of potential aboveground carbon accumulation rates during the first 30 years of natural regeneration across biomes [@Cook-Patton2020], we would expect the abandoned croplands we observe to accumulate between 30-70 Mg C per ha over 20 years, or 18%-45% of the 110-250 Mg C per ha total that could be accumulated over 100 years [@Cook-Patton2020].
Much of the abandonment we observe occurred in grassy biomes, where soil carbon sequestration may be more important.
However, like aboveground biomass, soil carbon may accumulate slowly, taking up to a century (or possibly longer) to return to reference levels [@Yang2019].
...
Moreover, merely abandoning croplands may be insufficient to achieve carbon or biodiversity goals for a variety of reasons, in addition to recultivation. 
The degradation and slow recovery of soils, a lack of nearby source populations, a lack of natural disturbance regimes, and climate change all pose significant challenges [@Crouzeilles2016; @Crouzeilles2019; @Parkhurst2021].
While natural regeneration can be cheaper and more successful than active restoration under the right conditions, more heavily altered systems require more active approaches and longer recovery times [@Crouzeilles2017; @Gilroy2014; @Meli2017].
Grassland regeneration faces particular challenges, because many grasslands require the maintenance of natural disturbance regimes (e.g. grazing or fire), which may be absent following abandonment [@Nerlekar2020].
In certain cases, grassland biodiversity may be lost if low-intensity farmlands are abandoned and replaced by woody vegetation, particularly in historically unforested ecosystems [@ReyBenayas2007; @Nerlekar2020; @Queiroz2014; @Sirami2008].

## Drivers of abandonment and recultivation

Our results also include a range of interesting temporal patterns of abandonment. 
For example, Bosnia & Herzegovina displays a strong bifurcated pattern, where early abandonment remained relatively durable, while croplands abandoned in later years were recultivated relatively quickly. 
This could be a function of sociopolitical upheaval in Eastern Europe and Russia in the early 1990s [@Prishchepov2012], but it could also simply be a function of time: the longer a field is abandoned, the less likely it is to be recultivated [@Dara2018]. 
If the least viable lands are abandoned first, then the most recently abandoned croplands should therefore be most attractive for recultivation.
Younger abandoned fields may also simply be easier to reclear, due to less woody regrowth (as farmers prefer [@Prishchepov2021a]). 
Our highest performing model of recultivation included two *time* terms (linear and logarithmic), indicating that time may influence recultivation through multiple mechanisms.

## Conclusions

Abandoned croplands at eleven sites across the globe show little evidence of remaining uncultivated for long periods of time. 
Without the development of policies and incentives to discourage recultivation, these areas are unlikely to provide significant biodiversity and carbon benefits. 
Our results reinforce growing calls for enhanced monitoring of regeneration [@Cook-Patton2021], and the development of a stronger policy framework for managing these lands [@Chazdon2020].
...
Agricultural abandonment is the product of broad societal changes, and the pain associated with the loss of certain types of landscapes and ways of life should be addressed.
Recent behavioral research on farmer decisions to recultivate abandoned croplands highlights the importance of addressing sociopolitical factors such as corruption, political and institutional support for agriculture, and demographics, alongside biophysical and environmental conditions, in order to understand and manage abandonment [@Prishchepov2021a].
...
This could involve incorporating abandoned fields in protected areas, incorporating natural regeneration into ecosystem service schemes to allow landowners to benefit economically, or taking steps to support sustainable long-term cultivation and reduce turnover among fields that have previously been part of long-term fallowing cycles. 
The relative durability of abandonment at our Chinese sites point to the potential influence of large-scale reforestation programs implemented over the last 25 years, despite ongoing opportunities to improve biodiversity outcomes [@Hua2016]. 
...
We use annual land cover maps for 1987-2017 [@Yin2020], derived from publicly available Landsat satellite imagery (30 m resolution), mapping four land cover classes: 1) cropland, 2) herbaceous vegetation (e.g. grassland), 3) woody vegetation (e.g. forests), and 4) non-vegetation (e.g. water, urban, or barren land).
Our eleven sites were mapped with high accuracy and provide broad coverage of different continents and ecosystems (Figure \@ref(fig:site-locations)).
We focus exclusively on cropland abandonment; pasture abandonment is more difficult to discern from satellite imagery and is not captured in our data.
Yin et al. [@Yin2020] selected sites where recent abandonment was likely due to socioeconomic, political, or environmental factors.
Thus, while our results are likely representative of those areas that have recently experienced abandonment, they likely overestimate the overall prevalence of abandonment across all agricultural regions.
...
Here, we define "abandonment" as cropland that is no longer under active cultivation and is left free of direct human influence, therefore excluding shifts to less intensive uses such as extensive grazing.
...
Recognizing that longer abandonment thresholds may be more appropriate in certain contexts, we performed a sensitivity analysis varying our definition (Section \@ref(abn-thresholds)) and found that, as expected, longer definitions resulted in less abandonment over all, longer average abandonment durations (Figure \@ref(fig:abn-thresholds-mean-duration)), and lower recultivation rates (Figure \@ref(fig:recult-by-threshold)).
However, even when only considering abandonment longer than 10 years, we still observed between `r recult_stats$min_10`% and `r recult_stats$max_10`% recultivation across our sites (at `r recult_stats$min_10_site` and `r recult_stats$max_10_site` respectively), enough to significantly curtail regeneration and any associated benefits.
...
Pixels that transitioned from cropland to the non-vegetation class were not considered "abandoned," and were excluded from our analysis.*^[These pixels represent a very small portion of our dataset, and non-vegetated land remained about constant or declined over time at most sites.]*
...
Because some abandonment periods are limited by the end of the time series, we next modeled the recultivation of abandoned croplands as a function of time since initial abandonment.
...
We then ran linear models predicting the proportion of abandoned cropland in each cohort remaining abandoned as a function of time since initial abandonment.
...
Importantly, we include *site* and *cohort* level fixed effects, fitting unique coefficients for each cohort at each site.
...
Modeled decay trajectories are shown for one example site, Shaanxi/Shanxi (China), in Figure \@ref(fig:abn-decay-s), and model coefficients for all sites are shown in Figure \@ref(fig:decay-mod-coef).

To look at changes in persistence over time, we calculate the time required for half of a given cohort to be recultivated based on the modeled recultivation trajectory of each cohort. 
We then ran a linear model on this value for each cohort to uncover temporal changes in recultivation patterns at each site (Section \@ref(section-methods-rate-of-change)).
...
(ref:caption-abn-decay-s) Decay of abandoned land at a single site, Shaanxi/Shanxi (China), showing the proportion of each cohort of abandoned land (i.e. abandoned in a given year) remaining abandoned over time. Points represent observations, dashed green lines represent model predictions for each cohort, and the solid purple line represents the mean trend across all cohorts (averaging across cohorts for each coefficient). All sites results are shown in Figure \@ref(fig:decay-model-grid) and Figures \@ref(fig:decay-model-indiv-site-b)-\@ref(fig:decay-model-indiv-site-w).
...
knitr::include_graphics(path = paste0(p_plots, run_label, "/decay/ms_fig_l3_cohort", run_label, "_s.pdf"))
```

### Extrapolation

Based on our modeled decay rates, we extrapolate abandonment and recultivation into the future, based on two simple assumptions: (1) a constant amount of cropland is newly abandoned each year, based on the mean area of cropland abandoned each year at each site (Figure \@ref(fig:turnover-grid)); (2) all abandoned croplands are recultivated at the mean predicted rate for each site (Figure \@ref(fig:decay-curves-by-site)). 
We considered a range of alternatives to these two assumptions (Section \@ref(section-extrapolation-si); *TBD*), but found these general patterns to hold, if anything reducing the extrapolated abandonment duration even further.

# Data Availability Statement

Code to replicate these analyses is available on GitHub at [https://github.com/chriscra/abandonment_trajectories](https://github.com/chriscra/abandonment_trajectories), and is archived at [Zenodo (TBD)](https://doi.org/).^[*Question for He: do you intend to make the land cover map data public?*]
...
This work was supported by the High Meadows Foundation and performed using Princeton Research Computing resources at Princeton University.
...
(ref:caption-map-abn-age-2017) Duration of cropland abandonment (in years) as of 2017 at our eleven sites.
...
For example, a site may have a relatively short mean length of abandonment (e.g. Shaanxi/Shanxi [China], with a mean abandonment length of 13 years; see Figure \@ref(fig:site-locations)), but also have a gradual decay rate, indicating that land should stay abandoned for a relatively longer amount of time. 
...
2. We could set a limit to how much total area could be abandoned, perhaps based on a percent of cropland in a site, for example. 
3. We could alternatively assume that the amount of abandoned land each year would be more likely to gradually decline, so that less and less land is abandoned each year.
4. Or, finally, we could base this on the trend in abandoned land each year (running a regression on the gain columns in part d of the main results panel figures).
...
(ref:caption-decay-model-indiv-site-c) Decay model results for Chongqing (China), showing raw data (green points), modeled abandonment trajectories for individual years (green lines), and the mean decay trajectory for the site as a whole (blue lines). The mean decay trajectory is calculated in two ways: 1) from the mean of the coefficients for each cohort (solid blue line), and 2) as a single linear regression without cohort-level fixed effects (F.E.; dashed blue line).
...
(ref:caption-decay-model-indiv-site-s) Decay model results for Shaanxi/Shanxi (China), showing raw data (green points), modeled abandonment trajectories for individual years (green lines), and the mean decay trajectory for the site as a whole (blue lines). The mean decay trajectory is calculated in two ways: 1) from the mean of the coefficients for each cohort (solid blue line), and 2) as a single linear regression without cohort-level fixed effects (F.E.; dashed blue line).
