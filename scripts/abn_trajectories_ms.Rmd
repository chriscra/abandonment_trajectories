...
(ref:caption-site-locations) Observed duration of cropland abandonment (in years) as of 2017 in our eleven study sites. [Note: I will be updating this figure based on Volker’s recommendations.]
...
```{r 2yr-diff-table, include = FALSE}
abn_2yr_overestimation %>% 
  ...
  knitr::kable(., 
             ...
             caption = "Differences in cropland abandonment identified using only two years and an annual time series."
             ) 
...
When estimating abandonment at our sites based on cultivation in *only* two points in time (1987 and 2017), we find [X] Mha of "abandonment," compared to [X] Mha of abandonment as of 2017 estimated using our full annual time series. 
Furthermore, this two-year method identifies different areas of abandonment from those identified using our full annual time series, with an overlap of only [X]% (Jaccard similarity [area of intersection / area of union] = [X]).^[*Note: the previous estimate of 20-45% overestimation was based on the recultivation rate we saw at each site. This new estimate will be based on applying the two year method to our data. TBD.*]
...
Our eleven sites were mapped with high accuracy (average overall accuracy $85\pm4$%) and provide broad coverage of different continents and ecosystems (Figure \@ref(fig:site-locations)).
...
We also considered a range of alternatives to these two assumptions, but found these general patterns to hold, if anything reducing the extrapolated abandonment duration even further (results not shown; see Section \@ref(section-extrapolation-si) -- *Analysis and explanation TBD*).

# Data Availability Statement

Code to replicate these analyses is available on GitHub at [https://github.com/chriscra/abandonment_trajectories](https://github.com/chriscra/abandonment_trajectories), and is archived at [Zenodo (TBD)](https://doi.org/).
...
knitr::include_graphics(path = paste0(p_plots, run_label, "/decay/", "extrapolation_combo_SI", run_label, ".pdf"))
...
knitr::include_graphics(path = paste0(p_plots, run_label, "/decay/", "extrapolate_age_by_age_bin", run_label, ".pdf"))
