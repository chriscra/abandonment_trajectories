...
We find that abandonment is by and large fleeting, lasting on average only `r abn_duration_stats$mean_duration` years (SD = `r abn_duration_stats$sd_duration`).^[*Note: both values reported are the means across each site's corresponding summary statistics, i.e. the mean of means and mean of standard deviations*]
...
Contrary to previous assumptions, abandonment is short-lived and most abandoned croplands are recultivated within three decades or less.
...
Furthermore, in a world with increasing competition for land, these abandoned agricultural lands are highly sought after for diverse goals such as biofuel production [@Naess2021], carbon sequestration [@Griscom2017; @Chazdon2016a], or recultivation [@Meyfroidt2016].
...
Understanding the environmental impacts of agricultural abandonment requires not only detailed information on where and when abandonment takes place, but critically, what happens to croplands and pastures after they are abandoned.
...
As such, information on the long-term persistence and outcomes of abandoned lands is critical to understanding the potential for these areas to help mitigate the ongoing climate and biodiversity crises.
...
When derived from satellite imagery, abandonment is most frequently estimated by simply taking the difference in cultivated area between land cover maps for two distinct points in time (e.g. 1992 and 2015 [@Naess2021]).
Annual land-cover time series are becoming more common, but are typically limited to a short timespan (e.g. 2-12 years [@Alcantara2013; @Estel2015; @Lark2015]; though see @Dara2018).
These approaches all lack the spatial and temporal detail needed to understand long-term outcomes for individual croplands, and by masking dynamic patterns of abandonment and recultivation, may significantly overestimate abandonment [@Yin2020].

Previous analyses explored the age and persistence of secondary forests in the Neotropics by identifying regeneration through the detection of woody regrowth in satellite imagery, finding that forest regeneration is often short-lived [@Reid2019; @Nunes2020; @Smith2020; @Schwartz2020].
Half of secondary forests were recleared in as little as 5 to 8 years in the Brazilian Amazon [@Aguiar2016; @Chazdon2016a; @Nunes2020], and 20 years or less in southern Costa Rica [@Reid2019].
...
Yin et al. -@Yin2020 used a trajectory-based approach to produce accurate maps of cropland abandonment without relying on woody revegetation, allowing for a direct focus on abandonment and an expansion to a broader set of non-forest biomes.
Here, we utilize this new annual time-series of cropland abandonment [@Yin2020] to investigate the duration and persistence of abandonment at eleven sites across four continents, including sites in Brazil, the United States, Eastern Europe, Russia, the Middle East, and China (Figure \@ref(fig:site-locations)).

We address three sets of questions: How long does abandoned cropland stay abanadoned during our time series, on average, and how does this vary geographically? How quickly is recultivation expected to take place based on the year of abandonment, and how do these individual recultivation (or "decay") rates vary through time? Finally, if abandonment and recultivation continue at current rates, how will the mean age of abandoned croplands change in these landscapes into the future?

By making use of new, more accurate and finer resolution abandonment maps, and by covering a longer period and broader set of sites than previous studies, we provide the most detailed analysis to date of the timing and persistence of cropland abandonment.
Given that some abandonment periods are inherently limited by the length of our time series (making the mean duration a moving target), tracking recultivation rates for each year provides a more accurate understanding of persistence.
Together, these results provide much needed context on the temporal nature of cropland abandonment and its potential to sequester carbon and conserve biodiversity.
...
area_abn_stats <- list(
  ...
  "total_crop_extent_area_Mha_all_sites" = round(area_summary_df$total_crop_extent_ha_r %>% sum() / 10^6 , digits = 2),
  ...
  "total_area_abn_as_percent_of_total" = round((area_summary_df$area_abn_ha_2017 %>% sum()) / (area_summary_df$total_site_area_ha_2017 %>% sum()), digits = 4) * 100,
  
  "total_area_abn_as_percent_of_crop87" = round(
    (sum(area_summary_df$area_abn_ha_2017) / sum(area_summary_df$cropland_area_1987)), 
    digits = 4) * 100,
  
  "total_area_abn_as_percent_of_total_crop_extent" = round(
...
(ref:caption-site-locations) Site locations with observed duration of cropland abandonment (in years) as of 2017.
...
We detected `r area_abn_stats$total_area_abn_Mha_all_sites` million ha (Mha) of abandoned croplands across our eleven sites as of 2017 (`r area_abn_stats$total_area_abn_as_percent_of_total_crop_extent`% of the `r area_abn_stats$total_crop_extent_area_Mha_all_sites` Mha of croplands cultivated at some point during the time series, `r area_abn_stats$total_area_abn_as_percent_of_total`% of the `r area_abn_stats$total_area_Mha_all_sites` Mha total site area, and `r area_abn_stats$total_area_abn_as_percent_of_crop87`% of the `r area_abn_stats$total_crop_area_1987_Mha_all_sites` Mha total cropland area in 1987; Figure \@ref(fig:site-locations).
Cropland abandonment at individual sites ranged between `r area_abn_stats$min_area_as_prop_site`% (`r area_abn_stats$min_area_as_prop_site_site`) and `r area_abn_stats$max_area_as_prop_site`% (`r area_abn_stats$max_area_as_prop_site_site`) of total site area (Figures \@ref(fig:area-abn-by-age-class) and \@ref(fig:area-abn-panel)^[Note: figure numbers above 4 will be updated to read "Fig. S1," etc. in the final version.]).
The notable exception was Mato Grosso, where only `r area_abn_stats$mg_abn_as_percent`% of the total site area was abandoned as of 2017 (see Section \@ref(mato-grosso)).

However, we also found that many of these abandoned croplands were recultivated during our time series: `r recult_stats$mean_thr_5`% of abandoned area, averaged across all sites (SD = `r recult_stats$sd_thr_5`%; Figure \@ref(fig:recult-by-threshold)).
Accordingly, the mean duration of abandonment across all sites was short: `r abn_duration_stats$mean_duration` years (SD = `r abn_duration_stats$sd_duration`), ranging between `r abn_duration_stats$duration_min` years (`r abn_duration_stats$duration_min_site`) and `r abn_duration_stats$duration_max` years (`r abn_duration_stats$duration_max_site`; Figure \@ref(fig:mean-abn-duration)).
...
<!-- ^[Note: The SI currently includes histograms for the full distribution of duration values at each site. However, if it would be helpful, we could modify this figure to show thin violin plots behind the mean values, to represent the distributions here in the main text. I’m currently keeping it simple.] -->
(ref:caption-mean-abn-duration) Mean length of time abandoned (years) throughout our time series across study sites. Because a single pixel may be abandoned and recultivated multiple times, the mean abandonment duration is calculated across all periods of abandonment (in red) and across the maximum length of abandonment for each pixel (in blue).
...
We defined a cohort of abandoned cropland as the group of all cropland abandoned in a given year at a given site.
We then tracked these cohorts through time, modeling the proportion of each cohort remaining abandoned as a function of time since initial abandonment (see Section \@ref(methods), Figure \@ref(fig:abn-decay-s)).
...
The primary exception was our site in Shaanxi/Shanxi Provinces (China), which showed more durable abandonment.
...
These mean decay trajectories also display wide variation in the proportion of abandoned croplands that persist beyond 20 years: as few as `r decay_stats$percent_at_20_min`% at `r decay_stats$percent_at_20_min_site`, up to `r decay_stats$percent_at_20_max`% at `r decay_stats$percent_at_20_max_site`.

(ref:caption-decay-curves-by-site) Mean decay trajectories for each site, based on the mean coefficients across all cohorts of abandoned cropland at each site.
...
A simple linear regression on the half-life (the time required for 50% recultivation) revealed that just over half of our sites show negative rates of change that were significantly different from zero: Bosnia & Herzegovina, Volgograd, Wisconsin, Nebraska, Orenburg, and Iraq (Figure \@ref(fig:decay-rate-of-change)).
...
If decay rates remain about the same at each site, and a similar and constant area of cropland is newly abandoned each year, a simple extrapolation suggests that the impermanence we observe is not merely an artifact of the narrow window of our time series.
We expect the mean duration of abandonment to plateau at 20 years or less at most sites (Figure \@ref(fig:extrapolation-combo)), with the notable exception of Shaanxi, which is projected to plateau at a longer mean duration of `r extrapolation_stats$plateau_max` years. 
The time required to plateau depends on the time required for complete turnover, ranging between `r extrapolation_stats$plateau_min_time`-`r extrapolation_stats$plateau_max_time` years for all sites except Mato Grosso (`r decay_stats$mg_0` years) and Shaanxi (`r decay_stats$shaanxi_0` years).
...
Modeled decay trajectories are shown for one example site, Shaanxi/Shanxi Provinces, China, in Figure \@ref(fig:abn-decay-s), and model coefficients for all sites are shown in Figure \@ref(fig:decay-mod-coef).
