...
To conservationists, these lands represent a potentially large source of new habitat as ecosystems regenerate [@Chazdon2020; @Xie2019; @Navarro2012].
...
As such, information on the long-term persistence and outcomes of abandoned lands is critical to understand the potential for these areas to help mitigate the ongoing climate and biodiversity crises.
...
We address three specific questions: How long does cropland abandonment last, on average, and how does this vary geographically? How quickly is land recultivated, as measured by individual recultivation (or "decay") rates, and how do these rates vary through time? Finally, if abandonment and recultivation rates stay about the same, how can we expect the mean age of abandoned croplands to change in these landscapes into the future?

By making use of more accurate and finer resolution abandonment maps, covering a longer period and broader set of sites than previous studies, we provide the most detailed analysis of the timing and persistence of cropland abandonment to date.
Together, these results provide much needed context on the temporal nature of cropland abandonment and its potential to provide ecosystem services and conserve biodiversity.
...
We found substantial amounts of cropland abandonment, observing `r area_abn_stats$total_area_abn_Mha_all_sites` million ha (Mha) of abandoned croplands across our eleven sites as of 2017 (`r area_abn_stats$total_area_abn_as_percent_of_total`% of the `r area_abn_stats$total_area_Mha_all_sites` Mha total site area; Figure \@ref(fig:site-locations).
Cropland abandonment at individual sites ranged between `r area_abn_stats$min_area_as_prop_site`% (`r area_abn_stats$min_area_as_prop_site_site`) and `r area_abn_stats$max_area_as_prop_site`% (`r area_abn_stats$max_area_as_prop_site_site`) of total site area (Figures \@ref(fig:area-abn-by-age-class) and \@ref(fig:area-abn-panel)^[Note: figure numbers will be updated to read "Fig. S1," etc. in the final version.]).
...
We tracked individual groups (i.e. "cohorts") of croplands that were abandoned in a given year, modeling the proportion of each cohort remaining abandoned as a function of time since initial abandonment (see Section \@ref(methods), Figure \@ref(fig:abn-decay-s)).
...
Our models predicted `r decay_stats$shaanxi_0.5` years to decline by half, and `r decay_stats$shaanxi_0` years for complete turnover (Figure \@ref(fig:decay-curves-by-site)).
...
By modeling recultivation for each cohort of abandonment, we are also able to investigate whether recultivation is accelerating.
...
A simple linear regression on the time required for 50% recultivation revealed that just over half of our sites show negative rates of change that were significantly different from zero: Bosnia & Herzegovina, Volgograd, Wisconsin, Nebraska, Orenburg, and Iraq (Figure \@ref(fig:decay-rate-of-change)).
...
Yet, even the sites with the most durable abandonment are unlikely to see large amounts of land persist for longer than 50 years.
...
Furthermore, by using an annual time series and tracking cohorts of abandoned croplands based on the year of abandonment, we develop a more accurate picture of abandonment persistence.
For example, while Shaanxi had among the shortest observed abandonment durations, current recultivation trajectories indicate more durable abandonment than we can observe during the time series, given an increase in abandonment later in the time series.
...
There is little information on variation in fallowing times around the world [@Chazdon2016a], but the abandonment and impermanence that we observe generally matches that observed in other studies.
...
Our projected 50% recultivation times of 12-24 years at most sites were similar to that observed by Reid et al. (20 years @Reid2019), but were slightly longer than those observed elsewhere in the Neotropics [@Chazdon2016a; @Nunes2020; @Smith2020].
...
Our results show that, contrary to optimistic assumptions, a significant amount of abandonment is unlikely to be permanent.
Furthermore, estimates of abandonment based on short time series or two points in time (e.g. 83 Mha estimated by Næss et al. [@Naess2021]) likely overestimate the amount of durable abandonment on the order of 20-45% or more. 
Recultivation at this scale may dramatically limit the scope for abandoned croplands to play a role in carbon sequestration or the regeneration of biodiversity.
...
Rarer, forest-adapted, and old-growth dependent species return even more slowly, and metrics that better capture the recovery towards old-growth ecosystems, such as community composition, species similarity, and vegetation structure, can take much longer to recover than richness or abundance, which can be dominated by widespread generalist species [@Meli2017; @Arroyo-Rodriguez2017; @Crouzeilles2016; @Rydgren2020].
...
Our sites cover a range of biomes^[I previously included the biomes for each site, but removed them because they took up so much real estate. I can put them back in, however. "Our sites cover a range of biomes, including Temperate Broadleaf and Mixed Forests (Belarus/Smolensk, Bosnia & Herzegovina, Wisconsin), Temperate Grasslands, Savannas, and Shrublands (Nebraska, Orenburg, Volgograd), and Tropical Grassland (Goiás, Mato Grosso), Tropical Moist Broadleaf Forests (Chongqing, Mato Grosso), Montane Grassland and Shrublands (Shaanxi), and Deserts & Xeric Shrublands (Iraq)."] (Figure \@ref(fig:site-biomes)), and land cover outcomes for abandoned croplands showed wide variation across sites, with most abandonment in Wisconsin being classified as forest by 2017 (`r lc_stats$forest_max`% forest, `r lc_stats$forest_max_grassland`% grassland), but remaining mostly in grassland elsewhere (Figure \@ref(fig:abn-prop-lc)).
...
Tropical ecosystems are among the most diverse in the world, but in some cases can regenerate quite quickly, though not on a time scale as short as the abandonment we observe here.
...
Some studies indicate quick recovery of vertebrate groups: for example, bird and dung beetle communities in regenerating pastures had recovered similarity to those in primary reference forests after 15-30 years in the Colombian Andes [@Gilroy2014].
However, most other studies point to longer recovery times. 
...
Some evidence indicates that grassland ecosystems recover more quickly following disturbances than forests [@Jones2009], but recent studies have highlighted the long recovery times for secondary grasslands. 
Nerlekar & Veldman [@Nerlekar2020] show that on average, secondary grasslands contained only 53-76% of the plant species richness of old-growth grasslands, and even after full recovery of species richness (with minimum recovery times of >100 years), compositional similarity to old-growth grasslands remained low (43%).
...
Recultivation is not the only potential barrier to regeneration in abandoned croplands.
Soil degradation, a lack of nearby source populations, a lack of natural disturbance regimes, and climate change all pose significant challenges [@Crouzeilles2016; @Crouzeilles2019]. 
Soil conditions recover slowly in restored agricultural landscapes, and is often incomplete even after 50 years [@Parkhurst2021].
...
Grassland regeneration faces particular challenges, because many grasslands require the maintenance of natural disturbance regimes (e.g. grazing or fire), which are often nonexistent in areas experiencing abandonment [@Nerlekar2020].
Some ecologists highlight the potential loss of grassland and farmland biodiversity if low-intensity farmlands are abandoned and replaced by woody vegetation, causing some species to gain and others to lose habitat, particularly in historically unforested ecosystems [@Wright2012; @Queiroz2014; @Nerlekar2020; @Sirami2008].
In some cases, approaches that aim to restore disturbance regimes and a mix of habitats might result in better conservation outcomes, depending on the species and ecosystems of conservation concern [@ReyBenayas2007].

## Drivers of abandonment and recultivation

Our results also include a range of interesting temporal patterns of abandonment. 
For example, Bosnia & Herzegovina displays a strong bifurcated pattern, where early abandonment remains relatively durable, while later other abandonment is recultivated relatively quickly. 
...
The length of fallow periods may vary widely around the world, and many studies use different definitions of abandonment. 
While distinguishing abandonment from long fallow periods remains a challenge, it is possible that longer abandonment thresholds would be more appropriate in certain contexts.
We varied our definition (Section \@ref(abn-thresholds)) and found that, as expected, longer definitions resulted in less abandonment over all, longer average abandonment durations (Figure \@ref(fig:abn-thresholds-mean-duration)), and lower recultivation rates (Figure \@ref(fig:recult-by-threshold)). 
However, even when only considering abandonment longer than 10 years, we still observed between `r recult_stats$min_10`% and `r recult_stats$max_10`% recultivation at `r recult_stats$min_10_site` and `r recult_stats$max_10_site` respectively, enough to significantly curtail regeneration and any associated benefits.
...
Without the development of careful policies and concerted efforts to protect these croplands, they are unlikely to provide significant biodiversity and carbon benefits. 
...
Agricultural abandonment is the product of broad societal changes, and the pain associated with the loss of certain landscapes and ways of life should be addressed.
...
Our results make one thing clear: if cropland abandonment continues to be as short-lived as we show here, the negative socioeconomic consequences of abandonment will continue to dominate, and the large potential benefits of regenerating habitats to both store carbon and sustain biodiversity will remain an untapped opportunity.
...
Our 11 sites were mapped with high accuracy and provide broad coverage of different continents and ecosystems (Figure \@ref(fig:site-locations)).
...
To exclude short-term fallowing, we define croplands as abandoned when they remain uncultivated for five or more consecutive years (following FAO [@FAO2016]; see sensitivity analysis in Section \@ref(abn-thresholds)).
