...
# you can just use na_first!
# na_first and na_last are essentially the same, just different names for the variables.
gg_1 <- ggplot() + 
    theme_classic() + 
...
# original way
gg_1 + geom_line(data = persistence_list$na_last, size = 1.25,
                 mapping = aes(x = time_abn, 
                   # x = age, # (replicates the plot from na_last, which is called time_abn)
                   y = proportion, group = year_abn, color = year_abn))

gg_1 + geom_line(data = persistence_list$na_first, size = 1.25,
                 mapping = aes(x = age, # (replicates the plot from na_last, which is called time_abn)
                               y = proportion, group = year_abn, color = year_abn))
...
gg_1 + geom_line(data = persistence_list$na_first, size = 1.25,
                 mapping = aes(x = age, 
                   y = area_ha, group = year_abn, color = year_abn))
