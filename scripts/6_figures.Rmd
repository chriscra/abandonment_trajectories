...
test <- bind_rows(df, df_av)

# as percentage
base <- ggplot(data = persistence_list_s$na_last) + 
    theme_classic() + 
...
ggplot(data = persistence_list_s$na_last) + 
  theme_classic() + 
  ...
  #             mapping = aes(x = time_abn, y = proportion), 
...
lm(proportion ~ poly(time_abn, 2), data = persistence_list_s$na_last)
...
for (i in 2:11) {
  cc_save_area_persistence_plots(
    ...
    site_label = site_label_list[i], 
    blip_label = blip_label,
    outfile_label = paste0(site_label_list[i], blip_label),
