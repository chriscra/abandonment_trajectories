...
gg_age_class_bins_grid <-
    ...
    scale_fill_brewer(palette = "Greens", direction = 1) +
      facet_wrap(vars(site), scales = "free_y", labeller = as_labeller(cap_update_labels)) +  # see _util_misc_functions.R
...
age_r <- lapply(seq_along(age_files), function(i) {rast(age_files[i])})
names(age_r) <- site_df$site
for (i in seq_along(age_r)) {names(age_r[[i]]) <- paste0("y", 1987:2017)} # remember: these are just 1987:2017
...
max_age_r <- lapply(seq_along(max_age_files), function(i) {
  # tmp <- 
    ...
    # tmp <- rast(max_age_files[i]) # load as SpatRasters
# 
#   names(tmp) <- "max_age"
#   tmp
  })
...
# SpatRaster
max_age_t <- lapply(seq_along(max_age_files), function(i) {rast(max_age_files[i])})
...
levelplot(max_age_r$shaanxi, margin = list(FUN = 'mean'), #contour=TRUE, 
          par.settings = YlOrRdTheme,
          main = "Max. abandonment duration", maxpixels = 1e6)

levelplot(max_age_r$shaanxi, margin = F, #contour=TRUE, 
          par.settings = YlOrRdTheme,
          main = "Max. abandonment duration",
...
# test it out
for(i in 9) {cc_age_levelplot_hist(site_index = i, year_or_max = "max")}
...
names(tt) <- paste0("y", 1987:2017)
tt
plot(tt$y2017, type="interval", plg=list(x="topright"))
...
plot(age_t$shaanxi$y2017, 
     type = "interval", 
     breaks = seq(from = 5, to = 30, by = 5), 
     col = viridis(n = 5, direction = -1), colNA = "gray95")


plot(age_t$shaanxi$y2017, type = "interval", breaks = seq(from = 5, to = 30, by = 5),
     plg=list(x="topright"), mar = c(2.1, 2.1, 1.1, 1.1))
...
# rasterVis
# ------------------------------------------------------------------- #
levelplot(age_r$shaanxi$y2017,
          main = site_sf$lab_desc[9],
          colorkey = TRUE, scales=list(draw=TRUE),
          # par.settings = myTheme,
          at = seq(5, 30, by = 1), 
          xlab = NULL, ylab = NULL, margin = FALSE,
...
tm_shape(World) +
  ...
  # tm_shape(age_r$shaanxi$y2017) +
  #   tm_raster("y2017", palette = terrain.colors(10))
...
smol <- aggregate(age_r$shaanxi$y2017, fact = 10, fun = max)
smoler <- aggregate(smol, fact = 10)
plot(smol)
plot(smoler)

dev.off()

temp <- copy(age_r$shaanxi$y2017)
temp
plot(temp)

tm_shape(rast(age_r$shaanxi$y2017)) + tm_raster(alpha = 0.7)

tm_shape(smoler) + tm_raster(alpha = 0.7)
tm_shape(temp) + tm_raster(alpha = 0.7)
...
png(filename = paste0(p_output, "plots/terra_inset.png"), 
...
ggsave(plot = ggdraw() + 
         draw_plot(gg_world_crop, scale = 0.68) + 
         draw_image(paste0(p_output, "plots/terra_inset.png"),
                    x = 0.3, y = 0.3, scale = 0.25),
       filename = paste0(p_plots, run_label,
                         "/fig1_test", ".pdf"),
       width = 10, height = 6, units = "in", dpi = 400)
```


```{r sort-out-which-maps-to-plot}
# Question: how does terra make its bins for plotting?
...
plot(st, type = "classes", levels = rcl_df$labels %>% unique() %>% .[-1])


# reclassify into bins
s_rcl <- terra::classify(
  age_t[[9]]$y2017,
  rcl = select(rcl_df, is, becomes), 
  filename = paste0(p_dat_derived, "age_rasters/2017_bins/", site_df$site[9], "_y2017_bins.tif"),
  overwrite = TRUE)

plot(s_rcl)
plot(s_rcl, type = "classes",
     levels = rcl_df$labels %>% unique() %>% .[-1],
     col = viridis(n = 5, direction = -1), colNA = "gray95",
     main = inset_map_titles[9],
     legend = FALSE)



# save reclassified raster as pdf
pdf(file = paste0(p_dat_derived, "age_rasters/2017_bins/", site_df$site[9], "_y2017_bins.pdf"),
...
# base r plot with terra
pdf(file = paste0(p_dat_derived, "age_rasters/2017_bins/", site_df$site[9], "_y2017_interval.pdf"),
    width = 4.5, height = 4.5)
par(bg=NA, mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(age_t$shaanxi$y2017, type = "interval", 
     breaks = seq(from = 5, to = 30, by = 5), 
     col = viridis(n = 5, direction = -1), colNA = "gray95",
     main = "interval terra", legend = TRUE)
dev.off()


pdf(file = paste0(p_dat_derived, "age_rasters/2017_bins/", site_df$site[9], "_y2017_interval_man.pdf"),
    width = 4.5, height = 4.5)
par(bg=NA, mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(age_t$shaanxi$y2017, type = "interval", 
     breaks = c(4.99, 10, 15, 20, 25, 30), 
     col = viridis(n = 5, direction = -1), colNA = "gray95",
     main = "interval terra manual", legend = TRUE)
dev.off()

pdf(file = paste0(p_dat_derived, "age_rasters/2017_bins/", site_df$site[9], "_y2017_interval_man2.pdf"),
    width = 4.5, height = 4.5)
par(bg=NA, mar=c(0,0,0,0), oma=c(0,0,0,0))
plot(age_t$shaanxi$y2017, type = "interval", 
     breaks = c(5, 11, 16, 21, 26, 30), 
     col = viridis(n = 5, direction = -1), colNA = "gray95",
     main = "interval terra manual 2", legend = TRUE)
dev.off()


# based on this exploration, I now know that breaks c(5, 10, 15, 20, 25, 30) results in 
# intervals that are closed on the left (including 10, but not 15, for example). strange!
...
tt <- classify(age_t$shaanxi$y2017, seq(from = 5, to = 30, by = 5), right = FALSE, include.lowest = TRUE) # right = logical, indicating if the intervals should be closed on the right (and open on the left) or vice versa. The default is TRUE. A special case is to use right=NA. In this case both the left and right intervals are open.

ttr <- classify(age_t$shaanxi$y2017, c(0, seq(from = 5, to = 30, by = 5)), 
                right = TRUE, include.lowest = TRUE, othersNA = TRUE)

levels(tt)
levels(ttr)


plot(ttr, col = viridis(n = 5, direction = -1), colNA = "gray95")
plot(tt, col = viridis(n = 5, direction = -1), colNA = "gray95")



# trying to figure out the plotting... are the intervals open on the left and closed on the right (5, 10] {i.e. not including 5, but including 10},  or vice versa.
persistence_dat %>% print(n = 30)

plot(age_t$shaanxi$y2017, type = "interval", 
     breaks = seq(from = 5, to = 30, by = 5), 
     col = viridis(n = 5, direction = -1), bg = "dark gray")

plot(age_t$shaanxi$y2017, type = "interval", 
     breaks = seq(from = 5, to = 30, by = 5), 
     col = viridis(n = 6, direction = -1), bg = "dark gray")

plot(age_t$shaanxi$y2017, type = "interval", 
     breaks = seq(from = 5, to = 30, by = 5), 
     col = viridis(n = 6, direction = -1)[2:6], bg = "dark gray")



# ok - the plan is to simply reclassify the age data into bins, so that I know exactly what's being plotted.
...
# reclassify into bins (age rasters)
...
# reclassify into bins (max_age rasters)
...
plot(age_t_bins[[9]], type = "classes",
     levels = rcl_df$labels %>% unique() %>% .[-1],
     col = viridis(n = 5, direction = -1), colNA = "gray95",
     main = inset_map_titles[9],
     legend = FALSE)
...
world_states_simple %>% st_drop_geometry() %>% as_tibble()

names(world_states_simple)
world_states_simple$type_en %>% unique


world_states_simple %>% st_drop_geometry() %>% as_tibble() %>% select(name, name_alt, type_en, adm0_label, adm0_a3, geonunit)


states_union <- world_states_simple %>% 
  group_by(geonunit) %>%
...

{
  pdf(file = paste0(p_output, "plots/fig1", 
                    if(include_borders) {"_borders_simple2"},
                    ".pdf"),
      width = 7, height = 6)
...
pdf(file = paste0(p_dat_derived, "age_rasters/2017_bins/fig1_y2017_composite_v.pdf"),
    width = 7.5, height = 10)
...
par(panel.background = list(col="skyblue"))
plot(age_t_bins$iraq)

dims <- lapply(1:11, function(i) {
  tibble(site = site_df$site[i],
         x = dim(age_t_bins[[i]])[2],
         y = dim(age_t_bins[[i]])[1])
}) %>% bind_rows() %>%
  mutate(ratio = y/x)
...
cc_save_fig1_inset <- function(i, include_borders = FALSE, titles_vector = inset_map_titles, maxpixels = 5e6,
                               myh = 4.5, myw = 4.5) {
 pdf(file = paste0(p_output, "plots/fig1/x", 
                    if(!include_borders) {"fig1_inset"} else {"fig1_inset_borders"}, 
                    site_df$label[i], ".pdf"),
    width = myw, height = myh)
  
  # png(filename = paste0(p_output, "plots/fig1/fig1_inset", site_df$label[i], ".png"), 
  #   width = 4.5, height = 4.5, units = "in", res = 400)
...
cc_save_fig1_inset(i = 8, myh = 4.25, myw = 4.25)
...
{
pdf(file = paste0(p_output, "plots/fig1/fig1_inset_legends_test.pdf"), 
    width = 4, height = 5)
plot.new()
par(bg=NA, mar=c(0,0,0,0), oma=c(0,0,0,0))
...
# ------------------------------------------------------------------ #
# Save site age map as level plot
# ------------------------------------------------------------------ #
...
strwrap(site_df$description, width = 0.5) %>% paste(., sep = "\n")

for (i in 1:11) {
  pdf(paste0(p_output, "plots/fig1/test_fig1_inset_lp_border", site_df$label[i], ".pdf"), 
    width = my_width, height = my_height)
  
    # png(paste0(p_output, "plots/fig1/fig1_inset", site_df$label[i], "_lp.png"), 
    # width = 4.5, height = 4.5, units = "in", res = 400)
    
  myTheme <- viridisTheme()
  myTheme$regions$col <- viridis(n = 5, direction = 1)
  myTheme$panel.background$col = 'gray95' 
  ...
  print(
    levelplot(
        age_r[[i]]$y2017,
        # main = inset_map_titles[i],
        colorkey = include_legend,
        scales=list(draw=TRUE),
  ...
  layer(sp.polygons(world_states %>% 
                      {if (i %in% c(4, 6)) . else  st_crop(., extent(site_sf[i, ]))} %>% 
                      as_Spatial(), 
                    col = "gray50")) +
  layer(sp.polygons(world_10 %>% 
                      {if (i %in% c(4, 6)) . else  st_crop(., extent(site_sf[i, ]))} %>% 
                      as_Spatial(), 
                    col = "black"))
        )
...
```{r ggdraw-w-levelplots}
# first with aggregated rasters, since they're much easier to work with
age_2017_aggregated_10_max_l

# save level plots to objects
for (i in 1:11) {
  assign(
  x = paste0("lpt", site_df$label[i]),
  value =
      levelplot(
        age_2017_aggregated_10_max_l[[i]],
        main = site_sf$lab_desc[i],
...
# save as pdf
for (i in 1:11) {
  png(filename = paste0(p_output, "plots/terra_inset2", site_df$label[i], ".png"), 
...
gg_fig1 <- ggdraw() + 
  draw_plot(gg_world_crop, scale = 0.68) + 
  draw_plot(lpt_w, x = 0.10, y = 0.6, scale = 0.25) + 
  draw_plot(lpt_b, x = 0.30, y = 0.6, width = 0.25, height = 0.25, vjust = 0) + 
  draw_plot(lpt_v, x = 0.50, y = 0.6, width = 0.25, height = 0.25, vjust = 0) + 
  draw_plot(lpt_s, x = 0.70, y = 0.6, width = 0.25, height = 0.25, vjust = 0)
...
# figure out plotting alignment
gcircle <- grid::circleGrob()
ggdraw() + 
  draw_plot(gcircle, height = 0.25, 
            # ok, starts in the middle of the plot (x axis), bottom (y axis)
            x = 0.1, y = 0.1,
            hjust = 0.5, vjust = 0.5, 
            # halign = 0, valign = 0
            )
...
gg_fig1.2 <-
  ggdraw() + 
  draw_plot(gg_world_crop, height = 0.45, 
            x = 0.5, y = 0.5, hjust = 0.5, vjust = 0.5) + 
    
  # top row, left to right
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[11], ".png"),
             x = 0.1, y = top_row_y, hjust = 0.5, vjust = 0.5, 
             height = inset_height) +
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[1], ".png"),
             x = 0.375, y = top_row_y, hjust = 0.5, vjust = 0.5, 
             height = inset_height) +
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[10], ".png"),
             x = 0.625, y = top_row_y, hjust = 0.5, vjust = 0.5, 
             height = inset_height) +
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[8], ".png"),
             x = 0.9, y = top_row_y, hjust = 0.5, vjust = 0.5,
             height = inset_height) #+
  
  # left column
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[7], ".png"),
             x = 0, y = 0.6, hjust = 0.5, vjust = 0.5, 
             height = inset_height) +   
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[6], ".png"),
             x = 0, y = 0.4, hjust = 0.5, vjust = 0.5, 
             height = inset_height) +
  
  # right column
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[9], ".png"),
             x = 1, y = 0.5, hjust = 0.5, vjust = 0.5, 
             height = inset_height) +   
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[3], ".png"),
             x = 1, y = 0.5, hjust = 0.5, vjust = 0.5, 
             height = inset_height) +
    
    
  # bottom row
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[4], ".png"),
             x = 0, y = bottom_row_y, hjust = 0.5, vjust = 0.5, 
             height = inset_height) +   
  
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[2], ".png"),
             x = 0, y = bottom_row_y, hjust = 0.5, vjust = 0.5, 
             height = inset_height) +   
  
  draw_image(paste0(p_output, "plots/terra_inset", site_df$label[5], ".png"),
             x = 0, y = bottom_row_y, hjust = 0.5, vjust = 0.5, 
             height = inset_height)


ggsave(plot = gg_fig1.2,
       filename = paste0(p_plots, run_label,
                         "/fig1_test3", ".pdf"),
...
# 
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[11], ".png"),
#              x = 0, y = 0, hjust = 0, vjust = 0, halign = 0.0, valign = 1,
#              height = inset_height) #+
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[1], ".png"),
#              x = 0, y = 1, hjust = 0, vjust = 1, halign = 1/3, valign = 1,
#              height = inset_height) +
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[10], ".png"),
#              x = 0, y = 1, hjust = 0, vjust = 1, halign = 2/3, valign = 1,
#              height = inset_height) +
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[8], ".png"),
#              x = 0, y = 1, hjust = 0, vjust = 1, halign = 1, valign = 1,
#              height = inset_height) +
#   
#   # left column
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[7], ".png"),
#              x = 0, y = 0.6, hjust = 0, vjust = 0.5, halign = 0, valign = 0,
#              height = inset_height) +   
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[6], ".png"),
#              x = 0, y = 0.4, hjust = 0, vjust = 0.5, halign = 0, valign = 0,
#              height = inset_height) +
#   
#   # right column
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[9], ".png"),
#              x = 1, y = 0.5, hjust = 1, vjust = 0, halign = 1, valign = 0,
#              height = inset_height) +   
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[3], ".png"),
#              x = 1, y = 0.5, hjust = 1, vjust = 1, halign = 1, valign = 0,
#              height = inset_height) +
#     
#     
#   # bottom row
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[4], ".png"),
#              x = 0, y = 0, hjust = 0, vjust = 0, halign = 0, valign = 0,
#              height = inset_height) +   
#   
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[2], ".png"),
#              x = 0, y = 0, hjust = 0, vjust = 0, halign = 1/3, valign = 0,
#              height = inset_height) +   
#   
#   draw_image(paste0(p_output, "plots/terra_inset", site_df$label[5], ".png"),
#              x = 0, y = 0, hjust = 0, vjust = 0, halign = 2/3, valign = 0,
#              height = inset_height)
```
