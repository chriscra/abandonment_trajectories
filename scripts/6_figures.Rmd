...
max_age_files <- list.files(paste0(p_dat_derived, "max_age/", run_label), 
                        full.names = TRUE) %>%
  grep(".tif", ., value = TRUE)
max_age_r2 <- lapply(seq_along(max_age_files), function(i) {brick(max_age_files[i])})
names(max_age_r2) <- site_df$site

max_age_r <- lapply(seq_along(max_age_files), function(i) {rast(max_age_files[i])})
...
world_10 <- ne_countries(scale = "large", returnclass = "sf") # can set returnclass to sf or sp.
...
plot(age_t$shaanxi$y2017, type = "interval", breaks = seq(from = 5, to = 30, by = 5), col = viridis(n = 5), bg = "dark gray")

tt <- classify(age_t$shaanxi$y2017, seq(from = 5, to = 30, by = 5), right = FALSE)
ttr <- classify(age_t$shaanxi$y2017, seq(from = 5, to = 30, by = 5), right = TRUE)

levels(tt)
levels(ttr)

plot(tt)
plot(ttr, col = viridis(n = 5, direction = -1))
...
world_crop <- st_crop(world, extent(site_sf) + c(50, 20))
...
for (i in 1:11) {
  ...
  pdf(file = paste0(p_output, "plots/fig1/fig1_inset_borders", site_df$label[i], ".pdf"),
