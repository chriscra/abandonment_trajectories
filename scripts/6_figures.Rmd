...
mod_decay_s <- lm(log(proportion) ~ age, 
                  data = persistence_list_b1_s$na_first)
...
dat
ggplot(data = dat, mapping = aes(x = time, y = proportion,
                                 group = year_abn, color = year_abn)) + 
  geom_point(size = 1.25) + 
  scale_color_distiller(palette = "Greens") + theme(legend.position = "bottom") + 
  
  geom_function(fun = function(x) {exp(lm_mod_offset$coefficients["time"]*x)},
                color = "dark blue", inherit.aes = FALSE) +
  
  geom_function(fun = function(x) {exp(coef(lm_mod_offset1)["time"]*x + 
                                         coef(lm_mod_offset1)["(Intercept)"])},
                color = "red", inherit.aes = FALSE) + 
  
  geom_function(fun = function(x) {exp(mean(coef(lmer_mod_offset)$year_abn$time)*x)},
...
lmer_mod_offset <- lmer(log(proportion) ~ 0 + time + (0 + time|year_abn), data = dat)

summary(lm_mod_offset)
coef(lm_mod_offset)

summary(lm_mod_offset1)
summary(lmer_mod_offset)
mean(coef(lmer_mod_offset)$year_abn$time)
tidy(lmer_mod_offset) %>% filter(term == "time") %>% select(estimate) %>% unlist()
...
lm_mod <- lm(formula = log(proportion) ~ age, data = dat)
summary(lm_mod)

# random slopes
lmer_mod <- lmer(log(proportion) ~ 
       age + # fixed effect (i.e. what we're curious about)
       (0 + age|year_abn), # random effects (random values drawn from a common error distribution, i.e. differences that we want to control for). 
...
lmer_mod_half_lives <- coef(lmer_mod)$year_abn %>%
...
hist(dat$proportion)
ggplot(data = dat, mapping = aes(x = proportion)) + 
