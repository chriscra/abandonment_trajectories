...
for (i in 1:11) {
  ...
  site_mn_coef_tmp <- tidy(lm_lin_log_lin_l[[i]]) %>% 
  ...
  ggplot(data = filter(dat_l, site == site_df$site[i]), 
    ...
    geom_function(fun = function(x) {
      site_mn_coef_tmp$mean_est[2] * log(x-4) + site_mn_coef_tmp$mean_est[1] * (x - 4 - 1) + 1},
                  mapping = aes(linetype = "Mean Decay Rate"),
                  color = "blue", size = 1, #linetype = "dashed", 
                  inherit.aes = FALSE) +
    scale_linetype(name = "")

  gg_lin_log_lin_cohort_tmp <- ggplot(mapping = aes(x = age, y = proportion, group = year_abn, color = year_abn)) + 
