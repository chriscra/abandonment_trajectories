...
# save mod_AIC
write_csv(mod_AIC, path = paste0(p_dat_derived, "mod_AIC.csv"))
# 
# # re load
...
mean_coef_proj_l3 <- bind_rows(mean_coef_df_wide_l3, 
                          ...
                          # time = i + 4 (since time is age - 5)
                          lapply(c(1, 6, 11, 16, 21, 26, 36, 46) - 1, FUN = function(x) {
...
time_to_l3 <- bind_rows(#select(mean_coef_df_wide_l3, -contains("half_")), 
                          ...
                          lapply(seq(from = 0.5, to = 0.9, by = 0.1), FUN = function(x) {
                            select(mean_coef_df_wide_l3, -contains("half_")) %>%
                             # need to update this to reflect the new formula:
                              #{mean_coef_tmp$mean_est[2]*log(x-4) + mean_coef_tmp$mean_est[1]*((x-4) - 1) + 1}
                              # mean*log(x) + mean*(x - 1) + 1
                               mutate(
                                 time_to_value = ((log_mean * 
                                                     lambertW0((lin_mean * 
                                                                  exp(lin_mean/log_mean + 
                                                                        x/log_mean - 1/log_mean)) / 
                                                                 log_mean)) / lin_mean) + 4,
                                 time_to_value_low = ((log_mean_low * 
                                                         lambertW0((lin_mean_low * 
                                                                      exp(lin_mean_low/log_mean_low + 
                                                                            x/log_mean_low - 1/log_mean_low)) / 
                                                                     log_mean_low)) / lin_mean_low) + 4,
                                 time_to_value_high = ((log_mean_high * 
                                                          lambertW0((lin_mean_high * 
                                                                       exp(lin_mean_high/log_mean_high + 
                                                                             x/log_mean_high - 1/log_mean_high)) /
...
# save
png(filename = paste0(p_output, "plots/time_to_0.5_l3.png"), 
    width = 6, height = 5, units = "in", res = 400)
