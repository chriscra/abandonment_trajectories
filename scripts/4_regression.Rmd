...
dat_l <- mutate(dat_l, site = as_factor(site))
...
write_csv(dat_l, path = paste0(p_dat_derived, "abn_decay_data.csv"))

# when reloading, must restructure factors:
dat_l <- read_csv(file = paste0(p_dat_derived, "abn_decay_data.csv"))
dat_l <- mutate(dat_l, 
                site = as.factor(site),
                bins = as.factor(bins),
...
# log-log-lin model (i.e. including both log and linear time effects), with cohort fixed effects
lm_log2_lin_l <- lapply(1:11, FUN = function(i) {
  lm(log(proportion) ~ 0 + log(time_1):cohort + time_1:cohort,  
               data = filter(dat_l, site == site_df$site[i]))
})
  
# modeling proportion as a linear term, predicted by log(time_1), with cohort fixed effects
lm_lin_log_l <- lapply(1:11, FUN = function(i) {
  lm((proportion - 1)  ~ 0 + log(time_1):cohort,  
               data = filter(dat_l, site == site_df$site[i]))
})

# linear proportion predicted by both log and linear time terms, with cohort fixede effects.
lm_lin_log_lin_l <- lapply(1:11, FUN = function(i) {
  lm((proportion - 1)  ~ 0 + log(time_1):cohort + time_1:cohort,  
               data = filter(dat_l, site == site_df$site[i]))
})
...
for (i in 1:11) {
  ...
  gg_lin_log_lin_cohort_tmp <- ggplot(data = filter(dat_l, site == site_df$site[i]), 
    ...
    labs(title = "Decay of abandoned land over time", 
         ...
         caption = "lm formula: (proportion - 1) ~ 0 + log(time_1):cohort + (time_1 - 1):cohort",
    ...
    geom_line(data = mutate(augment(lm_lin_log_lin_l[[i]]),
                            time_1 = round(exp(`log(time_1)`), digits = 0),
                            age = time_1 + 4,
                            proportion = .fitted + 1,
  ...
  png(filename = paste0(p_output, "plots/l3_cohort", site_df$label[i], ".png"), 
    width = 8, height = 6, units = "in", res = 400)
  print(gg_lin_log_lin_cohort_tmp)
  dev.off()
...
for (i in 1:11) {
  ...
  gg_lin_log_cohort_tmp <- 
    ...
    labs(title = "Decay of abandoned land over time", 
         subtitle = site_df$description[i], 
         caption = "lm formula: (proportion - 1) ~ 0 + log(time_1):cohort",
         x = "Time since initial abandonment (years)", 
         y = "Proportion of abandoned land remaining",
         color = "Cohort (Year \nFirst Abandoned)",
         linetype = "") + 
  ...
  png(filename = paste0(p_output, "plots/lin-log_cohort", site_df$label[i], ".png"), 
    width = 8, height = 6, units = "in", res = 400)
...
for (i in 1:11) {
  ...
  png(filename = paste0(p_output, "plots/l3_log2_combo", site_df$label[i], "_2.png"), 
...
# in order to plot each cohorts trajectory out to an age of 30:
mutate(augment(lm_log2_l[[i]],
        newdata = as_tibble(data.frame(time_1 = 1:26,
                                       cohort = as_factor(rep(1988:2013, each = 26))))),
       year_abn = as.numeric(levels(cohort))[cohort])
...
ggplot(mapping = aes(x = age, y = proportion, group = year_abn, color = year_abn)) + 
    ...
    # fitted values
    geom_line(data = mutate(augment(lm_log2_l[[i]],
                                    newdata = tibble(time_1 = rep(1:26, time = 26),
                                                     cohort = as_factor(rep(1988:2013, each = 26)))),
                            age = time_1 + 4,
                            proportion = exp(.fitted),
                            year_abn = as.numeric(levels(cohort))[cohort])) + 
...
for (i in 1:11) {
  ...
  png(filename = paste0(p_output, "plots/l3_log2_combo_extrapolation", site_df$label[i], ".png"), 
    width = 13, height = 7.5, units = "in", res = 400)
  
  print(plot_grid(gg_log2_cohort_tmp + theme(legend.position = "none"), 
              gg_lin_log_lin_cohort_tmp, 
...
# ------------------------------------------------ #
lm_log2_cohort_site <- lm(log(proportion) ~ 0 + log(time_1):cohort:site, data = dat_l)
...
lm_log2_site <- lm(log(proportion) ~ 0 + log(time_1):site, data = dat_l)


lm(log(proportion) ~ 0 + log(time_1):cohort:site, data = dat_l)
...
# diagnostic plots
# ------------------------------------------------ #
png(filename = paste0(p_output, "plots/resid_fitted_mega_lm.png"), 
    width = 5, height = 5, units = "in", res = 400)
# without the outlier:
plot(lm(log(proportion) ~ 0 + log(time_1):cohort:site, data = dat_l[-2092, ]), 1, main = "Model with all sites, all cohorts")
dev.off()

png(filename = paste0(p_output, "plots/mega_lm_diag_plots.png"), 
...
car::qqPlot(lm(log(proportion) ~ 0 + log(time_1):cohort:site, 
...
png(filename = paste0(p_output, "plots/raw_persist_data", site_label, ".png"), 
    width = 5, height = 5, units = "in", res = 400)
...
pl1 + 
  ...
  geom_function(fun = function(x) { (x - 4) ^ 
      coef(lm(log(proportion) ~ 0 + log(time_1), 
              data = filter(dat_l, site == "shaanxi")))},
...
lapply(1:11, FUN = function(i) {
  ...
  png(filename = paste0(p_output, "plots/log2_cohort_diag", site_df$label[i],".png"), 
    width = 8, height = 8, units = "in", res = 400)
  par(mfrow = c(2,2))
...
png(filename = paste0(p_output, "plots/log2_cohort_diag_resid_v_fitted.png"), 
...
# ----------------------- QQ diagnostics plot ----------------------- #
png(filename = paste0(p_output, "plots/log2_cohort_diag_qq.png"), 
    width = 9, height = 8, units = "in", res = 400)
par(mfrow = c(3, 4))
...
# ----------------------- fitted vs. residuals diagnostics plot ----------------------- #
png(filename = paste0(p_output, "plots/lin-log_cohort_diag_resid_v_fitted.png"), 
    width = 9, height = 8, units = "in", res = 400)
par(mfrow = c(3, 4))
...
# ----------------------- QQ diagnostics plot ----------------------- #
png(filename = paste0(p_output, "plots/lin-log_cohort_diag_qq.png"), 
    width = 9, height = 8, units = "in", res = 400)
par(mfrow = c(3, 4))
...
# ----------------------- fitted vs. residuals diagnostics plot ----------------------- #
png(filename = paste0(p_output, "plots/l3_cohort_diag_resid_v_fitted.png"), 
    width = 9, height = 8, units = "in", res = 400)
par(mfrow = c(3, 4))
...
# ----------------------- QQ diagnostics plot ----------------------- #
png(filename = paste0(p_output, "plots/l3_cohort_diag_qq.png"), 
    width = 9, height = 8, units = "in", res = 400)
par(mfrow = c(3, 4))
...
# ----------------------- QQ diagnostics plot ----------------------- #
png(filename = paste0(p_output, "plots/l3_cohort_diag_qq_subset.png"), 
    width = 9, height = 8, units = "in", res = 400)
par(mfrow = c(3, 4))
for (i in 1:11) {
  qqPlot(lm(formula = I(proportion - 1) ~ 0 + log(time_1):cohort + I(time_1 - 1):cohort, 
...
dev.off()
par(mfrow = c(3, 4))
plot(lm(formula = log(proportion) ~ 0 + log(time_1):cohort + I(time_1 - 1):cohort, 
        data = filter(dat_l, site == site_names[i])))

plot(lm(formula = I(proportion - 1) ~ 0 + log(time_1):cohort + I(time_1 - 1):cohort, 
...
# save
png(filename = paste0(p_output, "plots/log2_cohort_resid_all.png"), 
    width = 10, height = 5, units = "in", res = 400)
...
plot(lm(formula = log(proportion) ~ 0 + log(time_1):cohort, 
...
influencePlot(lm(formula = log(proportion) ~ 0 + log(time_1):cohort, 
...
lm_out <- lm(log(proportion) ~ 0 + log(time_1):cohort,  
...
ggplot(mapping = aes(x = age, y = proportion, group = year_abn, color = year_abn)) + 
  ...
  geom_line(data = mutate(augment(lm_log2_l[[9]], 
                                  newdata = filter(dat_l, site == "shaanxi") %>% 
                                    select(time_1, cohort, site, year_abn, age)),
                          proportion = exp(.fitted))) + 
...
gg_log2_cohort_coef_all <- 
  ...
  labs(y = "Model Coefficient (Power Term)",
       x = "Cohort (Year of Initial Abandonment)", 
       title = "Model Coefficients", #subtitle = "log(proportion) ~ 0 + log(time_1):cohort",
       color = "Site") + 
...
png(filename = paste0(p_output, "plots/log2_cohort_decay_proportions_by_site.png"), 
...
png(filename = paste0(p_output, "plots/model_decay_curves_by_site.png"), 
    width = 7, height = 6, units = "in", res = 400)
...
png(filename = paste0(p_output, "plots/log2_cohort_coefs_rate_of_change_by_site_flip.png"), 
    width = 5, height = 6, units = "in", res = 400)
...
coef_df_l3 <- lapply(lm_lin_log_lin_l, FUN = function(x) {
  tidy(x, conf.int = TRUE) %>% 
    ...
    mutate(term_long = term,
           term = ifelse(coef_term == "I(time_1 - 1)", "lin", "log")) %>%
...
mean_coef_df_wide_l3 <- mean_coef_df_l3 %>%
  ...
  # calculate the half_times
  mutate(half_time = ((log_mean * lambertW0((lin_mean * exp(lin_mean/log_mean + 0.5/log_mean - 1/log_mean))/log_mean))/lin_mean) + 4,
         half_time_low = ((log_mean_low * lambertW0((lin_mean_low * exp(lin_mean_low/log_mean_low + 0.5/log_mean_low - 1/log_mean_low))/log_mean_low))/lin_mean_low) + 4,
         half_time_high = ((log_mean_high * lambertW0((lin_mean_high * exp(lin_mean_high/log_mean_high + 0.5/log_mean_high - 1/log_mean_high))/log_mean_high))/lin_mean_high) + 4)


# join to all coef df
coef_df_l3 <- left_join(coef_df_l3, 
                        mean_coef_df_l3 %>% select(-term),
                        by = c("site", "coef_term"))# %>% filter(site == "belarus") %>% print(n = 30)
...
coef_df_wide_l3 <- 
  coef_df_l3 %>%
  pivot_wider(id_cols = c(site, cohort, year_abn), 
              names_from = term,
              names_glue = "{term}_{.value}",
              values_from = estimate) %>%
  mutate(half_time = ((log_estimate * lambertW0((lin_estimate * exp(lin_estimate/log_estimate + 0.5/log_estimate - 1/log_estimate))/log_estimate))/lin_estimate) + 4)


# join mean half time to coef_df_wide_l3
coef_df_wide_l3 <- left_join(coef_df_wide_l3,
                             mean_coef_df_wide_l3 %>% select(site, lin_mean, log_mean, mean_half_time = half_time),
                             by = c("site"))
...
mean_coef_proj_l3 <- bind_rows(mean_coef_df_wide_l3, 
                          ...
                          # time = i + 4 (since time_1 is age - 4)
                          lapply(c(1, 6, 11, 16, 21, 26, 36, 46), FUN = function(x) {
                            mean_coef_df_wide_l3 %>%
                             # need to update this to reflect the new formula:
                              #{mean_coef_tmp$mean_est[2]*log(x-4) + mean_coef_tmp$mean_est[1]*((x-4) - 1) + 1}
                              # mean*log(x) + mean*(x - 1) + 1
                               mutate(mean = log_mean*log(x) + lin_mean*(x - 1) + 1,
                                     mean_low = log_mean_low*log(x) + lin_mean_low*(x - 1) + 1,
                                     mean_high = log_mean_high*log(x) + lin_mean_high*(x - 1) + 1,
                                     term = paste0("p", x + 4))
                            }))

mean_coef_proj_l3 %>% select(term) %>% unique
```
...
i <- 3
for (i in 1:11) {
  a <- filter(mean_coef_df_wide_l3, site == site_df$site[i])$log_mean_high # log coef
  b <- filter(mean_coef_df_wide_l3, site == site_df$site[i])$lin_mean_high # lin coef
  p <- 0.5 # proportion of interest 
  # W(z) # is the product log function
  
  # solve a log(t) + b(t-1) + 1 - p = 0 for t
  # t = (a W((b e^(b/a + p/a - 1/a))/a))/b
  
  print(site_df$site[i])
  print((a * lambertW0((b * exp(b/a + p/a - 1/a))/a))/b + 4)
}



# plot mean half times for each site:
gg_half_time_l3 <- ggplot(data = mean_coef_df_wide_l3,
                          mapping = aes(x = fct_reorder(site, desc(half_time)), 
                                        y = half_time, color = site)) + 
  theme_classic() +
  geom_point(size = 2) + 
  #geom_errorbar(mapping = aes(ymin = half_time_low, ymax = half_time_high), width = 0.3) +
  labs(y = "Mean Time to Half (Years)", x = "Site", 
       title = "Time required for abandoned land to decay by half",
       subtitle = "lm formula: (proportion - 1) ~ 0 + log(time_1):cohort + (time_1 - 1):cohort",
       caption = "Error bars represent the mean of high and low cofficients from 95% confidence interval") + 
  # geom_vline(xintercept = 0, linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 320, vjust = 1, hjust = 0), 
        legend.position = "none", plot.caption = element_text(face = "italic"),
...
# save
png(filename = paste0(p_output, "plots/half_time_l3.png"), 
    width = 6, height = 5, units = "in", res = 400)
print(gg_half_time_l3)
dev.off()
...
gg_mean_coef_site_l3 <- ggplot(data = mutate(mean_coef_df_l3, term = fct_relevel(term, c("log", "lin"))),
...
# save
png(filename = paste0(p_output, "plots/l3_cohort_coefs_by_site.png"), 
    width = 7, height = 5, units = "in", res = 400)
...
gg_example_proportions_by_site_l3 <-
  ...
  scale_color_discrete(name = "Proportion \nremaining \nafter: ", 
                      labels = c(p5 = "5 years", p10 = "10 years", #old = "new",
                                 p15 = "15 years", p20 = "20 years", 
                                 p25 = "25 years", p30 = "30 years",
                                 p40 = "40 years", p50 = "50 years")) #+ 
  # guides(col = guide_legend(ncol = 2, byrow = TRUE))


# save
png(filename = paste0(p_output, "plots/l3_cohort_decay_proportions_by_site.png"), 
...
fitted_df_l3 <- lapply(site_df$site, FUN = function(x) {
  data.frame(site = x,
    ...
    mutate(fitted = log_mean*log(time) + lin_mean*(time - 1) + 1,
           age = time + 4)
}) %>% bind_rows()
...
gg_model_curves_by_site_l3 <- 
  ggplot(data = fitted_df_l3, 
       mapping = aes(x = age, y = fitted, color = fct_reorder(site, lin_mean))) +
  geom_line() +
  geom_hline(yintercept = 0.5, linetype = "dashed") + 
  labs(title = "Decay of abandoned land over time", 
       caption = "lm formula: (proportion - 1) ~ 0 + log(time_1):cohort + (time_1 - 1):cohort",
       ...
       color = "Site") + 
  guides(col = guide_legend(reverse = TRUE)) #+ 
  # geom_vline(xintercept = hl[9]) +
  # coord_cartesian(xlim = c(hl[9] - .3, hl[9] + .3), ylim = c(0.49, 0.51))


png(filename = paste0(p_output, "plots/model_decay_curves_by_site_l3.png"), 
    width = 7, height = 6, units = "in", res = 400)
print(gg_model_curves_by_site_l3)
dev.off()

# combo
png(filename = paste0(p_output, "plots/model_decay_curves_by_site_l3_combo.png"), 
    width = 10.5, height = 6, units = "in", res = 400)
print(plot_grid(gg_model_curves_by_site_l3 + theme(legend.position = "none") + labs(caption = ""),
                gg_model_curves_by_site_l3 + coord_cartesian(ylim = c(0, 1)) + theme(legend.position = "none"),
...
ggplot(data = coef_df_wide_l3 %>% filter(),
       mapping = aes(x = year_abn, y = half_time, group = site, color = site)) + 
  theme_classic() +
  geom_point(position = position_jitter()) + 
  #geom_errorbar(mapping = aes(ymin = conf.low, ymax = conf.high), width = 0.3) +
  labs(y = "Time until Half Remains (Years)",
       x = "Cohort (Year of Initial Abandonment)", 
       title = "Half Times",
...
gg_l3_cohort_coef_all <- 
  ggplot(data = coef_df_wide_l3 %>% 
           arrange(mean_half_time) %>%
           mutate(site_reorder = rep(1:11, each = 26),
                  cluster = cut(site_reorder, breaks = c(1, 3, 6, 9, 11),
                                labels = c("a", "b", "c", "d"), 
                                include.lowest = TRUE),
                  half_time_breaks = cut(mean_half_time, breaks = 4)),
         mapping = aes(x = year_abn, y = half_time, color = fct_reorder(site, mean_half_time))) +
  theme_classic() +
  geom_point(size = 2) + 
  labs(y = "Model Coefficient (Power Term)",
       x = "Cohort (Year of Initial Abandonment)", 
  ...
  geom_smooth(method = "lm", se = FALSE)  + 
  facet_wrap(~ cluster, labeller = labeller(cluster = c(#old = "new",
    a = "Q1, mean half time (shortest)",
    b = "Q2, mean half time",
    c = "Q3, mean half time",
    d = "Q4, mean half time (longest)")))



# ------------------------------------------------------------- #
# save:
png(filename = paste0(p_output, "plots/l3_cohort_coefs_all.png"), 
    width = 9, height = 7, units = "in", res = 400)
...
```{r l3-rate-of-change}
lm_coef_l3 <- lm(half_time ~ year_abn * site - 1, data = filter(coef_df_wide_l3, !is.na(half_time)))
...
# plot right on:
# ------------------------ use this one ---------------------------- #
gg_coef_rate_change_by_site_l3 <- ggplot(data = left_join(coef_df2_l3, mean_coef_df_wide_l3, by = "site"),
                                      aes(x = fct_reorder(site, estimate_slope), 
                                          y = estimate_slope, 
                                          color = fct_reorder(site, half_time), 
                                          fill = fct_reorder(site, half_time))) + 
  theme_classic() +
  geom_col() + 
  geom_errorbar(mapping = aes(ymin = conf.low_slope, ymax = conf.high_slope), width = 0.2) +
  geom_hline(yintercept = 0) + 
...
# save
png(filename = paste0(p_output, "plots/l3_cohort_coefs_rate_of_change_by_site.png"), 
    width = 5, height = 5, units = "in", res = 400)
print(gg_coef_rate_change_by_site_l3)
dev.off()
```
...
gg_lin_log_cohort_coef_all <- 
  ...
  labs(y = "Model Coefficient (Power Term)",
       x = "Cohort (Year of Initial Abandonment)", 
       title = "Model Coefficients", subtitle = "(proportion - 1) ~ 0 + log(time_1):cohort",
       color = "Site") + 
  theme(legend.position = "right", 
        plot.caption = element_text(face = "italic"),
        plot.margin = unit(c(0.4, 0.5, 0.2, 0.2), "cm")) + 
...
# ------------------------------------------------------------- #
# save:
png(filename = paste0(p_output, "plots/lin_log_cohort_coefs_all.png"), 
    width = 9, height = 7, units = "in", res = 400)
...
gg_mean_coef_site_lin_log <- ggplot(data = mean_coef_proj_lin_log %>% filter(term == "coef"),
  ...
  labs(x = "Mean Coefficient (log term)", y = NULL, 
       title = "Mean Decay Rates By Site",
       subtitle = "(proportion - 1) ~ 0 + log(time_1):cohort",
       caption = "Error bars represent the mean of high and low cofficients from 95% confidence interval") + 
  geom_vline(xintercept = 0, linetype = "dashed") +
  theme(plot.caption = element_text(face = "italic"))

# save
png(filename = paste0(p_output, "plots/lin_log_cohort_coefs_by_site.png"), 
    width = 5, height = 6, units = "in", res = 400)
...
png(filename = paste0(p_output, "plots/lin_log_cohort_decay_proportions_by_site.png"), 
    width = 7.5, height = 4.5, units = "in", res = 400)
...
png(filename = paste0(p_output, "plots/model_decay_curves_by_site_lin_log.png"), 
    width = 7, height = 6, units = "in", res = 400)
...
# save
png(filename = paste0(p_output, "plots/lin_log_cohort_coefs_rate_of_change_by_site.png"), 
    width = 5, height = 5, units = "in", res = 400)
...
png(filename = paste0(p_output, "plots/lin_log_cohort_coefs_rate_of_change_by_site_flip.png"), 
    width = 5, height = 6, units = "in", res = 400)
...
filter(dat_l, site == "shaanxi", year == 2000)
filter(dat_l, site == "shaanxi", year_abn == 2000) %>% 
  select(initial_area_abn) %>% unique() 

lm_log2_l$shaanxi

# coef
coef_df %>% filter(site == "shaanxi") %>% .$estimate %>% mean(na.rm = TRUE)

# fitted values, which are equal to the predicted proportion of a given cohort in a given age, using the mean coefficient for the site over the whole time-series.
filter(fitted_df, site == "shaanxi", age == 6) %>% .$fitted

# year: 2000
# cohort: 1988, in the year 2000
filter(dat_l, site == "shaanxi", year_abn == 1988) %>% 
  select(initial_area_abn) %>% unique() %>% .$initial_area_abn * 
  filter(fitted_df, site == "shaanxi", age == 2000 - 1988 + 1) %>% .$fitted
...
# to estimate the total area abandoned in a given year, Y
Y <- 2012
i <- 2012
cc_estimate_area_abn <- function(year) {
  lapply(1988:(year - 4), FUN = function(i) {
  data.frame(year_abn = i) %>% 
    mutate(initial_area_abn = filter(dat_l, site == "shaanxi", year_abn == i) %>%
             select(initial_area_abn) %>% unique() %>% .$initial_area_abn,
           exp_proportion = filter(fitted_df, site == "shaanxi", 
                                   age == year - i + 1) %>% .$fitted,
           exp_area = initial_area_abn*exp_proportion
    )
}) %>% 
  bind_rows() #%>% summarise(area = sum(exp_area)) %>% as.numeric()
}

cc_estimate_area_abn(year = 2017)
mean_coef_df %>% as_tibble()

sapply(1992:2012, FUN = function(i) {cc_estimate_area_abn(year = i)})


filter(dat_l, site == "shaanxi", year == 2000)

filter(dat_l, site == "shaanxi") %>% select(year_abn, initial_area_abn) %>% unique() %>% .$initial_area_abn %>% mean()

ggplot(data = filter(dat_l, site == "shaanxi") %>% 
         select(year_abn, initial_area_abn) %>% unique(),
       mapping = aes(x = year_abn, y = initial_area_abn)) +
  geom_col()

test_dat <- filter(dat_l, site == "shaanxi") %>% group_by(year) %>% 
  summarise(total_abn = sum(area_ha)) %>% 
  mutate(exp_area = sapply(year, FUN = function(i) {cc_estimate_area_abn(year = i)}))
...
ggplot(data = filter(dat_l, site == "shaanxi"),
  ...
  geom_line(data = test_dat, mapping = aes(x = year, y = total_abn/10^3), inherit.aes = FALSE) +
  geom_line(data = test_dat, mapping = aes(x = year, y = exp_area/10^3), inherit.aes = FALSE,
            color = "red")
...
data.frame(site = x,
           time = 1:100,
           coef = filter(mean_coef_df, term == "coef", site == x)$mean) %>%
    mutate(fitted = time ^ coef,
           age = time + 4) %>% as_tibble()
...
data.frame(site = x, 
           time = 1:100,
           coef = filter(mean_coef_df, term == "coef", site == x)$mean) %>% 
  mutate(exp_proportion = time ^ coef,
         age = time + 4
         )
