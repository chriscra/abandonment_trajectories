...
cc_estimate_area_abn <- function(year) {
  ...
  bind_rows() %>% 
  summarise(area = sum(exp_area)) %>% as.numeric()
}

cc_estimate_area_abn(year = 1993)
sapply(1992:2012, FUN = function(i) {cc_estimate_area_abn(year = i)})

data.frame(year2 = 1992:2017) %>% 
  mutate(area = sapply(year2, FUN = function(i) {cc_estimate_area_abn(year = i)}))
