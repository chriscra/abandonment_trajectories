...
lm_glance2 <- lapply(lm_log2_year_abn_l, FUN = function(x) {
  glance(x)
}) %>% bind_rows(.id = "site") %>% mutate(mod = "log2_year_abn") %>% select(site, AIC, mod, everything()) %>% arrange(AIC)

lm_glance3 <- lapply(lm_log_lin_l, FUN = function(x) {
  glance(x)
}) %>% bind_rows(.id = "site") %>% mutate(mod = "log_lin_cohort") %>% select(site, AIC, mod, everything()) %>% arrange(AIC)

lm_glance4 <- lapply(lm_0_l, FUN = function(x) {
  glance(x)
}) %>% bind_rows(.id = "site") %>% mutate(mod = "log_lin_0") %>% select(site, AIC, mod, everything()) %>% arrange(AIC)


lm_g <- bind_rows(lm_glance1, lm_glance2, lm_glance3, lm_glance4)

# view results
lm_g %>% arrange(site, AIC) %>% select(site, AIC, mod, adj.r.squared) %>% print(n = 44)
...
tidy(lm_log2_cohort_site) %>% mutate(site = sub(".*:site", "", term),
                                     cohort = gsub(".*:cohort|:site.*", "", term))
tidy(lm_log2_l$belarus)




# just site fixed effects, akin to lm_log2_no_cohort_l
...
names(lm_log2_l)
lapply(1:11, FUN = function(x) {
  print(names(lm_log2_l[[x]]))
  })
...
# mean_coef per site, see from above
mean_coef_df0

# calculate the proportion remaining at various times 
mean_coef_df <- bind_rows(mean_coef_df0, 
...
gg_example_proportions_by_site <- ggplot(data = mean_coef_df %>% filter(term != "coef"),
...
# plot right on:
# ------------------------ use this one ---------------------------- #
gg_coef_rate_change_by_site <- ggplot(data = left_join(coef_df2, mean_coef_df0, by = "site"),
...
# plot, flipped
gg_coef_rate_change_by_site_flipped <- ggplot(data = left_join(coef_df2, mean_coef_df0, by = "site"), 
                                      aes(x = fct_reorder(site, estimate_slope), 
                                          y = estimate_slope, 
                                          color = fct_reorder(site, mean)
                                          )) + 
