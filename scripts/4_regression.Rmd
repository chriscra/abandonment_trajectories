...
for (i in 1:11) {
  ...
  gg_lin_log_lin_cohort_tmp <- ggplot(data = filter(dat_l, site == site_df$site[i]), 
    ...
    geom_point() + scale_color_distiller(palette = "Greens", direction = 0) + 
...
mutate(augment(lm_log2_l[[i]],
        newdata = as_tibble(data.frame(time = 1:26,
...
ggplot(mapping = aes(x = age, y = proportion, group = year_abn, color = year_abn)) + 
    ...
    # fitted values
    geom_line(data = mutate(augment(lm_log2_l[[i]],
                                    newdata = tibble(time = rep(0:25, time = 26),
                                                     cohort = as_factor(rep(1988:2013, each = 26)))),
                            age = time + 5,
                            proportion = exp(.fitted),
                            year_abn = as.numeric(levels(cohort))[cohort])) + 
...
par(mfrow = c(3, 4))
for (i in 1:11) {
  qqPlot(lm(formula = I(proportion - 1) ~ 0 + log(time_1):cohort + I(time):cohort, 
...
dev.off()
par(mfrow = c(3, 4))
plot(lm(formula = log(proportion) ~ 0 + log(time_1):cohort + I(time):cohort, 
        data = filter(dat_l, site == site_names[i])))

plot(lm(formula = I(proportion - 1) ~ 0 + log(time_1):cohort + I(time):cohort, 
...
resid_df_log2 <- resid_df_log2 %>% mutate(mod = "log_log")
resid_df_l3 <- resid_df_l3 %>% mutate(mod = "lin_log_lin")
resid_df_lin_log <- resid_df_lin_log %>% mutate(mod = "lin_log")

mega_resid_df <- bind_rows(resid_df_log2, resid_df_l3, resid_df_lin_log)

lapply(list("lm_log2_l" = lm_log2_l,
            "lm_log2_lin_l" = lm_log2_lin_l,
            #"lm_log2_lin_l_not0" = lm_log2_lin_l_not0,
            "lm_lin_log_l" = lm_lin_log_l,
            "lm_lin_log_lin_l" = lm_lin_log_lin_l,
            ...
            #"lm_0_l" = lm_0_l,
            "lm_log_lin_l" = lm_log_lin_l), 
       FUN = function(x){lapply(x, augment) %>% bind_rows(.id = "site") %>% bind_cols(., dat_l)}
       )
...
ggplot(data = resid_df_log2[-2092, ], aes(x = time, y = .std.resid, color = year_abn)) + 
  geom_point() + scale_color_distiller(palette = "Greens") + labs(y = "Residuals", x = "Time Past Abandonment Threshold")

ggplot(data = resid_df_l3[-2092, ], aes(x = time, y = .std.resid, color = year_abn)) + 
  geom_point() + scale_color_distiller(palette = "Greens") + labs(y = "Residuals", x = "Time Past Abandonment Threshold")

png(filename = paste0(p_output, "plots/resid_time_multi.png"), 
    width = 10, height = 5, units = "in", res = 400)
print(
  ggplot(data = mega_resid_df, aes(x = time, y = .std.resid, color = year_abn)) + 
...
coef_df_l3 <- lapply(lm_lin_log_lin_l, FUN = function(x) {
  tidy(x, conf.int = TRUE) %>% 
    ...
    mutate(term_long = term,
           term = ifelse(coef_term == "I(time)", "lin", "log")) %>%
...
gg_half_time_l3 <- ggplot(data = mean_coef_df_wide_l3,
  ...
  labs(y = "Mean Time to Half (Years)", x = "Site", 
       ...
       subtitle = "lm formula: (proportion - 1) ~ 0 + log(time_1):cohort + (time):cohort",
       caption = "Error bars represent the mean of high and low cofficients from 95% confidence interval") + 
  # geom_vline(xintercept = 0, linetype = "dashed") +
  theme(axis.text.x = element_text(angle = 320, vjust = 1, hjust = 0), 
        legend.position = "none", plot.caption = element_text(face = "italic"),
...
gg_example_proportions_by_site_l3 <-
  ...
  labs(y = "Proportion of abandoned land remaining", 
       caption = "lm formula: (proportion - 1) ~ 0 + log(time_1):cohort + (time):cohort",
       #caption = "Error bars represent the mean of 95% confidence interval coefficient estimates",
       #title = "Decay rates by site",
       x = "Site" 
       ) + 
