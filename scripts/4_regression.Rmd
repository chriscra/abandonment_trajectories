...
pl1 + 
  ...
  geom_function(fun = function(x) { (x - 4) ^ mean(coef(lm(log(proportion) ~ 0 + log(time_1)*year_abn, 
                                                      data = filter(dat_l, site == "shaanxi"))), na.rm = TRUE)},
                size = 1, color = "red", linetype = "dashed", inherit.aes = FALSE) + 
...
gg_model_curves_by_site <- ggplot(data = fitted_df, 
       mapping = aes(x = age, y = fitted, color = fct_reorder(site, desc(coef)))) +
  geom_line() +
  geom_hline(yintercept = 0.5, linetype = "dashed") + 
  labs(title = "Decay of abandoned land over time", 
       caption = "lm formula: log(proportion) ~ 0 + log(time_1):cohort",
