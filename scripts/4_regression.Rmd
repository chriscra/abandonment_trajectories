...
# with quadrants:
# fct_reorder(site, mean)
site_mean_decay_order <- coef_df %>% 
  mutate(site_n = rep(1:11, each = 26)) %>% 
  arrange(mean) %>% select(site, site_n) %>% 
  unique() # %>% .$site_n

coef_df %>% arrange(mean) %>%
       mutate(#site_n = rep(1:11, each = 26),
         site_reorder = rep(1:11, each = 26),
              #site_reorder = rep(site_mean_decay_order, each = 26), 
              cluster = cut(site_reorder, breaks = 4, include.lowest = TRUE)) %>%
  select(site, mean, #site_n,
         site_reorder, cluster) %>% 
  unique() %>% arrange(site_reorder)
...
# plot, flipped
gg_coef_rate_change_by_site_flipped <- ggplot(data = coef_df2 %>% mutate(site = fct_reorder(site, estimate_slope)), 
                                      aes(x = site, y = estimate_slope, 
...
# plot right on:
gg_coef_rate_change_by_site <- ggplot(data = coef_df2 %>% mutate(site = fct_reorder(site, estimate_slope)), 
                                      aes(x = site, y = estimate_slope, 
                                          color = site, fill = site)) + 
  theme_classic() +
  geom_col() + 
...
What would the age classes look like in 30 years, if the same decay rate holds?
