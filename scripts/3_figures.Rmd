...
# save a version of the above, with the total area abandoned (>= 5) and the potential area abandoned
ggsave(plot = gg_age_class_bins_grid + 
         geom_line(data = 
...
ggsave(gg_total_c,
       file = paste0(p_plots, run_label,
                     "/carbon/carbon_accumulated_over_time.pdf"), 
       width = 7, height = 6)
...
gg_carbon_inset <-
  ggplot(data = 
           site_carbon_df %>% 
           filter(type == "Observed", threshold == 5) %>%
         ...
         mapping = aes(x = year, y = var, #linetype = type,
                       color = fct_reorder(site, var, .desc = TRUE))) +
  geom_line(size = 1.5) + 
  labs(x = "Year", 
       y = expression(#"10"^{6}*
         "Mg C / total area abandoned in 2017 (ha)"),
