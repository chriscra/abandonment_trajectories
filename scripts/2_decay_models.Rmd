...
for(i in c("a", "b", "all")
  ...
  sites_tmp <- if(sites_tmp_switch == "a") {site_df$site[1:6]
    } else {if(sites_tmp_switch == "b") {site_df$site[c(1, 7:11)]} else {site_df$site}}
...
ggsave(gg_endpoints_by_site_5_25,
  ...
  height = ifelse(sites_tmp_switch == "a|b", 6, 10), 
  units = "in"
  )
}
...
ggsave(plot = gg_half_life_endpoints_f_trim,
...
coef_df_wide_l3 %>%
  ...
  mutate(time_to_0.5 = case_when(
    year_abn < 2012 & is.na(time_to_0.5) ~ 200
  )) %>%
  print()
...
# gg_all_half_lives <-
  ...
  geom_point(data = coef_df_wide_l3 %>%
               filter(year_abn < 2012, is.na(time_to_0.5)) %>%
               mutate(time_to_0.5 = case_when(year_abn < 2012 & is.na(time_to_0.5) ~ 230)),
             shape = 17, show.legend = T, aes(linetype = "NA values"))
...
ggsave(plot = gg_half_life, 
       filename = paste0(p_plots, run_label, "/decay/", "half_lives_w_trim", run_label, ".pdf"), 
       width = 8, height = 5, units = "in")
...
ggsave(plot =
         ...
         scale_x_continuous(n.breaks = 15) + 
         theme(legend.position = c(0.76, 0.70)),
       filename = paste0(p_plots, "/fig3_ci.pdf"),
       width = 7, height = 3.5, units = "in", dpi = 400)
...
ggsave(plot =
         ...
         scale_x_continuous(n.breaks = 15) + 
         theme(legend.position = c(0.76, 0.70)),
       filename = paste0(p_plots, "/fig3_ci_trim.pdf"),
       width = 7, height = 3.5, units = "in", dpi = 400)
