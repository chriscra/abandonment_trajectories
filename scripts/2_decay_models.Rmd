...
save(coef_df_l3, 
     ...
     mean_coef_proj_l3, 
     time_to_l3,
     file = paste0(p_dat_derived, run_label, "/derived_data", "/decay_mod_coef_dfs.rds"))


load(file = paste0(p_dat_derived, run_label, "/derived_data", "/decay_mod_coef_dfs.rds"),
     verbose = TRUE)
```
...
coef_l3_mega %>% names()
coef_lin_log_mega %>% names()
coef_log2_lin_mega %>% names()
...
common_endpoint_dat_l <- lapply(10:29, function(i) {
  persistence_dat %>%
  filter(age <= i, 
         year_abn %in% unique(filter(persistence_dat, age == i)$year_abn))
})
names(common_endpoint_dat_l) <- paste0("age_", 10:29)
...
# add the mean trajectory from l3, straight mean
ggsave(plot = 
         gg_endpoints_mean_traj_all + 
         new_scale_color() +
         geom_line(data = fitted_combo %>% filter(mod == "l3", type != "no_cohort"), 
...
lm_mega_lin_log_lin_trim <- 
  lm_mega_lin_log_lin_l %>%
  stats::update(data = dat_trim)

# linear - log - linear
lm_mega_lin_log_lin_trim0 <- 
  lm(I(proportion - 1) ~ 0 + log(time + 1):cohort:site + I(time):cohort:site,  
     data = dat_trim)

all.equal(
  tidy(lm_mega_lin_log_lin_trim),
  # tidy(lm_mega_lin_log_lin_trim0),
  tidy(mod_ll_mega_trim[[4]])
)
# checks out!
...
# age_range <- 5:30
age_range <- 5:150
# type = "mean", "w_obs", "w_uncertainty", "no_cohort"

fitted_l3 <- 
  # combine fitted values for lin-log-lin models
  lapply(1:11, function(i) {
    bind_rows(
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "l3", type = "mean",
           a = filter(combo_mean_coefs_w, mod == "l3", 
                      site == site_df$site[i], coef_term == "log")$estimate,
           b = filter(combo_mean_coefs_w, mod == "l3", 
                      site == site_df$site[i], coef_term == "lin")$estimate),
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "l3", type = "no_cohort",
           a = filter(coef_l3_no_cohort_mega, mod == "l3_no_cohort", 
                      site == site_df$site[i], coef_term == "log")$estimate,
           b = filter(coef_l3_no_cohort_mega, mod == "l3_no_cohort", 
                      site == site_df$site[i], coef_term == "lin")$estimate),
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "l3", type = "w_obs",
           a = filter(combo_mean_coefs_w, mod == "l3", 
                      site == site_df$site[i], coef_term == "log")$mean_w_obs,
           b = filter(combo_mean_coefs_w, mod == "l3", 
                      site == site_df$site[i], coef_term == "lin")$mean_w_obs),
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "l3", type = "w_uncertainty",
           a = filter(combo_mean_coefs_w, mod == "l3", 
                      site == site_df$site[i], coef_term == "log")$mean_w_unc,
           b = filter(combo_mean_coefs_w, mod == "l3", 
                      site == site_df$site[i], coef_term == "lin")$mean_w_unc)
      ) %>%
      mutate(proportion = a * log(age-4) + b * ((age-4) - 1) + 1)
}) %>% bind_rows()

coefs_l3_mega
combo_mean_coefs_w$mod %>% unique()
fitted_combo$mod %>% unique()
fitted_combo %>% select(mod, type) %>% unique()
coef_l3_no_cohort_mega %>% names
names(combo_mean_coefs_w)
...
fitted_combo <-
  ...
  pivot_longer(cols = c(estimate:conf.high), values_to = "value") %>%
  pivot_wider(id_cols = c(mod, site, name), names_from = coef_term, values_from = "value") %>%
  left_join(tibble(age = 5:150), by = character()) %>%
  mutate(
...
# ----------------------------------------------------------------- #
# now, add the fitted values for lin-log and log log lin models:
# formula(lm_mega_lin_log_l)
# combo_mean_coefs_w$mod %>% unique()
# combo_mean_coefs_w %>% filter(mod == "lin_log")
...
fitted_lin_log <- 
  lapply(1:11, function(i) {
    bind_rows(
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "lin_log", type = "mean",
           a = filter(combo_mean_coefs_w, mod == "lin_log", 
                      site == site_df$site[i], coef_term == "log")$estimate),
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "lin_log", type = "w_obs",
           a = filter(combo_mean_coefs_w, mod == "lin_log", 
                      site == site_df$site[i], coef_term == "log")$mean_w_obs),
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "lin_log", type = "w_uncertainty",
           a = filter(combo_mean_coefs_w, mod == "lin_log", 
                      site == site_df$site[i], coef_term == "log")$mean_w_unc)
      ) %>%
      mutate(proportion = a * log(age-4) + 1)
}) %>% bind_rows()


# formula(lm_mega_log2_lin_l)
...
fitted_log2_lin <- 
  lapply(1:11, function(i) {
    bind_rows(
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "log2_lin", type = "mean",
           a = filter(combo_mean_coefs_w, mod == "log2_lin", 
                      site == site_df$site[i], coef_term == "log")$estimate,
           b = filter(combo_mean_coefs_w, mod == "log2_lin", 
                      site == site_df$site[i], coef_term == "lin")$estimate),
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "log2_lin", type = "w_obs",
           a = filter(combo_mean_coefs_w, mod == "log2_lin", 
                      site == site_df$site[i], coef_term == "log")$mean_w_obs,
           b = filter(combo_mean_coefs_w, mod == "log2_lin", 
                      site == site_df$site[i], coef_term == "lin")$mean_w_obs),
      tibble(age = age_range,
           site = site_df$site[i],
           mod = "log2_lin", type = "w_uncertainty",
           a = filter(combo_mean_coefs_w, mod == "log2_lin", 
                      site == site_df$site[i], coef_term == "log")$mean_w_unc,
           b = filter(combo_mean_coefs_w, mod == "log2_lin", 
                      site == site_df$site[i], coef_term == "lin")$mean_w_unc)
      ) %>%
      mutate(proportion = ((age-4) ^ a ) * exp(1) ^ (b * (age - 5)))
}) %>% bind_rows()
...
fitted_combo <- 
  ...
  mutate(proportion = ifelse(proportion >= 0, proportion, 0)) # fitted proportion can't be less than 0!
```
...
ggsave(plot =
         gg_mean_decay_by_mod,
       filename = paste0(p_plots, run_label, "/decay/", "mean_decay_by_mod", ".pdf"),
...
coef_l3_mega %>% 
  ...
  filter(
    #site %in% site_df$site[c(3,9)],
         ...
         # year_abn > 2003,
         year_abn < 2008) %>%
  pivot_wider(id_cols = c(site, year_abn, coef_term), names_from = c(site), values_from = uncertainty_range) %>%
  mutate()
