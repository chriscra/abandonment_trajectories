...
2. Running linear models of decay, with a range of specifications. The final form of these models was linear-log-linear, meaning that two time terms were included (log and linear), with fixed effects for cohort and site:
`stats::lm(formula = I(proportion - 1) ~ 0 + log(time + 1):cohort:site + I(time):cohort:site, ...)`
  a. Assessing model fit, checking assumptions, diagnostic plots for SI
3. Producing figures 
  a. for the main text of the manuscript (Fig. 6 & 7), 
  b. exploratory data analysis, 
  c. linear model coefficients,
  d. half-lives across a range of common end points
  e. alternative model specifications, and 
  f. basic plots for presentations.
4. Calculating the rate of change of half-lives at each site over time.

Note that linear models were run on a pooled dataset including all sites.
The models based on this full dataset are collectively referred to as "mega" models throughout the document.

Not all of the code is necessary; some code chunks are included largely for exploratory purposes.
Code chunks are generally meant to be walked through sequentially.
...
This script is oriented towards answering question 2, which involved fitting linear models and modeling recultivation ("decay") trajectories at each site.

These linear models involve grouping abandoned land into "cohorts" based on the year of initial abandonment at each site.
...
We then investigated how half-lives varied over time and across sites.
...
ggsave(plot = gg_AIC_mega, 
       filename = paste0(p_plots, run_label, "/decay/", "mod_AIC_mega_plot", run_label, ".pdf"), 
...
car::qqPlot(update(lm_mega_lin_log_lin_l, data = dat_trim[-c(2014, 2036),]),
...
png(filename = paste0(p_plots, run_label, "/decay/", "decay_mod_diagnostics_l3_combo", run_label, ".png"), 
    width = 8, height = 5, units = "in", res = 400)
par(mfrow = c(1, 2))
# plot(lm_mega_lin_log_lin_l, 1:2)
plot(update(lm_mega_lin_log_lin_l, data = persistence_dat[-2092, ]), 1:2)
dev.off()
...
# MEAN Coefficient plots, with uncertainty:
library(MetBrewer)
...
ggplot(data = coef_l3_mega %>% 
  ...
  stat_summary(fun.data = "mean_cl_boot", 
               mapping = aes(group = coef_term, color = coef_term),
               # geom = "errorbar",
               # width = 0.2,         
               # inherit.aes = TRUE,
               # colour = "red", 
...
#  Modeling through common endpoints
...
for(i in c("a", "b"#, "all"
  ...
  sites_tmp_switch <- i
  sites_tmp <- if(sites_tmp_switch == "a") {site_df$site[1:5]
    } else {if(sites_tmp_switch == "b") {site_df$site[c(6:11)]} else {site_df$site}}
...
gg_endpoints_by_site_5_25 <-
  ggplot(data = common_endpoint_dat %>% 
           ...
           # add full observations
           bind_rows(persistence_dat %>% 
                       mutate(endpoint = "full model", 
                              endpoint = as_factor(endpoint))) %>%
           filter(site %in% sites_tmp),
       mapping = aes(x = age, y = proportion, 
                     group = year_abn, color = year_abn)) + 
  theme_classic() + 
  geom_point(size = 0.8) +
...
# fitted lines
    geom_line(data = augment_endpoints %>% 
                ...
                # add observations for the full dataset
                bind_rows(
...
# add mean trajectories
  ...
  scale_colour_manual(name = NULL, 
                      #labels = "Mean\ntrajectory", # can specify here too, but not necessary
                      values = "red" 
                      )
...
ggsave(gg_endpoints_by_site_5_25,
  filename = paste0(p_plots, run_label, "/decay/",
                    "endpoints_by_site_5_25_",
                    sites_tmp_switch, run_label, ".pdf"), 
  width = ifelse(sites_tmp_switch == "a|b", 6.5, 6.5),
  height = ifelse(sites_tmp_switch %in% c("a", "b"), 
                  ifelse(sites_tmp_switch == "a", 7, 8), 
                  10), 
  units = "in"
  )
}
...
gg_half_life_endpoints_by_obs <-
  ...
  labs(x = "Half-life (years)", y = "Site", 
       color = "Endpoint<br>(years)", size = "Observations<br>(*n*)",
       fill = "Cohorts<br>(*n*)",) +
...
ggsave(plot = gg_half_life_endpoints_by_obs + 
       ...
       filename = paste0(p_plots, run_label, "/decay/", "half_lives_endpoints_by_obs", run_label, "_grid.pdf"), 
       width = 7, height = 6, units = "in")
...
# -------------------------------------------------------- # 
# this is the final figure to go into the main text of the manuscript
# -------------------------------------------------------- # 

# test with just circles, rather than a continuous color gradient
ggplot(data = endpoint_half_lives %>% 
           filter(endpoint %in% c(10, 15, 20, 25)) %>%
         mutate(var = paste0(endpoint, " years\n(", n_cohorts, " cohorts)")), 
       mapping = aes(x = mean, y = fct_reorder(site, mean), 
                     color = as_factor(var), group = endpoint, 
                     # fill = as_factor(n_cohorts), 
                     size = total_obs)) +
  geom_point(position = position_dodge(0.7), alpha = 0.7) + 
  scale_color_viridis(direction = -1, discrete = T) +
  
  # add another fill scale geom with number of cohorts, to get a corresponding color bar into the legend
  scale_fill_viridis(breaks = c(6, 11, 16, 21), discrete = T) +
  ...
  scale_y_discrete(labels = fancy_labels) + 
  labs(x = "Half-life (years)", y = "Site", 
       color = "Endpoint\n(n cohorts)", size = "Observations\n(n)",
       fill = "Cohorts\n(n)",) +
  
  theme_classic() +
  theme(legend.position = c(0.725, 0.2),
        legend.spacing = unit(3, units = "mm"),
        legend.background = element_blank(),
        legend.box.background = element_rect(colour = "black"),
        legend.box = "horizontal"
        ) + 
...
ggsave(plot = gg_half_life_endpoints_by_obs %+% 
         ...
         scale_x_continuous(breaks = seq(from = 10, to = 240, by = 10)) +
         theme(legend.position = c(0.675, 0.23),
               ...
               legend.box.background = element_rect(colour = "black"),
               legend.box = "horizontal"),
       filename = paste0(p_plots, run_label, "/decay/", "half_lives_endpoints_by_obs_10_25", run_label, "x.pdf"), 
...
ggsave(plot = gg_half_life_endpoints_by_obs %+% 
         ...
         theme(legend.position = c(0.675, 0.23),
               ...
               legend.box = "horizontal"),
       filename = paste0(p_plots, run_label, "/decay/", "half_lives_endpoints_by_obs_10_25", run_label, "_grid.pdf"), 
       width = 5, height = 4.5, units = "in")
...
save(lm_mega_lin_log_lin_l, 
     ...
     fitted_combo, 
     time_to_combo,
     file = paste0(p_derived2, "/decay_mod_files", run_label, ".rds"))
...
save(lm_mega_lin_log_lin_l, 
     ...
     fitted_combo, 
     time_to_combo,
     file = paste0(p_derived2, "/decay_mod_archive", run_label, ".rds"))

save(common_endpoint_dat, 
     endpoint_n,
     coef_l3_endpoints,
     augment_endpoints,
     fitted_endpoints,
     time_to_endpoints,
     endpoint_half_lives,
     file = paste0(p_derived2, "/decay_endpoints_files", run_label, ".rds"))
...
# Testing other model specifications
...
## Lumping cohorts by number of observations (e.g., 6-10 obs.)
...
## Plot alternative model specifications

Note that chunk labels refer to the list index corresponding to various models in "mod_ll_mega"
```{r}
names(mod_ll_mega)
```
...
```{r plot-decay-mod-grid-for-alt-specs}
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# plot decay mod grids for alternative model specifications: 1 and 2
# [1] "log(p) ~ 0 + log(time + 1):cohort:site"                      
# [2] "log(p) ~ 0 + log(time + 1):cohort:site + \ntime:cohort:site"
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
...
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# plot decay mod grids for alternative model specifications: 3 and 4
# [3] "p - 1 ~ 0 + log(time + 1):cohort:site"
# [4] "p - 1 ~ 0 + log(time + 1):cohort:site + \ntime:cohort:site"
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
names(mod_ll_mega)[3:4]
...
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# plot decay mod grids for alternative model specifications: 5
# [5] "p - 1 ~ 0 + log(time + 1):site + \ntime:site"
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
names(mod_ll_mega)[5]
...
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# plot decay mod grids for alternative model specifications: 6
# [6] "log(p) ~ 0 + log(time + 1):site"
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
names(mod_ll_mega)[6]
...
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# plot decay mod grids for alternative model specifications: 7
# [7] "log(p) ~ 0 + log(time + 1):site + \nlog(time + 1):year_abn:site"
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
names(mod_ll_mega)[7]
...
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# plot decay mod grids for alternative model specifications: 8
# exponential decay
# [8] "log(p) ~ 0 + time:cohort:site"
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
names(mod_ll_mega)[8]
...
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# plot decay mod grids for alternative model specifications: 11
# 11-decay-panel-l3-continuous
# [11] "p - 1 ~ 0 + log(time + 1):year_abn:site + \ntime:year_abn:site"
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
# -------------------------------------------------------------------------------------- #
names(mod_ll_mega)[11]
names(mod_ll_mega)
...
```{r mod-trajectories-shaanxi}
# ------------------------------------------------ #
# formerly for the manuscript main text (figure 4): shaanxi, lin-log-lin.
# this has been replaced with full model results for all 11 sites.
# ------------------------------------------------ #
...
gg_decay_mega_w_mean <- gg_decay_mega +
  # mean trajectory
  geom_line(
    data = fitted_combo %>% 
      filter(mod == "l3", 
...
# with the mean trajectory
ggsave(plot = gg_decay_mega_w_mean, 
       filename = paste0(p_plots, run_label, "/decay/", "decay_mod_grid_w_mean_traj", run_label, ".pdf"), 
    width = 8, height = 8, units = "in")
...
Note, these figures were removed during revisions. 
...
# Formerly MS Figure 3 (removed during revisions)
...
gg_base_s <-
  ggplot(data = persistence_dat %>% filter(site == "shaanxi"), 
    ...
    labs(x = "Time since initial abandonment (years)", 
         ...
         color = "Cohort \n(Year Abandoned)",
         linetype = "") + 
  scale_alpha_manual(name = NULL, values = c(1), guide = guide_legend(order = 2)) +
  facet_wrap(vars(site), nrow = 3,
             labeller = as_labeller(long_labels)) 

gg_s_ppt1 <- gg_base_s + geom_line(size = 1.3) + scale_color_viridis()
gg_s_ppt2 <- gg_base_s + geom_point(size = 1.3) + scale_color_viridis()
gg_s_ppt3 <- gg_base_s + geom_point(size = 1.3) + scale_color_viridis() +
    geom_line(data = mutate(augment(lm_mega_lin_log_lin_l, se_fit = TRUE),
                            time = round(exp(`log(time + 1)`), digits = 0) - 1,
                            age = time + 5,
                            proportion = .fitted + 1,
                            year_abn = as.numeric(levels(cohort))[cohort]) %>% 
                filter(site == "shaanxi"),
              linetype = 5, aes(alpha = "Fitted"))


# save basic plot
ggsave(plot = gg_s_ppt1, width = 5.5, height = 4, units = "in",
       filename = paste0(p_plots, run_label, "/decay/ppt/", 
                         "decay_mod_ex_shaanxi_lines", run_label, ".pdf"))
ggsave(plot = gg_s_ppt2, width = 5.5, height = 4, units = "in",
       filename = paste0(p_plots, run_label, "/decay/ppt/", 
                         "decay_mod_ex_shaanxi_points", run_label, ".pdf"))
ggsave(plot = gg_s_ppt3, width = 5.5, height = 4, units = "in",
       filename = paste0(p_plots, run_label, "/decay/ppt/", 
                         "decay_mod_ex_shaanxi_fitted", run_label, ".pdf"))
