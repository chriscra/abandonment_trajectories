  ...
  lapply(1:11, function(i){
    ggplot(data = site_ecoregions2017 %>% 
      ...
      theme(legend.position = "right",
            plot.margin = margin(2, 0, 2, 4, unit = "pt"),
            axis.text = element_text(size = 7.5),
            axis.text.x = element_text(angle = (360-320), vjust = 1, hjust = 1)) + 
...
gg_biomes2017_for_nonfor <-
  lapply(1:11, function(i){
    ...
    st_cast(), 
      ...
      labs(fill = NULL) +
      theme(legend.position = "none",
            # plot.margin = margin(2, 10, 2, 4, unit = "pt"),
            axis.text = element_text(size = 7.5),
            axis.text.x = element_text(angle = (360-320), vjust = 1, hjust = 1),
...
input <- "site_forest_soc_combined"
i <- 5
for (input in c(
  "site_forest_soc_combined"#, "site_forest_soc_combined_per_pixel"
  )) {
  for(x in 1:2) {
  layer_index <- x # change here to do Y20 and YSS
  
  pdf(file = paste0(p_output, "plots/", run_label, "/carbon/",
                    gsub("site_", "", input), 
                    if(layer_index == 1) "_Y20" else "_YSS",
                    run_label, ".pdf"),
      width = 7, height = 6)
  ...
  for (i in 1:11) {
    terra::plot(
      if (i %in% c(5, 8, 10)) { # rounding iraq, orenburg, and volgograd to prettify the legend
        round(get(eval(input))[[i]][[layer_index]], digits = 2) 
        } else  {
          get(eval(input))[[i]][[layer_index]]
          },
      type = ifelse(input == "site_savanna_mask", "classes", "continuous"),
      # type = "continuous",
      col = viridis(n = 10, direction = -1), 
