...
rmarkdown::render("scripts/Crawford_abandonment_MS+SI.Rmd",
                  output_file = paste0(p_proj, "writing/", "Crawford_abandonment_MS+SI.pdf"),
...
load(file = paste0(p_derived2, "/decay_mod_coef_dfs", run_label, ".rds"), 
     verbose = TRUE)
...
However, those environmental benefits depend on the duration and persistence of abandonment, which is poorly known.
...
At most sites, we project that \>50% of abandoned croplands will be recultivated within 25 years, limiting their ability to accumulate substantial amounts of carbon and biodiversity.
...
Short-term abandonment limits environmental gains^[Alternative: Cropland abandonment has limited environmental benefit]
...
Optimistic scenarios in which biodiversity loss is reversed often rely on forest and grassland regeneration in former agricultural lands [@Henry2019; @Martins2020; @Leclere2020; @Folberth2020].
...
Tens of millions of hectares of agricultural lands have been abandoned since 1950 [@Li2017], and while abandonment is predicted to continue in many places [@Sanderson2018; @Popp2017], for how long and to what end remains unclear.
...
Knowing how long abandonment persists is therefore critical to understanding its potential to help mitigate the ongoing climate and biodiversity crises, yet this question of durability has received surprisingly little attention from researchers.
...
Yin et al. [-@Yin2020] used a trajectory-based approach to produce accurate maps of annual cropland abandonment at 30-m resolution from 1987 to 2017 without relying on woody revegetation as a proxy of abandonment, thereby allowing for a direct measurement of abandonment at earlier stages of regeneration and in different biomes.

Here, we utilize this new time-series [@Yin2020] to quantify the duration and persistence of abandonment at eleven sites across four continents (Fig. \@ref(fig:maps-abn-duration)).
We address three questions: (1) How long does abandoned cropland stay abandoned on average (i.e., duration of abandonment), and how does this vary geographically?
(2) How quickly is abandoned cropland recultivated (i.e., persistence of abandonment), and do recultivation (or "decay") rates vary through time?
(3) If abandonment and recultivation continue at current rates, what will ultimately be the mean age of abandoned croplands?

By making use of more accurate and finer resolution abandonment maps for a longer period and a broader set of sites than previous studies, we provide the most detailed analysis to date of the duration and persistence of cropland abandonment.
...
```{r maps-abn-duration, fig.cap = '(ref:caption-maps-abn-duration)', fig.pos = "h", echo=FALSE}
...
area_abn_stats <- list(
  ...
  "min_area_2017_as_prop_total_crop_site" =  area_summary_df %>% 
    filter(site != "mato_grosso") %>%
    slice_min(area_2017_as_prop_total_crop) %>% .$site %>% capwords(),
...
lc_stats <- list(
  ...
  forest_min_site = abn_prop_lc_2017 %>% 
    filter(lc_2017 == 2) %>% slice_min(prop_lc_area_ha) %>% .$site %>% capwords(),
  ...
  forest_max_site = abn_prop_lc_2017 %>% filter(lc_2017 == 2) %>% slice_max(prop_lc_area_ha) %>% .$site %>% capwords()
  )
...
extrapolation_stats <- list(
  ...
  "prop_over_30_upper_range_site" = extrap_prop %>% arrange(desc(prop_over_30)) %>% filter(!site %in% c("shaanxi", "mato_grosso")) %>% slice_max(prop_over_30) %>% .$site %>% capwords(),
   ...
   "prop_over_50_shaanxi" = extrap_prop %>% filter(site == "shaanxi") %>% .$prop_over_50 %>% round(digits = 2)
)
...
We found that `r area_abn_stats$total_area_ever_abn_Mha_all_sites` million ha (Mha) of croplands were abandoned at least once between 1987 and 2017 across our eleven sites.
...
At individual sites, the area of cropland abandoned at least once ranged from `r area_abn_stats$min_area_ever_abn_as_prop_total_crop`% (`r area_abn_stats$min_area_ever_abn_as_prop_total_crop_site`, USA) to `r area_abn_stats$max_area_ever_abn_as_prop_total_crop`% (Vitebsk, `r area_abn_stats$max_area_ever_abn_as_prop_total_crop_site`/Smolensk, Russia) of the total cropland extent (Figs. \@ref(fig:area-abn-panel) and \@ref(fig:area-abn-by-age-class)), except for Mato Grosso, Brazil, where only `r area_abn_stats$mg_ever_abn_as_percent_total_crop`% was abandoned at least once (see Section \@ref(mato-grosso)).

However, we also found that many of these abandoned croplands were recultivated: `r recult_stats$mean_thr_5`% of abandoned area had been recultivated as of 2017, on average, across sites (SD = `r format(recult_stats$sd_thr_5, nsmall = 2)`%; Fig. \@ref(fig:recult-by-threshold)).
In fact, only `r area_abn_stats$total_area_abn_Mha_2017_all_sites` Mha of croplands remained abandoned as of 2017 across our sites (Fig. \@ref(fig:maps-abn-duration)).
...
Accordingly, the mean duration of abandonment across all sites was short: `r abn_duration_stats$mean_of_means_duration` years (SD = `r abn_duration_stats$sd_of_mean_duration`), ranging from `r abn_duration_stats$duration_min` (`r abn_duration_stats$duration_min_site`, Russia/Uralsk, Kazakhstan) to `r format(abn_duration_stats$duration_max, nsmall = 2)` years (`r abn_duration_stats$duration_max_site`; Fig. \@ref(fig:mean-abn-duration)).
...
```{r mean-abn-duration, fig.cap = '(ref:caption-mean-abn-duration)', fig.pos = "h", out.width = "11cm", echo=FALSE}
...
## Modeling abandonment decay & recultivation {.unnumbered}
...
These models also allow us to calculate the mean recultivation (or "decay") trajectory at each site (Fig. \@ref(fig:decay-curves-by-site)).
To do so, we defined a cohort of abandoned cropland as all cropland abandoned in a given year at a given site.
We modeled the proportion of each cohort remaining abandoned as a function of time since initial abandonment (see Fig. \@ref(fig:abn-decay-s), Materials and Methods).
Given that some abandonment periods are inherently limited by the length of our time series (and that cropland may remain abandoned beyond the three decades covered by our data), tracking recultivation rates by year abandoned provides a more accurate estimate of persistence.

Recultivation occurred quickly: we predict that >50% of abandoned croplands will be recultivated within 25 years of initial abandonment at almost all sites (Fig. \@ref(fig:decay-curves-by-site)).
At individual sites, the half-life (defined as the time required for half of the croplands abandoned in a given year to be recultivated) ranged from `r decay_stats$t0.5_min` (`r decay_stats$t0.5_min_site`/Uralsk) to `r decay_stats$t0.5_max` years (`r decay_stats$t0.5_max_site`).
Shaanxi/Shanxi (China) was a notable exception; here our models predicted a half-life of `r decay_stats$shaanxi_0.5` years, and `r decay_stats$shaanxi_0` years for complete turnover (defined as the time required for all abandoned cropland in a given cohort to be recultivated; Fig. \@ref(fig:decay-curves-by-site)).
...
Most sites showed a negative trend in the time until recultivation, indicating that abandoned croplands were being recultivated more quickly in recent years (Fig. \@ref(fig:decay-rate-of-change)).
Based on a linear regression on the half-life, over half of our sites had rates of change that were significantly different from zero and negative, corresponding to recultivation accelerating through time: Bosnia & Herzegovina, Volgograd (Russia), Wisconsin, Nebraska, Orenburg/Uralsk, and Iraq (Fig. \@ref(fig:decay-rate-of-change)).
...
## Projecting future abandonment and recultivation {.unnumbered}

Assuming that abandonment rates and recultivation rates remain constant at each site, our projection suggests that the impermanence of abandonment we observed is not an artifact of our time series being only 30 years long.
We project the mean duration of abandonment to plateau at 20 years or less at most sites (Fig. \@ref(fig:extrapolation-combo)), with the notable exception of Shaanxi/Shanxi, which plateaus at a mean duration of `r extrapolation_stats$plateau_max` years.
...
Furthermore, we project that only a relatively small proportion of abandoned croplands will persist beyond 30 years (<`r extrapolation_stats$prop_over_30_upper_range`% at most sites; Fig. \@ref(fig:extrapolation-area-by-age)).
...
Furthermore, estimates of abandonment based on just two points in time (e.g., 83 Mha estimated in ref. [@Naess2021]) may underestimate abandonment by excluding any new cultivation and subsequent abandonment that takes place between the chosen time points.
...
Our projected half-lives of 12-24 years at most sites were similar to those found in Costa Rica ($\leq20$ years, ref. [@Reid2019]), but slightly longer than in other parts of the Neotropics, where empirical evidence shows that secondary forest regeneration is often short-lived [@Chazdon2016a; @Nunes2020; @Smith2020; @Schwartz2020].
...
## Implications for biodiversity recovery and carbon sequestration {.unnumbered}
...
Moreover, the area around these sites retained relatively high forest cover (76% on average), however, and recovery is likely to be slower in more deforested landscapes [@Rozendaal2019].
...
Based on estimates of potential aboveground carbon accumulation rates during the first 30 years of natural regeneration in different biomes [@Cook-Patton2020], we predict that the abandoned cropland areas at our sites that are regenerating as forests (see Fig. \@ref(fig:abn-prop-lc)) could accumulate 30-70 Mg C/ha in aboveground biomass total over 20 years, or 18%-45% of the 110-250 Mg C/ha total that could be accumulated in aboveground biomass over 100 years [@Cook-Patton2020].
...
Moreover, beyond the short-lived duration of abandonment, the very fact that these lands were used for agriculture can impede full achievement of carbon or biodiversity goals.
...
Our eleven sites were mapped with high accuracy (average overall accuracy $85\pm4$%) and provide broad coverage of different continents and ecosystems (Fig. \@ref(fig:maps-abn-duration)).
...
## Calculating abandonment duration {.unnumbered}
...
Model assumptions were tested through visual inspection of diagnostic plots (Figs. \@ref(fig:diag-resid-fitted) and \@ref(fig:diag-qq), full details in Section \@ref(decay-models-si)).
We calculated each site's mean recultivation trend by taking the mean of the model coefficients across all cohorts (Fig. \@ref(fig:mean-abn-duration)).
Modeled decay trajectories are shown for one example site, Shaanxi/Shanxi (China), in Fig. \@ref(fig:abn-decay-s), and model coefficients for all sites in Fig. \@ref(fig:decay-mod-coef).

To estimate changes in persistence over time, we calculated the time required for half of the abandoned cropland in each cohort to be recultivated (the "half-life") based on the modeled recultivation trajectory of each cohort respectively.
We parameterized a linear model on these half-life values (*n* = 26) to identify temporal changes in recultivation patterns at each of our eleven sites (Section \@ref(section-methods-rate-of-change)).
...
Based on our modeled decay rates, we project abandonment and recultivation in the future based on two assumptions: (1) a constant amount of cropland is newly abandoned each year, based on the mean area of cropland abandoned each year at each site (Fig. \@ref(fig:turnover-grid)); and (2) all abandoned croplands are recultivated at the mean predicted rate for each site (Fig. \@ref(fig:decay-curves-by-site)).
Total abandonment in our projection remained below the total cropland extent at each site.
We also considered an alternative assumption in which annual abandonment linearly declined to 0 between 2017 and 2050, but found similarly short mean abandonment durations (below 37 years at all sites by 2050; see Figs. \@ref(fig:extrapolation2-combo) and \@ref(fig:extrapolation2-area-by-age)).
...
Taken together, these 11 models correspond to a pooled model for data across all 11 sites, as described by a `stats::lm()` call of `lm(formula = I(proportion - 1)  ~ 0 + log(time + 1):cohort:site + I(time):cohort:site)`.
...
```{r abn-panel-table, echo = FALSE, include = FALSE}

area_summary_df %>% 
  select(site, area_abn_ha_2017, area_ever_abn_ha, area_2017_as_prop_total_crop) %>% 
  left_join(., select(site_df, site, description)) %>%
  select(site = description, everything(), -site) %>%
  arrange(area_2017_as_prop_total_crop) %>%
  mutate(
    area_abn_ha_2017 = area_abn_ha_2017 %>% round(digits = 0) %>% prettyNum(big.mark = ","), 
    area_ever_abn_ha = area_ever_abn_ha %>% round(digits = 0) %>% prettyNum(big.mark = ","), 
    area_2017_as_prop_total_crop = round(area_2017_as_prop_total_crop*100, digits = 2) %>% 
      paste0(., "%")
    ) %>%
# c("Site", "Area (annual, as of 2017)", 
#                            "Area (two year: 2017-1987)", 
#                            "Percent Difference", "Jaccard Similarity",
#                            "Percent Area <5 Years Old (two year)")
knitr::kable(., 
             col.names = c("Site", "Area Abandoned, 2017 (Mha)", 
                           "Area Ever Abandoned (Mha)",
                           "Area Abandoned, 2017, as percent of total cropland extent"),
             align = "r", #format = "simple",
             format = "latex",
             booktabs = TRUE,
             caption = "Area abandoned as of 2017, at any point between 1987-2017, and as of 2017 as a percent of the total cropland extent (i.e., the area of all lands that were cultivated at some point during the time series). This table replicates Figure S2. Note that sites are shown in ascending order of area abandoned as of 2017 as a proportion of total cropland extent."
             ) %>%
  kableExtra::kable_styling(
    # latex_options = "scale_down"
    latex_options = "hold_position"
