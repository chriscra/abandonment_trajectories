...
However, those environmental benefits depend on the persistence of abandonment, which is poorly known.
...
Tens of millions of hectares of agricultural lands have been abandoned since 1950 [@Li2017; @Chazdon2020], and recent estimates from satellite imagery indicate that as much as $115.5\pm24.1$ Mha of gross cropland abandonment took place globally between 2003 and 2019, amidst $217.5\pm37.7$ Mha of gross cropland expansion [@Potapov2022].
Expansion remains the dominant threat to biodiversity and carbon stocks, yet many analysts have viewed demographic shifts as creating an opportunity for natural ecosystems to regenerate on abandoned croplands, in the process restoring biodiversity [@Chazdon2020; @Xie2019; @Navarro2012; @Perino2019], sequestering carbon [@Griscom2017; @Chazdon2016a; @Yang2019], and potentially offsetting some of the impact of expansion.
...
While abandonment is predicted to continue in Europe, Russia and Central Asia, East Asia, and North America [@Sanderson2018; @Popp2017; @Leclere2020], for how long (and to what end)^[VR, HY recommend removing this clause] remains unclear.
...
Importantly, the extent to which agricultural expansion is offset by abandonment, which is the underlying assumption when net statistics are reported, depends on its duration.
...
Knowing how long abandonment persists for is therefore critical to understanding its potential to help mitigate the ongoing climate and biodiversity crises, yet this question of persistence has received surprisingly little attention from researchers.

Until recently, it has been difficult to gather detailed information on the duration and long-term trajectories of abandonment.
...
Here, we utilize these time-series [@Yin2020] to quantify the magnitude and persistence of abandonment at eleven sites across four continents (Fig. \@ref(fig:global-map)).
We address four questions: 
(1) How long did abandoned cropland stay abandoned on average during our time series, and how does this vary among sites?
(2) If we model the persistence of abandonment as a decay process as a function of time, how long do abandoned croplands persist before they are recultivated, and do recultivation (or "decay") rates vary through time and among sites?
...
(Given that land use trends in Mato Grosso were dominated by expansion, not abandonment, we exclude this site from the general results, unless otherwise noted; see detailed results in Section \@ref(mato-grosso).)
...
`r recult_duration_stats$duration_max_site`, had a mean recultivation length of `r recult_duration_stats$duration_max` years, whereas recultivation at the other sites lasted between `r recult_duration_stats$duration_min` and `r recult_duration_stats$duration_2nd_highest` years (Fig. \@ref(fig:mean-recult-duration)).
...
These models also allow us to explore how recultivation rates changed over time at each site, and to compare mean recultivation (or "decay") trajectories among sites (see Materials and Methods).
...
Here, our models predicted much longer half-lives: greater than 50 years for endpoints of 16 years or longer (Figs. \@ref(fig:endpoint-half-lives) and \@ref(fig:endpoint-half-lives-all)).
...
This pattern was strongest in Bosnia & Herzegovina^[indicating that future persistence may be relatively limited], and a linear regression on the half-life confirmed this to be the only site to have a statistically significant and negative rate of change in half-life, where more recent cohorts were recultivated more quickly (Fig. \@ref(fig:decay-rate-of-change)).
...
These results highlight the importance of interpreting abandonment in a dynamic, annual context, and provide context for interpreting recent global estimates of abandonment.
Estimates of abandonment based on just two points in time (e.g., 83 Mha recently estimated in ref. [@Naess2021]) may underestimate abandonment by excluding any new cultivation and subsequent abandonment that takes place between the chosen time points. 
More importantly, these two-timepoint estimates almost certainly overestimate the amount of persistent abandonment, due to both a failure to exclude short-term fallow periods, and, as our analysis shows, high rates of recultivation. 
When comparing our annual results to an estimation of “abandonment” based on cultivation in only two points in time (1987 and 2017), we find `r round((area_abn_stats$total_area_abn_Mha_2017_all_sites - abn_2yr_stats$total_area_abn_Mha) / area_abn_stats$total_area_abn_Mha_2017_all_sites, digits = 4) * 100`% less abandonment as of 2017 (with even greater variation at individual sites), the inclusion of substantial amounts of “abandonment” that did not meet our five-year abandonment definition, and low spatial agreement between areas identified as abandoned (Table \@ref(tab:twoyr-diff-table), Section \@ref(twoyr-vs-annual)).

Our results also demonstrate the importance of considering dynamics of abandoned croplands as a function of time since abandonment, rather than relying only on the average durations observed during a time series.
This is apparent in Shaanxi/Shanxi, China: although this site had one of the shortest mean abandonment durations (because most abandonment took place towards the end of the time series), current recultivation trajectories indicate more persistent abandonment than we are able to observe during the time series, as indicated by some of the most consistently long half-lives at our sites (~30 years; Figs. \@ref(fig:endpoint-half-lives) and \@ref(fig:endpoint-half-lives-all)).
By modeling recultivation as a function of the time since initial abandonment, we estimate that half of the croplands abandoned in a given year will be recultivated within 10-30 years at almost all of our sites (Fig. 7).
...
This recultivation rate was similar to our most persistent site (`r recult_stats$min_5_site`, `r recult_stats$min_5`%), but was much lower than our average recultivation rate across sites (`r recult_stats$mean_thr_5`%), and lower still than our sites closest to Kazakhstan, Orenburg/Uralsk and Volgograd, which had higher recultivation rates of `r area_recult_threshold %>% filter(abn_threshold == 5, site == "orenburg") %>% .$proportion_recultivated %>% round(digits = 4) * 100`% and `r area_recult_threshold %>% filter(abn_threshold == 5, site == "volgograd") %>% .$proportion_recultivated %>% round(digits = 4) * 100`% respectively (Fig. \@ref(fig:recult-by-threshold)).
...
Across the tropics, only 33% of forests regenerating after recent deforestation were $\geq10$ years old [@Vancutsem2021].
...
Even the sites with the most persistent abandonment are unlikely to retain large areas of croplands abandoned continuously for $\geq50$ years.
...
In some cases species-rich tropical ecosystems can regenerate quickly, but never on a time scale as short as the abandonment durations we observed here.
...
Furthermore, these estimates may be overly optimistic, due to positive site selection bias [@Reid2018b] in that the area around these sites retained relatively high forest cover (76% on average), and recovery is likely to be slower in more deforested landscapes [@Rozendaal2019].
...
A few additional considerations and limitations are worth noting.
First, there are other factors that can significantly affect whether abandonment results in biodiversity and carbon sequestration beyond the short-lived duration of abandonment, which we focus on here.
For example, the very fact that these lands were used for agriculture can impede full achievement of carbon or biodiversity goals.
...
While natural regeneration is typically cheaper than active restoration and can be a viable and important strategy in many places, it is not clear that natural regeneration outperforms active restoration on abandoned croplands.
Meta-analyses suggesting that natural regeneration outperforms active restoration often suffer from site-selection biases, in that sites that did not regenerate naturally are not included, whereas examples of unsuccessful active restoration typically are [@Reid2018b].
Furthermore, more heavily altered systems typically require active restoration approaches such as invasive species control or translocation of plants and animals, and experience longer recovery times [@Crouzeilles2017; @Gilroy2014; @Meli2017; @Yang2019].
...
Although we focus here on the potential for long-term regeneration of vegetation on abandoned cropland to enhance biodiversity, there are cases where short-term regeneration can favor certain aspects of biodiversity. 
In those cases, active intervention and management (through grazing, hunting, or otherwise) may be necessary to maintain grasslands and other high-biodiversity habitats such as wetlands [@Navarro2012; @ReyBenayas2007].

Second, we emphasize that our sites were specifically selected based on documented abandonment.
Thus, while our results are likely representative of areas that have experienced substantial abandonment (with the exception of Mato Grosso, Brazil), they are not a representative sample of the globe, and focus on areas with more abandonment than the global average. 
Given the ephemerality of abandonment in these hotspot regions, it is plausible to expect regions with less abandonment overall to have equally short-lived abandonment, if not more so, when it occurs. 
...
At the most basic level, abandonment is related to agricultural suitability [@Levers2018]. 
However, the proximate drivers are complex and site-specific, but often encompass a combination of state policy, socioeconomic and demographic changes, and environmental factors [@Yin2019; @Prishchepov2021; @Meyfroidt2016; @Levers2018]. 
Abandonment is often a byproduct of urban migration and rural depopulation, particularly in Europe, East Asia, and Latin America [@ReyBenayas2007; Sloan2022].
Other cases are more directly related to sociopolitical changes. 
For example, the collapse of the Soviet Union triggered large-scale abandonment in Eastern Europe, especially in less productive areas that had been supported by state subsidies (e.g., Orenburg, Russia/Uralsk, Kazakhstan) [@Prishchepov2013; @Estel2015; @Alcantara2013; @Meyfroidt2016].
Abandonment in these economically marginal lands is more likely to persist [@Smaliychuk2016]. 
...
Indeed, we found relatively durable abandonment in Shaanxi/Shanxi (with some of the most consistently long half-lives), suggesting the effectiveness of China’s reforestation policy. 
The persistence of abandonment in this region, however, depends on the consistency and continuity of national land use policy [@Yin2018a].
Recultivation may occur following abandonment when short-term stressors are removed, as is frequently observed in post-conflict regions [@Yin2019; @Olsen2021], when subsidies for cultivation are reestablished [@Meyfroidt2016], when incentives for reforestation are removed [@Yin2018a], or when migration pathways are disrupted [@Sloan2022]. 
Our research demonstrates the value of long-term analysis at annual intervals, which has the potential to improve our understanding the drivers of abandonment and recultivation.

Even once abandonment occurs, there are many socioeconomic and political barriers hindering habitat regeneration in abandoned croplands.
...
Behavioral research highlights the importance of human factors such as corruption, political and institutional support, and demographic changes for decisions to abandon or recultivate cropland, alongside biophysical and environmental conditions [@Prishchepov2021a].

In order for abandoned croplands to reach similar levels of carbon stocks and biodiversity, they must persist for more than 50 years [@Acevedo-Charry2019; @Rozendaal2019; @Nerlekar2020; @Isbell2019; @Poorter2016; @Yang2019] – at least three decades longer than the mean length of abandonment we observe here.
Reducing recultivation and lengthening regeneration on abandoned croplands may be difficult, but the temporal dynamics of recultivation at our sites may inform the feasibility of doing so 
When recultivation occurred, its duration was relatively short (Fig. \@ref(fig:mean-recult-duration)). These short recultivation periods may partly be a function of when recultivation took place (which must follow abandonment by at least five years, by definition), but could also indicate that these croplands continue to be marginal economically, and that taking them out of production for longer periods of time may be an option.

Encouraging the persistence of abandonment could be achieved by designating abandoned fields as protected areas, incorporating natural regeneration into payments for ecosystem service programs to allow landowners to benefit economically, or by taking steps to support sustainable long-term cultivation of some sites, thereby reducing turnover among fields that have previously been part of long-term fallowing cycles.
The relative durability of cropland abandonment in Shaanxi/Shanxi Province provides some evidence that such incentive programs designed to restore or reforest cropland are indeed possible. 
This is encouraging, yet there continues to be a need to improve the biodiversity outcomes of such programs, many of which prioritize the planting of non-native forests at the expense of local biodiversity, ecosystem stability, and local livelihoods [@Hua2016]. 
More concerningly, recent evidence indicates that some large-scale reforestation programs fail to provide either environmental or livelihood benefits; this may especially be the case when non-forested biomes are targeted for afforestation, or when programs lack appropriate consideration of the local socioeconomic context [@Coleman2021]. 
Clearly, more research is needed to improve the outcomes of these sorts of programs, and we stress that any policies should be developed jointly with local communities to address trade-offs between biodiversity, carbon storage, and livelihoods.
...
Furthermore, Yin et al. [@Yin2020] selected these regions because of their documented histories of cropland abandonment in the past three decades. 
...
Here, we use data for 11 of the 14 areas originally mapped by Yin et al. [@Yin2020], dropping three sites (Sardinia, Nepal, and Uganda) due to low map accuracy (classification accuracy for abandonment < 0.4^[He: Can we include a statistic here that we used to exclude these three sites? Something like: <80% overall map accuracy. Or was this based on a few different metrics, like overall map accuracy, F1 scores, cropland map accuracy, etc.?]).
Accordingly, our study sites are not a representative sample for the globe, but are likely representative of those areas that have recently experienced abandonment.
...
We prioritize intermediate endpoints in our reporting of the results for two reasons: longer endpoints are biased towards older cohorts (by excluding more recent abandonment), and while shorter endpoints incorporate a more complete set of cohorts, they make use of fewer observations overall and place relatively more weight on the initial stages of abandonment.
...
## Comparing approaches to measuring abandonment: annual vs. two-timepoint estimates {#twoyr-vs-annual}
...
*Insert stats here*
To illustrate this point, if we estimate abandonment at our sites based on cultivation in only two points in time (1987 and 2017), we find only `r format(abn_2yr_stats$total_area_abn_Mha, nsmall = 2)` Mha of "abandonment," a `r  round((area_abn_stats$total_area_abn_Mha_2017_all_sites - abn_2yr_stats$total_area_abn_Mha) / area_abn_stats$total_area_abn_Mha_2017_all_sites, digits = 4) * 100`% underestimate compared to the `r area_abn_stats$total_area_abn_Mha_2017_all_sites` Mha of abandonment as of 2017 observed using our full annual time series.
This "abandonment" area ranged from `r abs(abn_2yr_stats$low_percent)`% less (`r abn_2yr_stats$low_p_site`) to `r abn_2yr_stats$high_percent`% more (`r abn_2yr_stats$high_p_site`) area abandoned at each site than the area identified using the full time series, and much of this "abandonment" incorporated croplands that had been abandoned for less than five years and therefore did not meet our definition of abandonment (ranging from `r abn_2yr_stats$percent_area_below_5_low`% to `r abn_2yr_stats$percent_area_below_5_high`% of area at all sites except Mato Grosso; see Table \@ref(tab:twoyr-diff-table)). 
Additionally, the two-timepoint assessments identify different areas of abandonment from those derived from our full annual time series, with spatial agreement ranging from `r abn_2yr_stats$jaccard_low`% (`r abn_2yr_stats$jaccard_low_site`) to `r abn_2yr_stats$jaccard_high`% (`r abn_2yr_stats$jaccard_high_site`) (see Table \@ref(tab:twoyr-diff-table), Section \@ref(twoyr-vs-annual)).
*Insert stats here*
...
Accordingly, pixels that were cropland or non-cropland classes throughout time were excluded.
This also meant that we excluded periods of non-cropland that began in the first year of the time series, because of a lack of information on whether this non-cropland had been previously cultivated.
Accordingly, if pixels were initially non-cropland, then changed to cropland and then were abandoned, we only counted these last periods as abandonment.
...
Though this pixel includes two periods of non-cropland classification of five or more years, we only include the second, nine year period, because it clearly followed cropland classification.
